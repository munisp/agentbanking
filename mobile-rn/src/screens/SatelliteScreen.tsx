...
const ConnIcon = ({ item }: { item: RecordItem }) => {
    const st = item.status || 'disconnected';
    const icons: Record<string, string> = { connected: '🟢', syncing: '🔄', failover: '🟡', disconnected: '🔴' };
    return (<View style={{ flexDirection: 'row', alignItems: 'center' }}>
      <Text>{icons[st] || '❓'} </Text><Text style={{ fontSize: 11, color: st === 'connected' ? '#22c55e' : '#ef4444' }}>{st}</Text></View>);
  };
...
export default function SatelliteScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/satellite.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/satellite.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Satellite Connectivity...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Satellite Connectivity</Text>
            <Text style={styles.subtitle}>Starlink/AST rural connectivity</Text>
          </View>
          <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📡</Text>
            <Text style={styles.statLabel}>Active Links</Text>
            <Text style={styles.statValue}>{stats?.activeLinks ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🔄</Text>
            <Text style={styles.statLabel}>Failovers</Text>
            <Text style={styles.statValue}>{stats?.failoversToday ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>☁️</Text>
            <Text style={styles.statLabel}>Data Synced</Text>
            <Text style={styles.statValue}>{stats?.dataSynced ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🌍</Text>
            <Text style={styles.statLabel}>Coverage</Text>
            <Text style={styles.statValue}>{stats?.coveragePercent ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.agentCode || `Record #${item.id}`}</Text>
