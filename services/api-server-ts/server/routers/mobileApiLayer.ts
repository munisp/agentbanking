...
function validateMobileapilayerInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_MOBILEAPILAYER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_MOBILEAPILAYER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_MOBILEAPILAYER.validateRange(data.amount, 0, 100_000_000)
    )
  ...
  push: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "mobileApiLayer",
        "mutation",
        "Executed mobileApiLayer mutation"
      );
