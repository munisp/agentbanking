...
from sqlalchemy import (
    Column, String, Integer, Numeric, Boolean, DateTime, Time,
...
class StoreLocation(Base):
    ...
    opening_hours = Column(Text, nullable=True)         # JSON: {mon: {open: "08:00", close: "18:00"}, ...}
...
class StoreReview(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    __table_args__ = (
        Index("ix_review_store", "store_id"),
    )
...
class StoreMapService:
    ...
    def register_store(self, req: StoreLocationCreate) -> StoreLocation:
        ...
        existing = self.db.query(StoreLocation).filter(StoreLocation.entity_id == req.entity_id).first()
        if existing:
        ...
        store = StoreLocation(
            ...
            state=req.state,
            phone=req.phone,
            services=json.dumps([s.value for s in req.services]) if req.services else None,
        ...
        logger.info(f"Registered store {req.entity_id} at ({req.latitude}, {req.longitude})")
        return store
    ...
    def find_nearby_stores(self, req: NearbySearchRequest) -> List[Dict]:
        ...
        q = self.db.query(StoreLocation).filter(
            StoreLocation.latitude.between(req.latitude - lat_delta, req.latitude + lat_delta),
            StoreLocation.longitude.between(req.longitude - lng_delta, req.longitude + lng_delta),
        ...
        for store in candidates:
            ...
            if dist <= req.radius_km:
                ...
                results.append({
                    ...
                    "phone": store.phone,
                    "last_active_at": store.last_active_at.isoformat() if store.last_active_at else None,
    ...
    def get_map_clusters(self, zoom_level: int, state: Optional[str] = None) -> List[MapCluster]:
        """
        Return store clusters for map rendering at a given zoom level.
        Lower zoom = larger clusters; higher zoom = individual stores.
        """
        q = self.db.query(StoreLocation).filter(StoreLocation.status != StoreStatus.SUSPENDED)
        if state:
        ...
        return [
            MapCluster(
                cluster_id=hashlib.sha256(str(k).encode()).hexdigest()[:8] if True else str(k),
    ...
    def get_coverage_stats(self) -> StoreCoverageStats:
        ...
        active = self.db.query(StoreLocation).filter(StoreLocation.status == StoreStatus.OPEN).count()
    ...
    def add_review(self, req: StoreReviewCreate) -> StoreReview:
        ...
        store = self.db.query(StoreLocation).filter(StoreLocation.id == req.store_id).first()
        if store:
    ...
    def update_store_status(self, entity_id: str, status: StoreStatus):
        store = self.db.query(StoreLocation).filter(StoreLocation.entity_id == entity_id).first()
        if store:
            store.status = status
            store.last_active_at = datetime.utcnow() if status == StoreStatus.OPEN else store.last_active_at
            self.db.commit()

    def get_store(self, entity_id: str) -> Optional[StoreLocation]:
        return self.db.query(StoreLocation).filter(StoreLocation.entity_id == entity_id).first()
