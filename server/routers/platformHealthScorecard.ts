...
const acknowledgeAlert = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed platformHealthScorecard mutation"
    );
...
function validatePlatformhealthscorecardInput(
...
const INTEGRITY_RULES_PLATFORMHEALTHSCORECARD = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PLATFORMHEALTHSCORECARD.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PLATFORMHEALTHSCORECARD.validateRange(
        data.amount,
        0,
        100_000_000
      )
