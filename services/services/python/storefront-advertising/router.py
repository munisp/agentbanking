...
from .service import (
    StorefrontAdvertisingService, AdCreate, AdResponse, PromoCampaignCreate,
    PromoValidationRequest, PromoValidationResponse, AdMetricsResponse, AdType
)
...
@router.post("/promos")
def create_promo_campaign(payload: PromoCampaignCreate, svc: StorefrontAdvertisingService = Depends(get_svc)):
...
def validate_promo(payload: PromoValidationRequest, svc: StorefrontAdvertisingService = Depends(get_svc)):
...
def get_active_campaigns(merchant_id: str, svc: StorefrontAdvertisingService = Depends(get_svc)):
    ...
    return [
        {
            ...
            "discount_percentage": str(c.discount_percentage) if c.discount_percentage else None,
            "discount_amount_ngn": str(c.discount_amount_ngn) if c.discount_amount_ngn else None,
...
def create_flash_sale(
    ...
    campaign = svc.generate_flash_sale(merchant_id, merchant_name, discount_pct, duration_hours, ids)
    return {
        "id": campaign.id,
        "promo_code": campaign.promo_code,
        "name": campaign.name,
