...
export const transactionReconciliationRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((reconciliationBatches as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[transactionReconciliation] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(reconciliationBatches)
        .where(eq((reconciliationBatches as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`transactionReconciliation record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionReconciliation] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(reconciliationBatches);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(reconciliationBatches)
        .where(gte((reconciliationBatches as any).createdAt, since))
        .orderBy(desc((reconciliationBatches as any).id))
        .limit(input.limit);
