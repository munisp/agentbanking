...
export const bulkDisbursementEngineRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[bulkDisbursementEngine] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`bulkDisbursementEngine record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[bulkDisbursementEngine] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
