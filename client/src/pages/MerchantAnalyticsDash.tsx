...
export default function MerchantAnalyticsDash() {
  ...
  const summary = trpc.merchantAnalyticsDash.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.merchantAnalyticsDash.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Merchant performance metrics, revenue analysis, and growth tracking</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Export Report initiated")}>Export Report</Button>
        <Button variant="outline" onClick={() => toast.success("Schedule Report initiated")}>Schedule Report</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalMerchants ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
        ...
        <Card key="3">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.topCategory ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">GMV</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Growth</th>
              ...
              <td className="p-3 font-mono">{formatCurrency(row.gmv)}</td>
              <td className="p-3">{Number(row.orders ?? 0).toLocaleString()}</td>
              <td className="p-3 font-mono">{formatCurrency(row.revenue)}</td>
              <td className="p-3">{(row.growth ?? 0) + '%'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
