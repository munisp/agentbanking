...
const createPolicy = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "dataRetentionPolicy",
      "mutation",
      "Executed dataRetentionPolicy mutation"
    );
...
function validateDataretentionpolicyInput(
...
const INTEGRITY_RULES_DATARETENTIONPOLICY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DATARETENTIONPOLICY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DATARETENTIONPOLICY.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
export const dataRetentionPolicyRouter = router({
  listPolicies,
  list: listPolicies, // alias of listPolicies (broken-call fix)
  getPolicy,
  getRetentionStats,
  getStats,
  createPolicy,
