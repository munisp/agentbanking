  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/bnpl.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/bnpl.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading BNPL Engine...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Buy Now Pay Later plans and installments</Text>
      </View>
