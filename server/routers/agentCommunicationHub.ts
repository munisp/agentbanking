...
export const agentCommunicationHubRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[agentCommunicationHub] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`agentCommunicationHub record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentCommunicationHub] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
