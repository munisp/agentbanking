...
function validateEsgcarbontrackerInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_ESGCARBONTRACKER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ESGCARBONTRACKER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ESGCARBONTRACKER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for esgCarbonTracker ───────────────────────────────────────
