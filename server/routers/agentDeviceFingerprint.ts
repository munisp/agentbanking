...
export const agentDeviceFingerprintRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(devices)
          .orderBy(desc((devices as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentDeviceFingerprint] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(devices)
        .where(eq((devices as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentDeviceFingerprint record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentDeviceFingerprint] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(devices);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(devices)
        .where(gte((devices as any).createdAt, since))
        .orderBy(desc((devices as any).id))
        .limit(input.limit);
