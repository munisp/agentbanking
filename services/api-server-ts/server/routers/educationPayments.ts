...
function validateEducationpaymentsInput(
...
const INTEGRITY_RULES_EDUCATIONPAYMENTS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_EDUCATIONPAYMENTS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_EDUCATIONPAYMENTS.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "educationPayments",
        "mutation",
        "Executed educationPayments mutation"
      );
