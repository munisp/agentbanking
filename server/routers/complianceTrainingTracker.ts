...
function validateCompliancetrainingtrackerInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "complianceTrainingTracker",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_COMPLIANCETRAININGTRACKER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_COMPLIANCETRAININGTRACKER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_COMPLIANCETRAININGTRACKER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _complianceTrainingTracker_db = {
...
// ── Transaction Handling for complianceTrainingTracker ───────────────────────────────────────
