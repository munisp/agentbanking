  ...
  try {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      await client.query(
        ...
        [txId, txRef, txType, `agent_${Math.ceil(Math.random()*20)}`, `POS${Math.ceil(Math.random()*100)}`, grossAmount, grossFee, agentCommission, switchFee, aggregatorFee, platformNetFee, model, clientRevenue, platformRevenue, revSharePct, region, carrier]
      );
      count++;
    }
