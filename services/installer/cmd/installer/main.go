...
Usage:
  sudo ./54agent-installer                                  — install / upgrade
  sudo ./54agent-installer --uninstall                      — stop, disable, and remove all services
  sudo ./54agent-installer --status                         — show service status without changing anything
  sudo ./54agent-installer --verify                         — verify binary integrity only (no install)
  sudo ./54agent-installer --enroll-token <TOKEN>           — complete device enrollment with a one-time token
  sudo ./54agent-installer --enroll-token <TOKEN> --agent-code <CODE> --serial <SERIAL>
...
Security:
  ...
  - The persistent device token returned by enrollment is stored in /opt/54agent/device.token
    and is passed on every transaction to the 54agent backend for device verification.
*/
...
const (
	installDir = "/opt/54agent/bin"
	systemdDir = "/etc/systemd/system"
	dataDir    = "/var/lib/54agent"
	logDir     = "/var/log/54agent"
	tokenFile  = "/opt/54agent/device.token"
	appUser    = "54agent"
	appGroup   = "54agent"

	// Default API base for enrollment (can be overridden via FIFTYFOURLINK_API_BASE env var)
	defaultAPIBase = "https://54agent.manus.space/api/trpc"
)
...
var services = []service{
	{
		name:        "54agent-resilience-agent",
		binary:      "resilience-agent",
		embeddedSrc: "embedded/resilience-agent",
		port:        8031,
		healthPath:  "/health",
		envVars:     []string{"RESILIENCE_AGENT_PORT=8031"},
		description: "54agent Resilience Agent — connection probe, carrier detection, retry engine",
	},
	{
		name:        "54agent-offline-queue",
		...
		description: "54agent Offline Queue — SQLite WAL transaction queue and USSD encoder",
	},
	{
		name:        "54agent-analytics-service",
		binary:      "analytics-service",
		embeddedSrc: "embedded/analytics-service",
		port:        8033,
		healthPath:  "/health",
		envVars:     []string{"ANALYTICS_PORT=8033"},
		description: "54agent Analytics Service — 7-day success rate and transaction statistics",
	},
}
...
func main() {
	...
	case len(args) > 0 && args[0] == "--enroll-token":
		if len(args) < 2 {
			fatalf("Usage: sudo ./54agent-installer --enroll-token <TOKEN> [--agent-code <CODE>] [--serial <SERIAL>]")
		}
		token := args[1]
...
func runInstall() {
	banner("54agent POS — Service Installer")
	requireRoot()
	...
	if allOK {
		fmt.Println("╔══════════════════════════════════════════════════════════╗")
		fmt.Println("║  ✅  54agent POS services installed and running           ║")
		fmt.Println("╚══════════════════════════════════════════════════════════╝")
		fmt.Printf("\nServices installed to : %s\n", installDir)
		fmt.Printf("Data directory        : %s\n", dataDir)
		fmt.Printf("Logs                  : journalctl -u 54agent-*\n\n")
		for _, svc := range services {
			fmt.Printf("  %-40s  http://localhost:%d\n", svc.name, svc.port)
		}
		fmt.Println()
		fmt.Println("Next step: enroll this device with the 54agent backend:")
		fmt.Println("  sudo ./54agent-installer --enroll-token <TOKEN> --agent-code <CODE> --serial <SERIAL>")
		fmt.Println()
	} else {
		fmt.Println("╔══════════════════════════════════════════════════════════╗")
		fmt.Println("║  ⚠️   Installation complete but some services failed     ║")
		fmt.Println("║      Run: journalctl -u 54agent-* -n 50 --no-pager       ║")
		fmt.Println("╚══════════════════════════════════════════════════════════╝")
		os.Exit(1)
	}
}
...
func runUninstall() {
	banner("54agent POS — Service Uninstaller")
	requireRoot()
...
func runStatus() {
	banner("54agent POS — Service Status")
	for _, svc := range services {
	...
	if _, err := os.Stat(tokenFile); err == nil {
		...
		fmt.Println("  ✗  Device NOT enrolled — run: sudo ./54agent-installer --enroll-token <TOKEN>")
	}
}
...
func runVerify() {
	banner("54agent POS — Binary Integrity Verification")
...
func runEnroll(token, agentCode, serial string) {
	banner("54agent POS — Device Enrollment")
	...
	// Restrict ownership to the 54agent service user
	...
	for embPath, expectedHash := range integrityManifest {
		...
		if actual != expectedHash {
			...
			if strict {
				...
				fmt.Println("║      Download a fresh copy from the 54agent admin portal.║")
				fmt.Println("╚══════════════════════════════════════════════════════════╝")
				fatalf(msg)
			}
...
func requireRoot() {
	if os.Getuid() != 0 {
		fatalf("This installer must be run as root: sudo ./54agent-installer")
	}
}
