...
function validateWearablepaymentsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_WEARABLEPAYMENTS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_WEARABLEPAYMENTS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_WEARABLEPAYMENTS.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "wearablePayments",
        "mutation",
        "Executed wearablePayments mutation"
      );
