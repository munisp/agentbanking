...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "realtime-fee-splitter".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("realtime-fee-splitter").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "realtime-fee-splitter").await {
            eprintln!("[realtime-fee-splitter] Loaded persisted state from PostgreSQL");
