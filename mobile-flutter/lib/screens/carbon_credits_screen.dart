...
class CarbonCreditsScreen extends ConsumerStatefulWidget {
  const CarbonCreditsScreen({super.key});

  @override
  ConsumerState<CarbonCreditsScreen> createState() => _CarbonCreditsScreenState();
}

class _CarbonCreditsScreenState extends ConsumerState<CarbonCreditsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/carbon_credits.getStats');
      final listResp = await api.get('/api/trpc/carbon_credits.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildCarbonType(Map<String, dynamic> item) {
    final type = '${item[\'projectType\'] ?? \'reforestation\'}';
    final ic = {'reforestation': Icons.park, 'solar': Icons.solar_power, 'wind': Icons.air, 'cookstove': Icons.local_fire_department, 'biogas': Icons.gas_meter, 'waste_mgmt': Icons.recycling};
    final cc = {'reforestation': Colors.green, 'solar': Colors.amber, 'wind': Colors.lightBlue, 'cookstove': Colors.orange, 'biogas': Colors.teal, 'waste_mgmt': Colors.brown};
    return Chip(avatar: Icon(ic[type] ?? Icons.eco, size: 14, color: Colors.white), label: Text(type.replaceAll('_', ' '), style: const TextStyle(fontSize: 10, color: Colors.white)), backgroundColor: cc[type] ?? Colors.grey, padding: EdgeInsets.zero, materialTapTargetSize: MaterialTapTargetSize.shrinkWrap);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.eco, size: 24), const SizedBox(width: 8), const Text('Carbon Credits')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Carbon Credits', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('Carbon credit marketplace — projects, trading & retirement', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Projects', '${_stats?['totalProjects'] ?? '\u2014'}', Icons.forest, Colors.green),
                              _buildStatCard('Credits Issued', '${_stats?['creditsIssued'] ?? '\u2014'}', Icons.receipt_long, Colors.blue),
                              _buildStatCard('Credits Retired', '${_stats?['creditsRetired'] ?? '\u2014'}', Icons.check_circle, Colors.orange),
                              _buildStatCard('Market Volume', '₦${_stats?['marketVolume'] ?? '\u2014'}', Icons.attach_money, Colors.purple),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['projectName'] ?? item['projectType'] ?? item['creditsRequested'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildCarbonType(item),
                    ]),
