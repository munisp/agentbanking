...
export const regulatorySandboxTesterRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((complianceChecks as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[regulatorySandboxTester] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(complianceChecks)
        .where(eq((complianceChecks as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`regulatorySandboxTester record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[regulatorySandboxTester] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(complianceChecks);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(complianceChecks)
        .where(gte((complianceChecks as any).createdAt, since))
        .orderBy(desc((complianceChecks as any).id))
        .limit(input.limit);
