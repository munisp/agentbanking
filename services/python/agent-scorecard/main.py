import httpx
"""
Agent Scorecard Service — FastAPI Application Entry Point
"""
import os
import logging
...
async def lifespan(app: FastAPI):
    logger.info("Agent Scorecard Service starting — initialising database tables...")
    init_db()
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "agent-scorecard"),
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/agent_scorecard")
...
async def health():
    ...
    description=(
        ...
        "Transaction Performance, Customer Experience, Compliance & Risk, "
        "Training & Certification, and Network Growth."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
async def startup_event():
    ...
    await publish_kafka("agent.scorecard.started", {
