...
function validateDeepfaceInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_DEEPFACE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DEEPFACE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (!INTEGRITY_RULES_DEEPFACE.validateRange(data.amount, 0, 100_000_000))
