...
export default function MultiChannelPaymentOrch() {
  ...
  const summary = trpc.multiChannelPaymentOrch.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.multiChannelPaymentOrch.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Route payments across channels (USSD, mobile, web, POS) with
              failover
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Configure Routes initiated")}>
              Configure Routes
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Test Failover initiated")}
            >
              Test Failover
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card key="1">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.channelSplit ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.failoverEvents ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
