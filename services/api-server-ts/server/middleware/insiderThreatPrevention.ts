...
// NOTE: the "insider.*" topics are not yet members of the KafkaTopic union in
// ../kafkaClient (that file is owned by a concurrent audit wave). Cast to the
// declared topic type so this port stays decoupled from that file; publishEvent
// is fail-open at runtime.
type KafkaTopicName = Parameters<typeof publishEvent>[0];
...
export async function createApprovalRequest(params: {
  ...
  publishEvent("insider.approval.requested" as KafkaTopicName, request.id, {
...
export async function processApproval(params: {
  ...
  publishEvent("insider.approval.actioned" as KafkaTopicName, request.id, {
...
export async function checkStaffVelocity(
  ...
  if (reversalCount > MAX_REVERSALS_PER_HOUR) {
    publishEvent("insider.threat.velocity" as KafkaTopicName, `VEL-${agentId}-${Date.now()}`, {
  ...
  if (hvCount > MAX_HIGH_VALUE_PER_HOUR) {
    publishEvent("insider.threat.velocity" as KafkaTopicName, `VEL-HV-${agentId}-${Date.now()}`, {
