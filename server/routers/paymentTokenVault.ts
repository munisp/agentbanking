...
function validatePaymenttokenvaultInput(
...
const INTEGRITY_RULES_PAYMENTTOKENVAULT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PAYMENTTOKENVAULT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PAYMENTTOKENVAULT.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for paymentTokenVault ───────────────────────────────────────
