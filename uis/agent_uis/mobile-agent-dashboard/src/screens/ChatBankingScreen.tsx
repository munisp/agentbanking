...
const ChannelTag = ({ item }: { item: RecordItem }) => {
    const ch = item.channel || 'webchat';
    const colors: Record<string, string> = { whatsapp: '#25D366', telegram: '#0088cc', ussd: '#f59e0b', webchat: '#8b5cf6', sms: '#14b8a6' };
    const icons: Record<string, string> = { whatsapp: '💬', telegram: '✈️', ussd: '📱', webchat: '🌐', sms: '📩' };
    return (<View style={{ backgroundColor: colors[ch] || '#6b7280', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 12 }}>
      <Text style={{ fontSize: 10, color: '#fff', fontWeight: '600' }}>{icons[ch] || '💬'} {ch.toUpperCase()}</Text></View>);
  };

export default function ChatBankingScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/chat_banking.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/chat_banking.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Conversational Banking...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Conversational Banking</Text>
            <Text style={styles.subtitle}>WhatsApp, USSD & chat banking</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{stats?.messagesToday ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>⌨️</Text>
            <Text style={styles.statLabel}>Commands</Text>
            <Text style={styles.statValue}>{stats?.commandsExecuted ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>👍</Text>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.channel || `Record #${item.id}`}</Text>
