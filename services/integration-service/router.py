"""
Router for integration-service service
...
The real implementation lives in the standalone service (services/integration-service/).
...
router = APIRouter(prefix="/integration-service", tags=["integration-service"])

_UNAVAILABLE = (
    "integration-service endpoints are not served by this gateway router. "
    "Use the standalone integration-service service."
)
...
async def health_check():
    return JSONResponse(
        status_code=503,
        content={"status": "unavailable", "service": "integration-service", "detail": _UNAVAILABLE},
    )
