...
export const connectionPoolMonitorRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[connectionPoolMonitor] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`connectionPoolMonitor record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[connectionPoolMonitor] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
