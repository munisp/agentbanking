...
export const platformMaturityScorecardRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((platform_health_checks as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[platformMaturityScorecard] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(platform_health_checks)
        .where(eq((platform_health_checks as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`platformMaturityScorecard record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[platformMaturityScorecard] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(platform_health_checks)
        .where(gte((platform_health_checks as any).createdAt, since))
        .orderBy(desc((platform_health_checks as any).id))
        .limit(input.limit);
