...
function validateTransactionmonitoringInput(
...
const INTEGRITY_RULES_TRANSACTIONMONITORING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_TRANSACTIONMONITORING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_TRANSACTIONMONITORING.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _transactionMonitoring_db = {
...
// ── Transaction Handling for transactionMonitoring ───────────────────────────────────────
