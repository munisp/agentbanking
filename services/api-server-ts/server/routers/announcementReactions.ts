...
function validateAnnouncementreactionsInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "announcementReactions",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_ANNOUNCEMENTREACTIONS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ANNOUNCEMENTREACTIONS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ANNOUNCEMENTREACTIONS.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _announcementReactions_db = {
