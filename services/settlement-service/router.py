...
_DELETABLE_STATUSES = {SettlementStatus.PENDING.value, SettlementStatus.FAILED.value, SettlementStatus.CANCELLED.value}
...
def delete_settlement(settlement_id: int, db: Session = Depends(get_db)):
    """
    Deletes a settlement record. Only PENDING, REJECTED, or CANCELLED settlements may be
    deleted — COMPLETED and PROCESSING records are financial evidence and must not be removed.
    """
    db_settlement = get_settlement_by_id(db, settlement_id)

    if db_settlement.status not in _DELETABLE_STATUSES:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=(
                f"Cannot delete settlement in status '{db_settlement.status}'. "
                "Only PENDING, REJECTED, or CANCELLED settlements may be deleted."
            )
        )
...
def process_settlement(settlement_id: int, db: Session = Depends(get_db)):
    """
    Processes a PENDING settlement by executing the payout via payment-processing-service,
    then marks it COMPLETED or FAILED based on the result.

    The PENDING→PROCESSING transition is done with an atomic UPDATE…WHERE status='pending'
    so that two concurrent calls cannot both claim the same settlement.
    """
    cfg = get_settings()

    # Atomically claim the settlement — only one concurrent caller wins
    result = db.execute(
        text(
            "UPDATE settlements SET status = :proc, updated_at = :now "
            "WHERE id = :id AND status = :pend RETURNING id"
        ),
        {"proc": SettlementStatus.PROCESSING.value, "pend": SettlementStatus.PENDING.value,
         "now": datetime.utcnow(), "id": settlement_id},
    )
    if result.fetchone() is None:
        db_settlement = get_settlement_by_id(db, settlement_id)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Settlement is not PENDING. Current status: {db_settlement.status}."
        )
    db.commit()

    db_settlement = get_settlement_by_id(db, settlement_id)
    create_log_entry_internal(db, settlement_id, SettlementLogCreate(
        level=LogLevel.INFO,
        message="Settlement processing initiated.",
        details=f"Calling payment-processing-service for amount {db_settlement.amount} NGN",
    ))
    db.commit()

    try:
        payout_payload = {
            "agent_id": str(db_settlement.agent_id) if hasattr(db_settlement, "agent_id") and db_settlement.agent_id else "",
            "amount": float(db_settlement.amount),
            "currency": db_settlement.currency or "NGN",
            "settlement_ref": db_settlement.external_reference_id or str(settlement_id),
            "note": f"Settlement payout for settlement #{settlement_id}",
        }
        resp = httpx.post(
            f"{cfg.PAYMENT_PROCESSING_SVC_URL}/payment/settlement-payout",
            json=payout_payload,
            headers={"x-service-auth": cfg.SERVICE_AUTH_TOKEN, "x-tenant-id": "default"},
            timeout=30.0,
        )
        resp.raise_for_status()
        payout_ref = resp.json().get("reference", "")
        db_settlement.status = SettlementStatus.COMPLETED.value
        create_log_entry_internal(db, settlement_id, SettlementLogCreate(
            level=LogLevel.INFO,
            message="Payout executed successfully.",
            details=f"Payment reference: {payout_ref}",
        ))
        ...
        logger.error(f"Settlement {settlement_id} payout failed: {exc}")

    db.commit()
    db.refresh(db_settlement)
    logger.info(f"Settlement ID {settlement_id} final status: {db_settlement.status}")
    return db_settlement
...
NETTING_REFERENCE_PREFIX = "NETTING-"

@router.get(
    "/api/v1/netting/runs",
    status_code=status.HTTP_200_OK,
    summary="Get netting runs"
)
def get_netting_runs(
    limit: int = Query(50, ge=1, le=200),
    db: Session = Depends(get_db),
):
    """Retrieves multilateral netting settlement runs (settlements with NETTING- reference prefix)."""
    runs = (
        db.query(Settlement)
        .filter(Settlement.external_reference_id.like(f"{NETTING_REFERENCE_PREFIX}%"))
    ...
    return {
        ...
        "data": [
            {
                "id": r.id,
                "reference_id": r.external_reference_id,
                "status": r.status,
                "amount": r.amount,
                "currency": r.currency,
...
@router.post(
    "/api/v1/netting/trigger",
    status_code=status.HTTP_202_ACCEPTED,
    summary="Trigger an off-cycle netting run"
)
def trigger_netting_run(db: Session = Depends(get_db)):
    """Creates and immediately queues a multilateral netting settlement run."""
    import uuid
    ref_id = f"{NETTING_REFERENCE_PREFIX}{uuid.uuid4().hex[:12].upper()}"

    # Aggregate pending settlements to net
    from sqlalchemy import func as sqlfunc
    agg = db.query(
        sqlfunc.coalesce(sqlfunc.sum(Settlement.amount), 0.0).label("total"),
        sqlfunc.count(Settlement.id).label("count"),
    ).filter(Settlement.status == SettlementStatus.PENDING.value).one()

    netting_record = Settlement(
        settlement_date=datetime.utcnow(),
        amount=float(agg.total),
        currency="NGN",
        transaction_count=int(agg.count),
        external_reference_id=ref_id,
        status=SettlementStatus.PROCESSING.value,
    )
    db.add(netting_record)
    db.commit()
    db.refresh(netting_record)

    log_data = SettlementLogCreate(
        level=LogLevel.INFO,
        message="Off-cycle netting run triggered via API",
        details=f"Netting {agg.count} pending settlements totalling {agg.total} NGN",
    )
    create_log_entry_internal(db, netting_record.id, log_data)

    logger.info(f"Netting run triggered: {ref_id}")
    return {
        "status": "accepted",
        "reference_id": ref_id,
        "settlement_id": netting_record.id,
        "pending_settlements_included": int(agg.count),
...
@router.post(
    "/initialize-db",
    status_code=status.HTTP_200_OK,
    summary="Initialize database tables (internal use only)"
)
def initialize_database(
    x_service_auth: str = Header("", alias="x-service-auth"),
    db: Session = Depends(get_db),
):
    if x_service_auth != "settlement-service-internal":
