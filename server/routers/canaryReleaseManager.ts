...
function validateCanaryreleasemanagerInput(
...
const INTEGRITY_RULES_CANARYRELEASEMANAGER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CANARYRELEASEMANAGER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CANARYRELEASEMANAGER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _canaryReleaseManager_db = {
...
// ── Transaction Handling for canaryReleaseManager ───────────────────────────────────────
