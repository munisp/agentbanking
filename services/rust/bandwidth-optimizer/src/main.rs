...
// Persistence: audit log + state store for bandwidth-optimizer
...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "bandwidth-optimizer".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("bandwidth-optimizer").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "bandwidth-optimizer").await {
            eprintln!("[bandwidth-optimizer] Loaded persisted state from PostgreSQL");
