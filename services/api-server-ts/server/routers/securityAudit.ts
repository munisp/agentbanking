...
const runSecurityScan = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    throw new TRPCError({
      ...
      message: "securityAudit.runSecurityScan is not available in this deployment",
    });
  });
...
function validateSecurityauditInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_SECURITYAUDIT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SECURITYAUDIT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SECURITYAUDIT.validateRange(data.amount, 0, 100_000_000)
    )
