...
export const graphqlSubscriptionGatewayRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[graphqlSubscriptionGateway] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`graphqlSubscriptionGateway record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[graphqlSubscriptionGateway] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
