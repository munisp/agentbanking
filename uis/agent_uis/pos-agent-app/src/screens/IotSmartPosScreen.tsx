...
export default function IotSmartPosScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/iot_pos.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/iot_pos.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading IoT Smart POS...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>IoT device telemetry and predictive maintenance</Text>
      </View>
