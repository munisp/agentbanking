...
function validateDocumentmanagementInput(
...
const INTEGRITY_RULES_DOCUMENTMANAGEMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DOCUMENTMANAGEMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DOCUMENTMANAGEMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  uploadDocument: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "documentManagement",
        "mutation",
        "Executed documentManagement mutation"
      );
