...
class PaymentDisputeArbitrationScreen extends StatefulWidget {
  const PaymentDisputeArbitrationScreen({super.key});
  @override
  State<PaymentDisputeArbitrationScreen> createState() => _PaymentDisputeArbitrationScreenState();
}

class _PaymentDisputeArbitrationScreenState extends State<PaymentDisputeArbitrationScreen> {
  ...
  Future<void> _load() async {
    try {
      final result = await ApiService.instance.get('/disputes/list', queryParams: {'page': '1', 'limit': '50'});
      setState(() {
        _data = result ?? {};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Payment Dispute Arbitration'),
        actions: [
