...
function validateAlertnotificationsInput(
...
const INTEGRITY_RULES_ALERTNOTIFICATIONS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ALERTNOTIFICATIONS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ALERTNOTIFICATIONS.validateRange(
        data.amount,
        0,
        100_000_000
      )
