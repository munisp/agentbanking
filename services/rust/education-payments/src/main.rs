//! 54Link Education Payments — Rust Microservice
//!
//! Fee calculation, discount engine, installment planning, reconciliation
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/edu/fees/calculate — Calculate fees with discounts/scholarships
//!   POST /api/v1/edu/fees/installment-plan — Generate installment plan
//!   POST /api/v1/edu/reconcile — Reconcile school payments
//!   GET  /api/v1/edu/fees/schedule/{schoolId} — Fee schedule for school
//!
//! Port: 8258
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8258),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "education-payments"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "education-payments".into(),
...
async fn create_record(
    ...
    dapr.publish("edu.fee.paid", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("education-payments-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("edu_schools", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("education-payments:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("education_payments", &payload).await;
        // Persist to PostgreSQL
        match state.postgres.insert("education_payments", &payload).await {
            Ok(row) => info!("[Postgres] Inserted into education_payments: {:?}", row.get("id")),
            Err(e) => warn!("[Postgres] Insert into education_payments failed: {}", e),
        }

    // Persist to PostgreSQL
    let _ = state.postgres.persist("education_payments", &serde_json::json!({
        "service": "education-payments",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("education-payments:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("edu_schools", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Education Payments (Rust) starting on port {}", port);
