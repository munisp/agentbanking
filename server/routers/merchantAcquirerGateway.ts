...
export const merchantAcquirerGatewayRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[merchantAcquirerGateway] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`merchantAcquirerGateway record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[merchantAcquirerGateway] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
