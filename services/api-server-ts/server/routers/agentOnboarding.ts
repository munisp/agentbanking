  ...
  completeProfile: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "agentOnboarding",
        "mutation",
        "Executed agentOnboarding mutation"
      );
      ...
      try {
        ...
        if (agent.email) {
          const { subject, html, text } = buildAlertEmail({
            title: "Welcome to 54agent POS — Your Account is Active!",
            message: `Congratulations ${agent.name}! Your 54agent POS agent account (${input.agentCode}) has been fully activated. You can now process transactions on your terminal.`,
            severity: "low",
          });
