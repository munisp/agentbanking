...
const InstallmentBar = ({ item }: { item: RecordItem }) => {
    const paid = Number(item.paidInstallments || 0);
    const total = Number(item.installments || 6);
    const pct = total > 0 ? (paid / total) * 100 : 0;
    return (<View><Text style={{ fontSize: 10, color: '#9ca3af' }}>{paid}/{total} installments</Text>
      <View style={{ height: 4, backgroundColor: '#e5e7eb', borderRadius: 2, marginTop: 2 }}>
        <View style={{ height: 4, width: pct + '%', backgroundColor: pct >= 100 ? '#22c55e' : '#3b82f6', borderRadius: 2 }} />
      </View></View>);
  };
...
export default function BnplScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/bnpl_engine.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/bnpl_engine.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading BNPL Engine...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>BNPL Engine</Text>
            <Text style={styles.subtitle}>Buy Now, Pay Later — loans & installments</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>₦{stats?.totalDisbursed ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📈</Text>
            <Text style={styles.statLabel}>Repayment Rate</Text>
            <Text style={styles.statValue}>{stats?.repaymentRate ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.customerId || `Record #${item.id}`}</Text>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(item.status) + '20' }]}>
              <Text style={[styles.statusText, { color: getStatusColor(item.status) }]}>{(item.status || '—').toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.cardBody}>
            <InstallmentBar item={item} />
          </View>
        </View>
      )}
