...
function validatePaymentgatewayrouterInput(
...
const INTEGRITY_RULES_PAYMENTGATEWAYROUTER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PAYMENTGATEWAYROUTER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PAYMENTGATEWAYROUTER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _paymentGatewayRouter_db = {
