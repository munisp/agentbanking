...
function validatePensioncollectionInput(
...
const INTEGRITY_RULES_PENSIONCOLLECTION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PENSIONCOLLECTION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PENSIONCOLLECTION.validateRange(
        data.amount,
        0,
        100_000_000
      )
