...
export default function AgentPerformanceScorecardPage() {
  ...
  const agents = (data?.agents || []).filter((a: any) => !search || a.name?.toLowerCase().includes(search.toLowerCase()));
          ...
          {agents.map((agent: any, i: number) => (
            <Card key={i}><CardContent className="flex items-center justify-between py-4">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${getScoreBg(agent.score)}`}>
                  <span className={`text-lg font-bold ${getScoreColor(agent.score)}`}>{agent.score}</span>
                </div>
                <div><p className="font-medium">{agent.name}</p><p className="text-sm text-muted-foreground">{agent.territory} • {agent.role}</p>
                  <div className="flex gap-4 mt-1 text-xs text-muted-foreground">
                    <span><TrendingUp className="w-3 h-3 inline mr-1" />{agent.txnCount} txns</span>
                    <span><Star className="w-3 h-3 inline mr-1" />${agent.commission?.toLocaleString()} earned</span>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <div className="w-32 bg-gray-200 rounded-full h-2"><div className={`h-2 rounded-full ${agent.score >= 80 ? 'bg-green-500' : agent.score >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{width: `${agent.score}%`}} /></div>
                <p className="text-xs text-muted-foreground mt-1">{agent.score >= 80 ? 'Excellent' : agent.score >= 60 ? 'Good' : 'Needs Improvement'}</p>
              </div>
            </CardContent></Card>
          ))}
        </div>
