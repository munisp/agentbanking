...
export default function PaymentLinkGenerator() {
  ...
  const summary = trpc.paymentLinkGenerator.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.paymentLinkGenerator.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Generate and manage shareable payment links for agents and merchants</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Create Link initiated")}>Create Link</Button>
        <Button variant="outline" onClick={() => toast.success("Bulk Create initiated")}>Bulk Create</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalLinks ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.activeLinks ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Link</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Clicks</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.url ?? '—')}</td>
