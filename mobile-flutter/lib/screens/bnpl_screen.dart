...
class _BnplScreenState extends ConsumerState<BnplScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/bnpl.getStats');
      final listResp = await api.get('/api/trpc/bnpl.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildInstallmentProgress(Map<String, dynamic> item) {
    final paid = int.tryParse('${item[\'paidInstallments\'] ?? 0}') ?? 0;
    final total = int.tryParse('${item[\'installments\'] ?? 6}') ?? 6;
    final progress = total > 0 ? paid / total : 0.0;
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('$paid/$total installments', style: const TextStyle(fontSize: 12, color: Colors.grey)), const SizedBox(height: 4), LinearProgressIndicator(value: progress, backgroundColor: Colors.grey[300], color: progress >= 1.0 ? Colors.green : Colors.blue)]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.credit_card, size: 24), const SizedBox(width: 8), const Text('BNPL Engine')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('BNPL Engine', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('Buy Now, Pay Later — loans, installments & collections', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Active Loans', '${_stats?['activeLoans'] ?? '\u2014'}', Icons.account_balance_wallet, Colors.blue),
                              _buildStatCard('Total Disbursed', '₦${_stats?['totalDisbursed'] ?? '\u2014'}', Icons.payments, Colors.green),
                              _buildStatCard('Repayment Rate', '${_stats?['repaymentRate'] ?? '\u2014'}', Icons.trending_up, Colors.orange),
                              _buildStatCard('Overdue', '${_stats?['overdueCount'] ?? '\u2014'}', Icons.warning_amber, Colors.red),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['customerId'] ?? item['amount'] ?? item['installments'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildInstallmentProgress(item),
                    ]),
