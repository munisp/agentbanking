"""
Router for konga-service service
...
The real implementation lives in the standalone service (services/konga-service/).
...
router = APIRouter(prefix="/konga-service", tags=["konga-service"])

_UNAVAILABLE = (
    "konga-service endpoints are not served by this gateway router. "
    "Use the standalone konga-service service."
)
...
async def health_check():
    return JSONResponse(
        status_code=503,
        content={"status": "unavailable", "service": "konga-service", "detail": _UNAVAILABLE},
    )
