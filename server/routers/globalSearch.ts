  ...
  typeahead: protectedProcedure
    .input(
      z.object({
        ...
        entityType: z
          .enum(["agents", "transactions", "customers", "disputes"])
          .default("agents"),
    ...
    .query(async ({ input }) => {
      ...
      return opensearch.searchAsYouType(
        input.entityType,
        field,
        input.query,
        input.limit
  ...
  multiEntitySearch: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const queries = [
        {
          index: "agents",
          query: { match_phrase_prefix: { name: { query: input.query } } },
        },
        {
          index: "transactions",
          query: { match_phrase_prefix: { txRef: { query: input.query } } },
        },
        {
          index: "customers",
          query: { match_phrase_prefix: { name: { query: input.query } } },
        },
        {
          index: "disputes",
          query: {
            match_phrase_prefix: { description: { query: input.query } },
          },
        },
      ];
      const [agentResults, txResults, customerResults, disputeResults] =
        await opensearch.multiSearch(queries);
      return {
        agents: agentResults,
