...
export const customerSegmentationEngineRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(customers)
          .orderBy(desc((customers as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[customerSegmentationEngine] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(customers)
        .where(eq((customers as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`customerSegmentationEngine record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[customerSegmentationEngine] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(customers);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(customers)
        .where(gte((customers as any).createdAt, since))
        .orderBy(desc((customers as any).id))
        .limit(input.limit);
