...
export default function DigitalTwinSimulator() {
  ...
  const summary = trpc.digitalTwinSimulator.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.digitalTwinSimulator.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Digital Twin Simulator</h1>
            <p className="text-muted-foreground">System simulation, capacity modeling, and what-if scenario analysis</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("New Simulation initiated")}>New Simulation</Button>
        <Button variant="outline" onClick={() => toast.success("Clone Scenario initiated")}>Clone Scenario</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.activeSimulations ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalScenarios ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.avgAccuracy ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Compute Time</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.computeTime ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Sim ID</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{(row.accuracy ?? 0) + '%'}</td>
