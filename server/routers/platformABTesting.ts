...
function validatePlatformabtestingInput(
...
const INTEGRITY_RULES_PLATFORMABTESTING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PLATFORMABTESTING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PLATFORMABTESTING.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for platformABTesting ───────────────────────────────────────
