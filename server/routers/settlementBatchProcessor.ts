...
import { settlementReconciliation } from "../../drizzle/schema";
...
export const settlementBatchProcessorRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((settlementReconciliation as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[settlementBatchProcessor] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(settlementReconciliation)
        .where(eq((settlementReconciliation as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`settlementBatchProcessor record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[settlementBatchProcessor] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(settlementReconciliation);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(settlementReconciliation)
        .where(gte((settlementReconciliation as any).createdAt, since))
        .orderBy(desc((settlementReconciliation as any).id))
        .limit(input.limit);
