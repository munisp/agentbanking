...
export default function AutoComplianceWorkflow() {
  ...
  const summary = trpc.autoComplianceWorkflow.getStats.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.autoComplianceWorkflow.listWorkflows.useQuery({
    limit: 20,
  });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Automated Compliance</h1>
            <p className="text-muted-foreground">
              Regulatory compliance checks, automated workflows, and audit
              trails
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Run Check initiated")}>
              Run Check
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Schedule Audit initiated")}
            >
              Schedule Audit
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.violations ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">{String(row.reviewer ?? "—")}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
