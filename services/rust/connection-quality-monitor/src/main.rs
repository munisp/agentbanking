...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "connection-quality-monitor".to_string();
...
.bind(format!("audit_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()))
                .bind(&val)
                .bind("connection-quality-monitor")
                .execute(pool).await;
        }
        log.push(AuditEntry {
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("connection-quality-monitor").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "connection-quality-monitor").await {
            eprintln!("[connection-quality-monitor] Loaded persisted state from PostgreSQL");
