...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "carbon-credit-marketplace"),
...
async def health_check():
    return {
        "status": "healthy",
        "service": "carbon-credit-marketplace-analytics",
        "port": PORT,
...
async def analytics_summary():
    ...
    # Cache and publish
    cache.set("carbon-credit-marketplace:summary", summary)
    await dapr.publish("carbon-credit-marketplace.analytics.updated", summary)
    await fluvio.produce("carbon-credit-marketplace-analytics", summary)
    await lakehouse.ingest("carbon-credit-marketplace_analytics", summary)
...
async def forecast(request: ForecastRequest):
    ...
    await dapr.publish("carbon-credit-marketplace.forecast.generated", result)
    return result
...
async def register_apisix():
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            route = {
                "uri": "/api/v1/carbon/credit/marketplace/analytics/*",
                "upstream": {
                    "type": "roundrobin",
            ...
            await client.put(
                f"{APISIX_ADMIN_URL}/apisix/admin/routes/carbon-credit-marketplace-analytics",
                json=route,
                headers={"X-API-KEY": "edd1c9f034335f136f87ad84b625c8f1"},
            )
            logger.info(f"[APISIX] Registered carbon-credit-marketplace-analytics")
    except Exception as e:
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"54Link Carbon Credit Marketplace Analytics starting on port {PORT}")
