...
function validateFinancialnlengineInput(
...
const INTEGRITY_RULES_FINANCIALNLENGINE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_FINANCIALNLENGINE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_FINANCIALNLENGINE.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for financialNlEngine ───────────────────────────────────────
