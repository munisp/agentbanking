...
export const regulatoryReportGeneratorRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((complianceReports as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[regulatoryReportGenerator] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(complianceReports)
        .where(eq((complianceReports as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`regulatoryReportGenerator record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[regulatoryReportGenerator] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(complianceReports);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(complianceReports)
        .where(gte((complianceReports as any).createdAt, since))
        .orderBy(desc((complianceReports as any).id))
        .limit(input.limit);
