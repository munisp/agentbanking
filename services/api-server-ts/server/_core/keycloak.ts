/**
 * keycloak.ts — Keycloak OIDC integration for 54agent POS Shell
 *
 * Responsibilities:
 ...
 *  - KEYCLOAK_URL          e.g. https://auth.54agent.io
 *  - KEYCLOAK_REALM        e.g. 54agent
...
function getConfig(): KeycloakConfig {
  ...
  const realm = process.env.KEYCLOAK_REALM ?? "54agent";
...
export function mapKeycloakRoleToPlatformRole(
  ...
  if (roles.includes("54agent-admin") || roles.includes("admin")) return "admin";
  if (roles.includes("54agent-supervisor") || roles.includes("supervisor"))
    return "supervisor";
  return "user";
}
