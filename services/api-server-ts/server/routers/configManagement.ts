...
const updateConfig = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    throw new TRPCError({
      ...
      message: "configManagement.updateConfig is not available in this deployment",
    });
  });
...
function validateConfigmanagementInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_CONFIGMANAGEMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CONFIGMANAGEMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CONFIGMANAGEMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
