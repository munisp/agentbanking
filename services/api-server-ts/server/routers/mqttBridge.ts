/**
 * MQTT Bridge Router — 54agent Fluvio MQTT Source Connector management
...
const INTEGRITY_RULES_MQTTBRIDGE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_MQTTBRIDGE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (!INTEGRITY_RULES_MQTTBRIDGE.validateRange(data.amount, 0, 100_000_000))
  ...
  generateConnectorSpec: protectedProcedure
    ...
    .query(({ input }) => {
      ...
      const connectors = topicMappings.map((m, i) => {
        const connectorName = `54agent-mqtt-${m.fluvioTopic.replace(/\./g, "-")}-${i}`;
