"""
WesternUnion Payment Gateway Integration
...
class WesternUnionGateway:
    ...
    def __init__(self, api_key: str, api_secret: str, environment: str = "sandbox"):
        ...
        self.supported_currencies = ['USD', 'EUR', 'GBP', 'MXN', 'INR']
        self.supported_corridors = self._init_corridors()
    
    def _get_base_url(self) -> str:
        """Get API base URL based on environment."""
        if self.environment == "production":
            return "https://api.western_union.com/v1"
        return "https://sandbox-api.western_union.com/v1"
    ...
    async def get_quote(self, amount: Decimal, from_currency: str, 
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"WesternUnion API error: {response.status_code}")
    ...
    async def create_transfer(self, quote_id: str, sender: Dict, 
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code in [200, 201]:
                ...
                raise Exception(f"WesternUnion transfer failed: {response.status_code}")
