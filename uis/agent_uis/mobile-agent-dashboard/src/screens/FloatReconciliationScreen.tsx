...
const FloatReconciliationScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const { data } = await apiClient.get('/float/list?page=1&limit=50');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Float Reconciliation</Text>
...
export default FloatReconciliationScreen;
