 ...
 * - HMAC-SHA256 signature (X-54agent-Signature header)
  ...
  try {
    ...
    const response = await fetch(endpoint.url, {
      ...
      headers: {
        ...
        "X-54agent-Signature": signature,
        "X-54agent-Event": body ? JSON.parse(body).event : "",
        "X-54agent-Delivery": String(deliveryId),
