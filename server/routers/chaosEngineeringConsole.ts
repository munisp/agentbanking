...
export const chaosEngineeringConsoleRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[chaosEngineeringConsole] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`chaosEngineeringConsole record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[chaosEngineeringConsole] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
