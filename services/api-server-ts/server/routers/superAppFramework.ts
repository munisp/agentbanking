...
function validateSuperappframeworkInput(
...
const INTEGRITY_RULES_SUPERAPPFRAMEWORK = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SUPERAPPFRAMEWORK.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SUPERAPPFRAMEWORK.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "superAppFramework",
        "mutation",
        "Executed superAppFramework mutation"
      );
