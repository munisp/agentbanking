...
function validateSystemhealthmonitorInput(
...
const INTEGRITY_RULES_SYSTEMHEALTHMONITOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SYSTEMHEALTHMONITOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SYSTEMHEALTHMONITOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
