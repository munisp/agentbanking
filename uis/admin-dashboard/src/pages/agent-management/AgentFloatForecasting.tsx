...
// The forecasting backend does not currently expose a replenishment
// endpoint, so replenish actions are disabled rather than submitting
// fabricated amounts.
const REPLENISH_UNAVAILABLE_REASON =
  "Replenishment is unavailable: the float forecasting service is not connected.";
...
export default function AgentFloatForecasting() {
  ...
  const forecast = {data: null as any, isLoading: false, refetch: () => {}} as { data: any; isLoading: boolean; refetch: () => void };
  ...
  const agents = ((forecast.data as any)?.forecasts ?? []) as AgentForecast[];
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(stats.data as any)?.totalFloat != null
                  ? `₦${(stats.data as any).totalFloat.toLocaleString()}`
                  : "—"}
              </div>
              <p className="text-xs text-muted-foreground">
                {stats.data ? "Current reported pool" : "Unavailable"}
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-500">
                {(stats.data as any)?.stockoutRisk ?? "—"}
              </div>
              <p className="text-xs text-muted-foreground">
                {(stats.data as any)?.agentsMonitored != null
                  ? `of ${(stats.data as any).agentsMonitored} active agents`
                  : "Unavailable"}
              </p>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button disabled title={REPLENISH_UNAVAILABLE_REASON}>
                Auto-Replenish All
              </Button>
            </div>
          </CardHeader>
                        ...
                        <td className="py-3 px-2">
                          {agent.shortfall > 0 && (
                            <Button
                              ...
                              variant="outline"
                              disabled
                              title={REPLENISH_UNAVAILABLE_REASON}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-muted-foreground py-4 text-center">
                Model insights are unavailable — the forecasting service is not
                connected.
              </p>
            </CardContent>
          </Card>
