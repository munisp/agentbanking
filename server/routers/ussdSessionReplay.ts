...
export const ussdSessionReplayRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[ussdSessionReplay] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`ussdSessionReplay record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[ussdSessionReplay] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
