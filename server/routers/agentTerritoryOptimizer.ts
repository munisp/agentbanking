...
export const agentTerritoryOptimizerRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((agentGeofenceZones as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentTerritoryOptimizer] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(agentGeofenceZones)
        .where(eq((agentGeofenceZones as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentTerritoryOptimizer record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentTerritoryOptimizer] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(agentGeofenceZones);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(agentGeofenceZones)
        .where(gte((agentGeofenceZones as any).createdAt, since))
        .orderBy(desc((agentGeofenceZones as any).id))
        .limit(input.limit);
