import httpx
"""
Storefront Advertising - FastAPI microservice
Digital advertising platform for agent storefronts with campaign management and analytics
"""
import os
import logging
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "storefront-advertising"),
...
app = FastAPI(title="Storefront Advertising", description="Digital advertising platform for agent storefronts with campaign management and analytics", version="1.0.0")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/storefront_advertising")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "storefront-advertising", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
...
async def startup_event():
    ...
    await publish_kafka("storefront.advertising.started", {
        "service": "storefront-advertising",
