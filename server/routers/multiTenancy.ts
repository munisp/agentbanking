...
export const multiTenancyRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(tenants)
          .orderBy(desc((tenants as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[multiTenancy] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(tenants)
        .where(eq((tenants as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`multiTenancy record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[multiTenancy] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(tenants);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(tenants)
        .where(gte((tenants as any).createdAt, since))
        .orderBy(desc((tenants as any).id))
        .limit(input.limit);
