...
function validateCoalitionloyaltyInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_COALITIONLOYALTY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_COALITIONLOYALTY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_COALITIONLOYALTY.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
export const coalitionLoyaltyRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        const result = await db.execute(
          sql`SELECT id, data, status, created_at, agent_id FROM "loyalty_members" ORDER BY created_at DESC LIMIT ${lim} OFFSET ${off}`
        );
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "coalitionLoyalty",
        "mutation",
        "Executed coalitionLoyalty mutation"
      );
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const result = await db.execute(
        sql`SELECT id, data, status, created_at, agent_id, metadata FROM "loyalty_members" WHERE id = ${recordId}`
      );
  ...
  analytics: protectedProcedure.query(async () => {
    ...
    try {
      const result = await db.execute(
        sql`SELECT status, COUNT(*) as cnt FROM "loyalty_members" GROUP BY status`
      );
