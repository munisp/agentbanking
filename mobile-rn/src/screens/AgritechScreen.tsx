...
const SeasonChip = ({ item }: { item: RecordItem }) => {
    const season = item.season || 'dry';
    const colors: Record<string, string> = { planting: '#22c55e', growing: '#84cc16', harvesting: '#f59e0b', dry: '#92400e' };
    const icons: Record<string, string> = { planting: '🌱', growing: '🌿', harvesting: '🌾', dry: '☀️' };
    return (<View style={{ flexDirection: 'row', backgroundColor: (colors[season] || '#92400e') + '20', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 8 }}>
      <Text>{icons[season] || '☀️'} </Text><Text style={{ fontSize: 10, color: colors[season] || '#92400e', fontWeight: '600' }}>{season.toUpperCase()}</Text>
    </View>);
  };

export default function AgritechScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/agritech.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/agritech.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading AgriTech Payments...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>AgriTech Payments</Text>
            <Text style={styles.subtitle}>Farm inputs, crop sales & cooperatives</Text>
          </View>
          <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🌾</Text>
            <Text style={styles.statLabel}>Farms</Text>
            <Text style={styles.statValue}>{stats?.registeredFarms ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>👥</Text>
            <Text style={styles.statLabel}>Cooperatives</Text>
            <Text style={styles.statValue}>{stats?.cooperatives ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🛒</Text>
            <Text style={styles.statLabel}>Input Sales</Text>
            <Text style={styles.statValue}>₦{stats?.totalInputSales ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🌻</Text>
            <Text style={styles.statLabel}>Crop Sales</Text>
            <Text style={styles.statValue}>₦{stats?.totalCropSales ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.farmName || `Record #${item.id}`}</Text>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(item.status) + '20' }]}>
              <Text style={[styles.statusText, { color: getStatusColor(item.status) }]}>{(item.status || '—').toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.cardBody}>
            <SeasonChip item={item} />
          </View>
        </View>
      )}
