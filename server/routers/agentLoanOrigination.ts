...
export const agentLoanOriginationRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(agentLoans)
          .orderBy(desc((agentLoans as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentLoanOrigination] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(agentLoans)
        .where(eq((agentLoans as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentLoanOrigination record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentLoanOrigination] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(agentLoans);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(agentLoans)
        .where(gte((agentLoans as any).createdAt, since))
        .orderBy(desc((agentLoans as any).id))
        .limit(input.limit);
