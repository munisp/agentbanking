...
async def get_supported_corridors(client: Dict = Depends(verify_api_key)):
    """
...
async def get_usage_stats(client: Dict = Depends(verify_api_key)):
    """
...
async def send_webhook(url: str, secret: Optional[str], event: str, data: Dict):
