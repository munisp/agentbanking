...
export const agentRevenueAttributionRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((commissionPayouts as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentRevenueAttribution] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(commissionPayouts)
        .where(eq((commissionPayouts as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentRevenueAttribution record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentRevenueAttribution] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(commissionPayouts);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(commissionPayouts)
        .where(gte((commissionPayouts as any).createdAt, since))
        .orderBy(desc((commissionPayouts as any).id))
        .limit(input.limit);
