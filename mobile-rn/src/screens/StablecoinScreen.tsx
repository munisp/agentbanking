...
const PegBadge = ({ item }: { item: RecordItem }) => {
    const dev = Number(item.peg_deviation || 0);
    const color = Math.abs(dev) < 0.01 ? '#22c55e' : Math.abs(dev) < 0.05 ? '#f59e0b' : '#ef4444';
    return (<View style={{ flexDirection: 'row', alignItems: 'center' }}>
      <Text style={{ fontSize: 12, fontWeight: '700', color }}>{(dev * 100).toFixed(2)}%</Text></View>);
  };
...
export default function StablecoinScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/stablecoin.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/stablecoin.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Stablecoin Rails...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Stablecoin Rails</Text>
            <Text style={styles.subtitle}>cNGN stablecoin — mint, transfer, settle</Text>
          </View>
          <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>👛</Text>
            <Text style={styles.statLabel}>Wallets</Text>
            <Text style={styles.statValue}>{stats?.totalWallets ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🔄</Text>
            <Text style={styles.statLabel}>Circulating</Text>
            <Text style={styles.statValue}>cNGN {stats?.circulatingSupply ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📊</Text>
            <Text style={styles.statLabel}>Daily Volume</Text>
            <Text style={styles.statValue}>₦{stats?.dailyVolume ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📏</Text>
            <Text style={styles.statLabel}>Peg Deviation</Text>
            <Text style={styles.statValue}>{stats?.pegDeviation ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.walletAddress || `Record #${item.id}`}</Text>
