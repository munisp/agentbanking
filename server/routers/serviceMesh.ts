...
async function publishserviceMeshMiddleware(
  action: string,
  ref: string,
...
export const serviceMeshRouter = router({
  ...
  dashboard: protectedProcedure.query(async () => {
    ...
    await publishserviceMeshMiddleware("dashboard", `${Date.now()}`, {
      action: "dashboard",
    }).catch(() => {});
  ...
  toggleCircuitBreaker: protectedProcedure.mutation(async () => {
    // Middleware fan-out (fail-open)
    await publishserviceMeshMiddleware(
      "toggleCircuitBreaker",
      `${Date.now()}`,
      { action: "toggleCircuitBreaker" }
    ).catch(() => {});
