...
export const agentPerformanceLeaderboardRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((agentPerformanceScores as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentPerformanceLeaderboard] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(agentPerformanceScores)
        .where(eq((agentPerformanceScores as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentPerformanceLeaderboard record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentPerformanceLeaderboard] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(agentPerformanceScores)
        .where(gte((agentPerformanceScores as any).createdAt, since))
        .orderBy(desc((agentPerformanceScores as any).id))
        .limit(input.limit);
