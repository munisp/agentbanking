...
export const publishReadinessCheckerRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[publishReadinessChecker] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`publishReadinessChecker record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[publishReadinessChecker] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
