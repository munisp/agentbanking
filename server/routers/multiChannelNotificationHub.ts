...
export const multiChannelNotificationHubRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[multiChannelNotificationHub] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`multiChannelNotificationHub record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[multiChannelNotificationHub] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
