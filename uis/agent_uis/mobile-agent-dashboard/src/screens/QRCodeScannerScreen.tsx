...
const QRCodeScannerScreen: React.FC = () => {
  ...
  const handleManualSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api.54agent.io/v1/payments/resolve-qr', {
        method: 'POST',
        headers: {
...
const makeStyles = (colors: any) => StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: colors.primary, // 54agent background
  },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  corner: {
    ...
    width: 40,
    height: 40,
    borderColor: '#6C63FF', // 54agent primary
    borderWidth: 4,
  },
  topLeft: {
    top: 0,
  ...
  inputWrapper: {
    backgroundColor: '#fff', // 54agent card
    borderRadius: 12,
    padding: 8,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  input: {
    ...
    paddingHorizontal: 15,
    fontSize: 16,
    color: colors.primary, // 54agent text
  },
  submitButton: {
    backgroundColor: '#6C63FF',
    height: 44,
