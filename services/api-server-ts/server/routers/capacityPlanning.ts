...
function validateCapacityplanningInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_CAPACITYPLANNING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CAPACITYPLANNING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CAPACITYPLANNING.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for capacityPlanning ───────────────────────────────────────
