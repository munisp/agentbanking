...
import simOrchestratorService from "./simOrchestratorService";
import {
  isDeviceOnline,
...
class TransactionService {
  ...
  async sendTransferOnline(transferKind, data, txRef) {
    const headers = await paymentHubHeaders();
    const payload = this.buildTransferPayload(transferKind, data, headers);

    // Generate a ref if none was supplied (e.g. when called from the offline sync).
    const ref = txRef || `TXN-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
    const terminalId = await SecureStore.getItemAsync("terminalId").catch(() => null);

    // Arm the SIM watchdog for the duration of this transfer.
    await simOrchestratorService.signalTransactionStart(ref, terminalId);
    try {
      return await this.sendTransferRequest(payload, headers);
    } finally {
  ...
  async submitTransferWithOfflineFallback(transferKind, data) {
    ...
    try {
      const result = await this.sendTransferOnline(transferKind, data, txRef);
      return {
        queued: false,
        ...result,
      };
    } catch (error) {
      if (!error?.statusCode && isTransientNetworkError(error)) {
        const queueId = await queueTransferForSync(transferKind, data);
        return {
          queued: true,
          queueId,
