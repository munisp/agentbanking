...
export const paymentGatewayRouterRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[paymentGatewayRouter] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`paymentGatewayRouter record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[paymentGatewayRouter] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
