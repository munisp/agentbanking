...
async function publishsatelliteConnectivityMiddleware(
  action: string,
  ref: string,
...
export const satelliteConnectivityRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await publishsatelliteConnectivityMiddleware("create", `${Date.now()}`, {
        action: "create",
      }).catch(() => {});
  ...
  updateStatus: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Middleware fan-out (fail-open)
      await publishsatelliteConnectivityMiddleware(
        "updateStatus",
        `${Date.now()}`,
        { action: "updateStatus" }
      ).catch(() => {});
