...
export const geoFenceDedicatedRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(geoFences)
          .orderBy(desc((geoFences as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[geoFenceDedicated] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(geoFences)
        .where(eq((geoFences as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`geoFenceDedicated record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[geoFenceDedicated] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(geoFences);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(geoFences)
        .where(gte((geoFences as any).createdAt, since))
        .orderBy(desc((geoFences as any).id))
        .limit(input.limit);
