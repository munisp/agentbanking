...
function validatePartnerrevenuesharingInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "partnerRevenueSharing",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_PARTNERREVENUESHARING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PARTNERREVENUESHARING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PARTNERREVENUESHARING.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _partnerRevenueSharing_db = {
...
// ── Transaction Handling for partnerRevenueSharing ───────────────────────────────────────
