...
MARKETPLACE_API_BASE = os.getenv("KONGA_API_URL", "https://api.konga.com/v1")
MARKETPLACE_API_KEY = os.getenv("KONGA_API_KEY", "")
MARKETPLACE_SELLER_ID = os.getenv("KONGA_SELLER_ID", "")

async def marketplace_request(method: str, path: str, params: Optional[Dict] = None, body: Optional[Dict] = None) -> Dict[str, Any]:
    """Make authenticated request to Konga API."""
    ...
    try:
        ...
        if pool:
            await pool.execute(
                ...
                f"api_error_{path}", json.dumps({"error": str(e), "path": path, "method": method}), "konga-service"
            )
        raise
...
async def search_products(q: Optional[str] = None, page: int = 1, limit: int = 20):
    """Real Konga API: search_products"""
    ...
    try:
        ...
        pool = await get_pg_pool()
        if pool:
            await pg_set_dict("konga-service", "search_products_cache", result)
        return result
    except Exception as e:
        # Fallback to cached results
        pool = await get_pg_pool()
        if pool:
            cached = await pg_get_dict("konga-service", "search_products_cache")
            if cached:
                return {**cached, "_cached": True}
        raise HTTPException(503, f"Marketplace API unavailable: {e}")


@app.get("/marketplace/get_product")
async def get_product(q: Optional[str] = None, page: int = 1, limit: int = 20):
    """Real Konga API: get_product"""
    params = {"page": page, "limit": limit}
    if q:
        params["query"] = q
    try:
        ...
        pool = await get_pg_pool()
        if pool:
            await pg_set_dict("konga-service", "get_product_cache", result)
        return result
    except Exception as e:
        # Fallback to cached results
        pool = await get_pg_pool()
        if pool:
            cached = await pg_get_dict("konga-service", "get_product_cache")
            if cached:
                return {**cached, "_cached": True}
        raise HTTPException(503, f"Marketplace API unavailable: {e}")
...
async def get_orders(q: Optional[str] = None, page: int = 1, limit: int = 20):
    ...
    try:
        ...
        pool = await get_pg_pool()
        if pool:
            await pg_set_dict("konga-service", "get_orders_cache", result)
        return result
    except Exception as e:
        # Fallback to cached results
        pool = await get_pg_pool()
        if pool:
            cached = await pg_get_dict("konga-service", "get_orders_cache")
            if cached:
                return {**cached, "_cached": True}
        raise HTTPException(503, f"Marketplace API unavailable: {e}")


@app.post("/marketplace/update_order")
async def update_order(body: dict):
    """Real Konga API: update_order"""
    try:
        result = await marketplace_request("PUT", "/seller/orders/{order_id}", body=body)
        return result
    except Exception as e:
        raise HTTPException(503, f"Marketplace API error: {e}")
...
async def marketplace_status():
    ...
    try:
        ...
        return {"status": "connected", "marketplace": "Konga", "response": result}
    except Exception as e:
        return {"status": "disconnected", "marketplace": "Konga", "error": str(e)}
