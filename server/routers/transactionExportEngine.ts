...
export const transactionExportEngineRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((data_export_jobs as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[transactionExportEngine] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(data_export_jobs)
        .where(eq((data_export_jobs as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`transactionExportEngine record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionExportEngine] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(data_export_jobs);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(data_export_jobs)
        .where(gte((data_export_jobs as any).createdAt, since))
        .orderBy(desc((data_export_jobs as any).id))
        .limit(input.limit);
