...
const sendMessage = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    throw new TRPCError({
      ...
      message: "complianceChatbot.sendMessage is not available in this deployment",
    });
  });
...
function validateCompliancechatbotInput(
...
const INTEGRITY_RULES_COMPLIANCECHATBOT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_COMPLIANCECHATBOT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_COMPLIANCECHATBOT.validateRange(
        data.amount,
        0,
        100_000_000
      )
