...
async def generate_report(req: ReportRequest, token: str = Depends(verify_token)):
    """Register a report generation request.

    The request is persisted with status 'pending'. NO fabricated result is
    written here: a real generation worker must pick up pending reports,
    execute the actual query/render/store pipeline, and only then set
    status='completed' together with the produced artifact (file_url/result).
    """
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "status": row["status"],
            "detail": "Report request accepted; it will be generated by the reporting pipeline. Poll GET /api/v1/reports/{report_id} for status.",
        }
