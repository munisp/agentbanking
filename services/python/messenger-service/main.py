...
setup_logging("messenger-service")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/messenger_service")
...
# Configuration
class Config:
    API_KEY = os.getenv("MESSENGER_API_KEY", "demo_key")
    API_SECRET = os.getenv("MESSENGER_API_SECRET", "demo_secret")
    WEBHOOK_SECRET = os.getenv("MESSENGER_WEBHOOK_SECRET", "webhook_secret")
    API_BASE_URL = os.getenv("MESSENGER_API_URL", "https://api.messenger.com")
...
messages_cache = []  # PG-backed via pg_get_list("messenger-service", "messages")
orders_cache = []  # PG-backed via pg_get_list("messenger-service", "orders")
...
async def create_order(order: OrderMessage):
    """Create an order from Messenger message"""
    global order_count
