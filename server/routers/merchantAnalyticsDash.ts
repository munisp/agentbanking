...
export const merchantAnalyticsDashRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[merchantAnalyticsDash] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`merchantAnalyticsDash record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[merchantAnalyticsDash] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
