...
export const carrierSwitchingRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((multiSimProfiles as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[carrierSwitching] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(multiSimProfiles)
        .where(eq((multiSimProfiles as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`carrierSwitching record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[carrierSwitching] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(multiSimProfiles);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(multiSimProfiles)
        .where(gte((multiSimProfiles as any).createdAt, since))
        .orderBy(desc((multiSimProfiles as any).id))
        .limit(input.limit);
