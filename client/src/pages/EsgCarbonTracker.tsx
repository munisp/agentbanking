...
export default function EsgCarbonTracker() {
  ...
  const summary = trpc.esgCarbonTracker.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.esgCarbonTracker.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">ESG Carbon Tracker</h1>
            <p className="text-muted-foreground">Carbon footprint tracking, ESG compliance, and sustainability metrics</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Log Emissions initiated")}>Log Emissions</Button>
        <Button variant="outline" onClick={() => toast.success("Buy Offset initiated")}>Buy Offset</Button>
          </div>
        </div>
        ...
        <Card key="0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Total Emissions (tCO₂)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalEmissions ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.offsetCredits ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
        ...
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">YoY Reduction</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.reduction ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Emissions (tCO₂)</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.source ?? '—')}</td>
              <td className="p-3">{String(row.category ?? '—')}</td>
              <td className="p-3">{Number(row.emissions ?? 0).toLocaleString()}</td>
