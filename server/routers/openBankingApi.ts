...
async function publishopenBankingApiMiddleware(
  action: string,
  ref: string,
...
export const openBankingApiRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await publishopenBankingApiMiddleware("create", `${Date.now()}`, {
        action: "create",
      }).catch(() => {});
  ...
  updateStatus: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Middleware fan-out (fail-open)
      await publishopenBankingApiMiddleware("updateStatus", `${Date.now()}`, {
        action: "updateStatus",
      }).catch(() => {});
