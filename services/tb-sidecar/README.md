# 54agent TigerBeetle Sidecar

The TB sidecar is a Go 1.22 HTTP microservice that provides an **offline-first double-entry ledger** for the 54agent POS terminal. It persists transactions to a local SQLite database immediately (even without internet), then syncs to the TigerBeetle Zig cluster and PostgreSQL when connectivity is restored.
...
1. Create a `54agent` system user (no login shell)
2. Install the TigerBeetle v0.16.78 Zig binary to `/usr/local/bin/tigerbeetle`
3. Install the sidecar binary to `/usr/local/bin/54agent-tb-sidecar`
4. Install the start script to `/usr/local/bin/54agent-start-sidecar.sh`
5. Create `/etc/54agent/sidecar.env` (configuration file)
6. Register and enable `54agent-tb-sidecar.service` (systemd)
...
## Configuration (`/etc/54agent/sidecar.env`)
...
| `DATA_DIR`        | `/var/lib/54agent/tb-data` | Directory for SQLite + TigerBeetle data files  |
...
## Systemd Unit (`scripts/54agent-tb-sidecar.service`)
