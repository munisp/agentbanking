...
function validateAgentmicroinsuranceInput(
...
const INTEGRITY_RULES_AGENTMICROINSURANCE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTMICROINSURANCE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTMICROINSURANCE.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  createPolicy: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "agentMicroInsurance",
        "mutation",
        "Executed agentMicroInsurance mutation"
      );
