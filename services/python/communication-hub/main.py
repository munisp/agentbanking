import httpx
"""
Communication Hub - FastAPI microservice
Unified communication gateway for SMS, email, push notifications, WhatsApp, and in-app messaging
"""
import os
import logging
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "communication-hub"),
...
app = FastAPI(title="Communication Hub", description="Unified communication gateway for SMS, email, push notifications, WhatsApp, and in-app messaging", version="1.0.0")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/communication_hub")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "communication-hub", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
...
async def startup_event():
    ...
    await publish_kafka("communication.hub.started", {
