//! 54Link Pension Micro-Contributions — Rust Microservice
//!
...
//!
//! ## Endpoints:
//!   POST /api/v1/pension/calculate/projection — Calculate retirement projection
//!   POST /api/v1/pension/calculate/compound — Compound interest calculation
//!   POST /api/v1/pension/vesting/check — Check vesting eligibility
//!   GET  /api/v1/pension/regulatory/limits — Current regulatory limits
//!
//! Port: 8279
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8279),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "pension-micro"});
...
async fn create_record(
    ...
    dapr.publish("pension.contribution.made", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("pension-micro-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("pension_accounts", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("pension-micro:{}", id), &payload.to_string(), 3600);
    ...
    // Persist to PostgreSQL
    let _ = state.postgres.persist("pension_micro", &serde_json::json!({
        "service": "pension-micro",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("pension-micro:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("pension_accounts", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Pension Micro-Contributions (Rust) starting on port {}", port);
