...
export default function TransactionGraphAnalyzer() {
  ...
  const summary = trpc.transactionGraphAnalyzer.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.transactionGraphAnalyzer.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Network analysis of transaction flows, pattern detection, and anomaly identification</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Analyze Network initiated")}>Analyze Network</Button>
        <Button variant="outline" onClick={() => toast.success("Flag Anomaly initiated")}>Flag Anomaly</Button>
          </div>
        </div>
        ...
        <Card key="1">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalEdges ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.anomalies ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.clusters ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Risk</th>
              ...
              <td className="p-3">{String(row.sourceNode ?? '—')}</td>
              <td className="p-3">{String(row.targetNode ?? '—')}</td>
              <td className="p-3 font-mono">{formatCurrency(row.amount)}</td>
              <td className="p-3">{Number(row.frequency ?? 0).toLocaleString()}</td>
              <td className="p-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[String(row.riskScore)] || 'bg-gray-500/20 text-gray-400'}`}>{String(row.riskScore ?? '—')}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
