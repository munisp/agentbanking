...
export default function DynamicQrPayment() {
  ...
  const summary = trpc.dynamicQrPayment.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.dynamicQrPayment.list.useQuery({
    limit: 20,
    offset: page * 20,
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              QR code generation, payment collection, and merchant QR management
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Generate QR initiated")}>
              Generate QR
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Bulk Generate initiated")}
            >
              Bulk Generate
            </Button>
          </div>
          ...
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalQrCodes ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">
                            {Number(row.scans ?? 0).toLocaleString()}
                          </td>
                          <td className="p-3">
                            <span
