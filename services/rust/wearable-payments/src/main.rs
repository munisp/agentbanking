//! 54Link Wearable Payments — Rust Microservice
//!
//! NFC secure element, token management, cryptographic operations
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/wearable/nfc/tokenize — Generate NFC payment token
//!   POST /api/v1/wearable/nfc/verify — Verify NFC tap token
//!   POST /api/v1/wearable/nfc/derive-keys — Derive session keys
//!   GET  /api/v1/wearable/nfc/supported — Supported wearable types
//!
//! Port: 8270
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8270),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "wearable-payments"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "wearable-payments".into(),
...
async fn create_record(
    ...
    dapr.publish("wearable.provisioned", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("wearable-payments-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("wearable_devices", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("wearable-payments:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("wearable_transactions", &payload).await;
        // Persist to PostgreSQL
        match state.postgres.insert("wearable_transactions", &payload).await {
            Ok(row) => info!("[Postgres] Inserted into wearable_transactions: {:?}", row.get("id")),
            Err(e) => warn!("[Postgres] Insert into wearable_transactions failed: {}", e),
        }
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("wearable-payments:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("wearable_devices", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Wearable Payments (Rust) starting on port {}", port);
