...
import { Trophy, Star, TrendingUp, Users, Target, Award, Medal } from "lucide-react";
          ...
          {(["platinum", "gold", "silver", "bronze"] as const).map((tier) => (
            <Card key={tier} className={tierBg[tier]}>
              <CardContent className="pt-4 text-center">
                <Medal className={`w-6 h-6 mx-auto mb-1 ${tierColors[tier]}`} />
                <p className="text-xl font-bold">{mockAgents.filter((a) => a.tier === tier).length}</p>
                <p className="text-xs text-muted-foreground capitalize">{tier} Agents</p>
              </CardContent>
            </Card>
          ))}
        </div>
              ...
              {mockAgents.map((agent, idx) => (
                <div
                  ...
                  className={`flex items-center gap-4 p-3 rounded-lg cursor-pointer transition-colors ${
                    selectedAgent === agent.agentId ? "bg-muted" : "hover:bg-muted/50"
                  }`}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge className={`text-xs capitalize ${tierColors[agent.tier]}`} variant="outline">
                        {agent.tier}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <Progress value={agent.overallScore} className="h-2 flex-1" />
                      <span className="text-sm font-mono">{agent.overallScore.toFixed(1)}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`flex items-center gap-1 text-xs ${
                      agent.trend === "improving" ? "text-green-400" : agent.trend === "declining" ? "text-red-400" : "text-gray-400"
                    }`}>
                      {agent.trend === "improving" ? <TrendingUp className="w-3 h-3" /> : agent.trend === "declining" ? <TrendingUp className="w-3 h-3 rotate-180" /> : null}
                      {agent.trend}
                    </div>
                  </div>
                </div>
                ...
                {Object.entries(selected.breakdown).map(([key, kpi]) => (
                  <div key={key} className="p-3 rounded-lg bg-muted/30">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">Weight: {(kpi.weight * 100).toFixed(0)}%</span>
                    </div>
