...
function validateMerchantsettlementdashboardInput(
...
const INTEGRITY_RULES_MERCHANTSETTLEMENTDASHBOARD = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_MERCHANTSETTLEMENTDASHBOARD.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_MERCHANTSETTLEMENTDASHBOARD.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _merchantSettlementDashboard_db = {
