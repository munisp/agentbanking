...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "billing-stream-processor".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("billing-stream-processor").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "billing-stream-processor").await {
            eprintln!("[billing-stream-processor] Loaded persisted state from PostgreSQL");
