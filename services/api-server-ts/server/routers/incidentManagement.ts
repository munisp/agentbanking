...
function validateIncidentmanagementInput(
...
const INTEGRITY_RULES_INCIDENTMANAGEMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_INCIDENTMANAGEMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_INCIDENTMANAGEMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
