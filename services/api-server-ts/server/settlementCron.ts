...
function buildSettlementSms(data: AgentSettlement): string {
  return (
    `54agent Daily Settlement - ${new Date().toLocaleDateString("en-NG")}\n` +
...
export async function runDeadLetterDigest(): Promise<void> {
  try {
    ...
    if (failedItems.length <= autoRetryThreshold) {
      ...
      await notifyOwner({
        title: `[54agent POS] Auto-retried ${requeued} ERP dead-letter item(s)`,
        content: [
    ...
    await notifyOwner({
      title: `[54agent POS] ${failedItems.length} ERP dead-letter item(s) require attention`,
      content,
    });
...
export async function runWeeklyConnectivitySlaReport(): Promise<void> {
  try {
    ...
    await notifyOwner({
      title: `[54agent POS] Weekly Connectivity SLA — ${ranked.length} agents, ${belowSla.length} below SLA`,
      content,
    });
    console.log(
