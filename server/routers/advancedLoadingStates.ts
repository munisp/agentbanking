...
export const advancedLoadingStatesRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[advancedLoadingStates] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`advancedLoadingStates record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[advancedLoadingStates] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
