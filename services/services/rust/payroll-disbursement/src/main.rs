//! 54Link Payroll & Salary Disbursement — Rust Microservice
//!
//! Tax computation (PAYE, pension, NHF), net salary calculation, compliance
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/payroll/tax/compute — Compute PAYE, pension, NHF deductions
//!   POST /api/v1/payroll/tax/annual-return — Generate annual tax return
//!   GET  /api/v1/payroll/tax/brackets — Current tax brackets
//!   POST /api/v1/payroll/compliance/check — Compliance verification
//!
//! Port: 8252
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8252),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "payroll-disbursement"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "payroll-disbursement".into(),
...
async fn create_record(
    ...
    dapr.publish("payroll.batch.created", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("payroll-disbursement-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("payroll_employers", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("payroll-disbursement:{}", id), &payload.to_string(), 3600);
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("payroll-disbursement:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("payroll_employers", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Payroll & Salary Disbursement (Rust) starting on port {}", port);
