...
export const transactionDisputeResolutionRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[transactionDisputeResolution] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`transactionDisputeResolution record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionDisputeResolution] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
