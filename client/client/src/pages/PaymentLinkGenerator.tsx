...
export default function PaymentLinkGenerator() {
  ...
  const summary = trpc.paymentLinkGenerator.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.paymentLinkGenerator.list.useQuery({
    limit: 20,
    offset: page * 20,
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Generate and manage shareable payment links for agents and
              merchants
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Create Link initiated")}>
              Create Link
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Bulk Create initiated")}
            >
              Bulk Create
            </Button>
          </div>
          ...
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalLinks ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
