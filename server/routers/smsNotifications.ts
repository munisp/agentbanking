...
function validateSmsnotificationsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_SMSNOTIFICATIONS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SMSNOTIFICATIONS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SMSNOTIFICATIONS.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  send: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "smsNotifications",
        "mutation",
        "Executed smsNotifications mutation"
      );
