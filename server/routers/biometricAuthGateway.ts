...
import { fido2Credentials } from "../../drizzle/schema";
...
export const biometricAuthGatewayRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((fido2Credentials as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[biometricAuthGateway] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(fido2Credentials)
        .where(eq((fido2Credentials as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`biometricAuthGateway record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[biometricAuthGateway] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(fido2Credentials);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(fido2Credentials)
        .where(gte((fido2Credentials as any).createdAt, since))
        .orderBy(desc((fido2Credentials as any).id))
        .limit(input.limit);
