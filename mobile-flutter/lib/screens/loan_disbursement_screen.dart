...
class LoanDisbursementScreen extends StatefulWidget {
  const LoanDisbursementScreen({super.key});
  @override
...
class _LoanDisbursementScreenState extends State<LoanDisbursementScreen> {
  ...
  Future<void> _load() async {
    try {
      final result = await ApiService.instance.get('/lending/list', queryParams: {'page': '1', 'limit': '50'});
      setState(() {
        _data = result ?? {};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Loan Disbursement'),
        actions: [
