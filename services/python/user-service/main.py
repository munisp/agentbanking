...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "user-service"),
...
app = FastAPI(title="User Service", description="User Service for Remittance Platform", version="1.0.0")
...
async def health_check():
    try:
        ...
        return {"status": "degraded", "service": "user-service", "error": str(e)}
...
async def create_item(item: ItemCreate, token: str = Depends(verify_token)):
    ...
    async with pool.acquire() as conn:
        ...
        query = f"INSERT INTO user_profiles ({', '.join(cols)}) VALUES ({ph}) RETURNING *"
        row = await conn.fetchrow(query, *vals)
        return dict(row)

@app.get("/api/v1/user-service")
...
@app.get("/api/v1/user-service/{item_id}")
async def get_item(item_id: str, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM user_profiles WHERE id=$1", uuid.UUID(item_id))
        if not row:
            raise HTTPException(status_code=404, detail="Item not found")
        return dict(row)

@app.put("/api/v1/user-service/{item_id}")
async def update_item(item_id: str, item: ItemUpdate, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        existing = await conn.fetchrow("SELECT * FROM user_profiles WHERE id=$1", uuid.UUID(item_id))
        if not existing:
            raise HTTPException(status_code=404, detail="Item not found")
        ...
        for k, v in updates.items():
            ...
            idx += 1
        query = f"UPDATE user_profiles SET {', '.join(set_parts)}, updated_at=NOW() WHERE id=$1 RETURNING *"
        row = await conn.fetchrow(query, *params)
        return dict(row)

@app.delete("/api/v1/user-service/{item_id}")
async def delete_item(item_id: str, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        result = await conn.execute("DELETE FROM user_profiles WHERE id=$1", uuid.UUID(item_id))
        if result == "DELETE 0":
            raise HTTPException(status_code=404, detail="Item not found")
        return {"deleted": True}

@app.get("/api/v1/user-service/stats")
async def get_stats(token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        total = await conn.fetchval("SELECT COUNT(*) FROM user_profiles")
        today = await conn.fetchval("SELECT COUNT(*) FROM user_profiles WHERE created_at >= CURRENT_DATE")
        return {"total": total, "today": today, "service": "user-service"}
