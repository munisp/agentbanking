//! 54Link Conversational Banking — Rust Microservice
//!
//! Message parsing, intent extraction, NLU tokenizer, response templating
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/chat/parse — Parse and tokenize message
//!   POST /api/v1/chat/intent — Extract intent from message
//!   POST /api/v1/chat/entities — Extract entities (amount, account, name)
//!   POST /api/v1/chat/template/render — Render response template
//!
//! Port: 8261
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "conversational-banking"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "conversational-banking".into(),
...
async fn create_record(
    ...
    dapr.publish("chat.message.received", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("conversational-banking-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("chat_sessions", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("conversational-banking:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("chat_sessions", &payload).await;
        // Persist to PostgreSQL
        match state.postgres.insert("chat_sessions", &payload).await {
            Ok(row) => info!("[Postgres] Inserted into chat_sessions: {:?}", row.get("id")),
            Err(e) => warn!("[Postgres] Insert into chat_sessions failed: {}", e),
        }

    // Persist to PostgreSQL
    let _ = state.postgres.persist("conversational_banking", &serde_json::json!({
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("conversational-banking:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("chat_sessions", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Conversational Banking (Rust) starting on port {}", port);
