//! 54Link Carbon Credit Marketplace — Rust Microservice
//!
//! Double-entry ledger for credits, atomic swap engine, registry integrity
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/carbon/ledger/record — Record credit movement in ledger
//!   POST /api/v1/carbon/swap — Atomic swap of credits for payment
//!   GET  /api/v1/carbon/ledger/balance/{id} — Credit balance for entity
//!   POST /api/v1/carbon/ledger/verify — Verify ledger integrity
//!
//! Port: 8282
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8282),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "carbon-credit-marketplace"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "carbon-credit-marketplace".into(),
...
async fn create_record(
    ...
    dapr.publish("carbon.project.registered", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("carbon-credit-marketplace-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("carbon_projects", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("carbon-credit-marketplace:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("carbon_trades", &payload).await;
        // Persist to PostgreSQL
        match state.postgres.insert("carbon_trades", &payload).await {
            Ok(row) => info!("[Postgres] Inserted into carbon_trades: {:?}", row.get("id")),
            Err(e) => warn!("[Postgres] Insert into carbon_trades failed: {}", e),
        }

    // Persist to PostgreSQL
    let _ = state.postgres.persist("carbon_credit_marketplace", &serde_json::json!({
        "service": "carbon-credit-marketplace",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("carbon-credit-marketplace:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("carbon_projects", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Carbon Credit Marketplace (Rust) starting on port {}", port);
