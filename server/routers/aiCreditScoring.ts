...
function validateAicreditscoringInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_AICREDITSCORING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AICREDITSCORING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AICREDITSCORING.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "aiCreditScoring",
        "mutation",
        "Executed aiCreditScoring mutation"
      );
