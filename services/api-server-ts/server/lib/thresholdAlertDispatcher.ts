...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="margin-top: 24px; padding: 12px; background: #fef3c7; border-radius: 4px; font-size: 13px; color: #92400e;">
    <strong>Action Required:</strong> Please review this alert in the 54agent Dashboard under
    Data Threshold Alerts and acknowledge or resolve it.
  </div>

  <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #e5e7eb; font-size: 12px; color: #9ca3af;">
    <p>This is an automated alert from 54agent POS Shell. Rule ID: ${event.ruleId} | Event ID: ${event.eventId}</p>
...
Action Required: Review this alert in the 54agent Dashboard.
    ...
    switch (channel) {
      ...
      case "webhook": {
        ...
        const result = await sendWebhookNotification(
          "https://hooks.54agent.com/alerts",
          payload
        );
        const record: NotificationRecord = {
          id: `notif_${nextNotifId++}`,
          ...
          ruleId: event.ruleId,
          channel: "webhook",
          recipient: "https://hooks.54agent.com/alerts",
          status: result.success ? "sent" : "failed",
          sentAt: new Date().toISOString(),
        };
        dispatched.push(record);
