...
export const apiAnalyticsDashRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[apiAnalyticsDash] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`apiAnalyticsDash record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[apiAnalyticsDash] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
