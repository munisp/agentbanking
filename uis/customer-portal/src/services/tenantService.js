...
class TenantService {
  ...
  /** Get tenant_id from env variable or localStorage */
  getTenantId() {
    const envTenantId = import.meta.env.VITE_TENANT_ID;
    if (envTenantId) return envTenantId;
    return localStorage.getItem(this.TENANT_ID_KEY) || null;
  }

  /** Persist tenant_id in localStorage */
  ...
  getKeycloakRealm() {
    ...
    if (config?.feature_flags) {
      ...
      if (authFeature?.config?.realm) return authFeature.config.realm;
    }
  ...
  /** Fetch tenant data from core-banking API and cache it */
  async getTenant(tenantId) {
    const existing = this.getTenantConfig();
    if (existing) return existing;

    const id = tenantId || this.getTenantId();
    if (!id) {
      throw new Error(
        "Tenant ID is required. Please set VITE_TENANT_ID in .env.",
      );
    }
    ...
    try {
      ...
      if (data.message === "success" && data.tenant) {
        this.setTenantConfig(data.tenant);
        return data.tenant;
      }
  ...
  /** Read tenant config from localStorage */
  getTenantConfig() {
  ...
  /** Write tenant config to localStorage */
  ...
  getDefaultConfig() {
    const tenantId = this.getTenantId() || "54agent";
    return {
      id: 0,
  ...
  /** Boot: fetch & cache the tenant config on app startup */
  async initialize() {
