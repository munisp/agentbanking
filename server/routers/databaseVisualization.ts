...
const runHealthCheck = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed databaseVisualization mutation"
    );
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "databaseVisualization",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_DATABASEVISUALIZATION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DATABASEVISUALIZATION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DATABASEVISUALIZATION.validateRange(
        data.amount,
        0,
        100_000_000
      )
