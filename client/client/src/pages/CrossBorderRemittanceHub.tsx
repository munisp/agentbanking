...
export default function CrossBorderRemittanceHub() {
  ...
  const summary = trpc.crossBorderRemittanceHub.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.crossBorderRemittanceHub.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              International money transfers, corridor management, and FX rates
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            <Button
              variant="outline"
              onClick={() => toast.success("Check Rates initiated")}
            >
              Check Rates
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalTransfers ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
          ...
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.activeCorridors ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.avgFxRate ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">{String(row.corridor ?? "—")}</td>
                          <td className="p-3">
                            <span
