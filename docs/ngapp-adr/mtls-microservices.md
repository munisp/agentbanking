...
```
54agent Root CA  (offline, HSM-protected)
└── 54agent Intermediate CA  (online, rotated every 90 days)
    ├── pos-shell.svc.54agent.internal
    ├── kyc-service.svc.54agent.internal
    ├── fraud-service.svc.54agent.internal
    ├── settlement-service.svc.54agent.internal
    ├── float-service.svc.54agent.internal
    ├── analytics-service.svc.54agent.internal
    ├── geofencing-service.svc.54agent.internal
    └── tigerbeetle-sidecar.svc.54agent.internal
```
...
spec:
  ...
  subject:
    organizations: ["54agent"]
  commonName: pos-shell.svc.54agent.internal
  dnsNames:
    - pos-shell.svc.54agent.internal
    - pos-shell.54agent.svc.cluster.local
  issuerRef:
    name: 54agent-intermediate-ca
    kind: ClusterIssuer
```
...
const CERT_DIR = process.env.MTLS_CERT_DIR ?? "/etc/54agent/certs";
...
| ---------------------------- | ----------------------------------------- | ---- | ----------------- | ------------------------ |
| KYC Service (Python FastAPI) | `kyc-service.svc.54agent.internal`         | 8443 | mTLS              | Liveness + OCR endpoints |
| Fraud Service (Go)           | `fraud-service.svc.54agent.internal`       | 8443 | mTLS              | Real-time scoring        |
| Settlement Service (Rust)    | `settlement-service.svc.54agent.internal`  | 8443 | mTLS              | ISO 8583 bridge          |
| Float Service (Go)           | `float-service.svc.54agent.internal`       | 8443 | mTLS              | Balance + history        |
| Analytics Service (Python)   | `analytics-service.svc.54agent.internal`   | 8443 | mTLS              | Metrics aggregation      |
| Geofencing Service (Go)      | `geofencing-service.svc.54agent.internal`  | 8443 | mTLS              | Polygon enforcement      |
| TigerBeetle Sidecar          | `tigerbeetle-sidecar.svc.54agent.internal` | 3001 | mTLS              | Offline-first ledger     |
| Keycloak                     | `keycloak.svc.54agent.internal`            | 8443 | TLS (server-only) | OIDC discovery           |
| APISix Gateway               | `apisix.svc.54agent.internal`              | 9443 | mTLS              | Upstream auth            |
...
openssl s_client \
  -connect pos-shell.svc.54agent.internal:8443 \
  -cert /etc/54agent/certs/tls.crt \
  -key  /etc/54agent/certs/tls.key \
  -CAfile /etc/54agent/certs/ca.crt \
  -verify_return_error

# Check certificate expiry
openssl x509 -in /etc/54agent/certs/tls.crt -noout -dates
...
| ------------------------- | ----------------------- | --------------------------------------------------- |
| `MTLS_CERT_DIR`           | `/etc/54agent/certs`     | Directory containing `tls.crt`, `tls.key`, `ca.crt` |
