...
export const socialCommerceGatewayRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((ecommerceProducts as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[socialCommerceGateway] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(ecommerceProducts)
        .where(eq((ecommerceProducts as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`socialCommerceGateway record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[socialCommerceGateway] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(ecommerceProducts);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(ecommerceProducts)
        .where(gte((ecommerceProducts as any).createdAt, since))
        .orderBy(desc((ecommerceProducts as any).id))
        .limit(input.limit);
