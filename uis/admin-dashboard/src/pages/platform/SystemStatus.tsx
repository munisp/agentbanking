...
const API_URL = import.meta.env.VITE_API_URL || "https://54agent.upi.dev";
  ...
  const runHealthChecks = async () => {
    ...
    try {
      ...
      results.push({
        name: "API Gateway",
        status: res.ok ? "healthy" : "degraded",
        latency: Date.now() - gwStart,
        lastChecked: new Date(),
        details: `${Date.now() - gwStart}ms response`,
        icon: Wifi,
      });
    } catch {
      results.push({
  ...
  return (
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div
          className={`rounded-lg border p-6 text-center ${(statusColors as any)[overallStatus]}`}
        >
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Badge className={(statusColors as any)[check.status]}>
                    {check.status}
                  </Badge>
                </div>
              </div>
