...
export const advancedAuditLogViewerRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[advancedAuditLogViewer] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`advancedAuditLogViewer record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[advancedAuditLogViewer] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
