...
export default function RegulatoryReportingPage() {
  ...
  const [statsQuery, setStatsQuery] = useState<{ data: any; isLoading: boolean }>({ data: null, isLoading: false });

  useEffect(() => {
    setStatsQuery((s) => ({ ...s, isLoading: true }));
    fetch(`${CORE_BANKING_URL}/compliance/api/v1/regulatory-reporting-engine/stats`, { headers: authHeaders() })
      .then((r) => r.ok ? r.json() : null)
      .then((json) => setStatsQuery({ data: json, isLoading: false }))
      .catch(() => setStatsQuery({ data: null, isLoading: false }));
  }, []);
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">CBN, NIBSS, NFIU regulatory report generation and submission</p>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="settings">
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <p className="text-muted-foreground">Configure Regulatory Reporting settings and preferences.</p>
