...
func init() {
	...
	defaultConfigs := []BackupConfig{
		{ID: "pg-main", ResourceType: "postgresql", ResourceName: "54agent-production", Schedule: "0 2 * * *", RetentionDays: 30, StoragePath: "s3://54agent-backups/postgresql/", Compression: "gzip", Encryption: true, Enabled: true},
		{ID: "pg-wal", ResourceType: "postgresql-wal", ResourceName: "54agent-production-wal", Schedule: "*/15 * * * *", RetentionDays: 7, StoragePath: "s3://54agent-backups/wal-archive/", Compression: "lz4", Encryption: true, Enabled: true},
		{ID: "redis-rdb", ResourceType: "redis", ResourceName: "54agent-redis", Schedule: "0 */6 * * *", RetentionDays: 14, StoragePath: "s3://54agent-backups/redis/", Compression: "gzip", Encryption: true, Enabled: true},
		{ID: "tb-ledger", ResourceType: "tigerbeetle", ResourceName: "54agent-tigerbeetle", Schedule: "0 3 * * *", RetentionDays: 365, StoragePath: "s3://54agent-backups/tigerbeetle/", Compression: "zstd", Encryption: true, Enabled: true},
		{ID: "minio-data", ResourceType: "minio", ResourceName: "54agent-minio", Schedule: "0 4 * * 0", RetentionDays: 90, StoragePath: "s3://54agent-backups/minio/", Compression: "zstd", Encryption: true, Enabled: true},
		{ID: "opensearch", ResourceType: "opensearch", ResourceName: "54agent-opensearch", Schedule: "0 1 * * *", RetentionDays: 30, StoragePath: "s3://54agent-backups/opensearch/", Compression: "gzip", Encryption: true, Enabled: true},
		{ID: "keycloak-db", ResourceType: "postgresql", ResourceName: "keycloak-db", Schedule: "0 2 * * *", RetentionDays: 30, StoragePath: "s3://54agent-backups/keycloak/", Compression: "gzip", Encryption: true, Enabled: true},
	}
	...
	drPlans["dr-full"] = &DRPlan{
		...
		Status: "active",
		Steps: []DRStep{
			{Order: 1, Action: "Restore PostgreSQL from latest backup", Resource: "postgresql", Command: "pg_restore --dbname=54agent_production", Timeout: "30m"},
