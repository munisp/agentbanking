...
function validateTransactionmapvizInput(
...
const INTEGRITY_RULES_TRANSACTIONMAPVIZ = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_TRANSACTIONMAPVIZ.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_TRANSACTIONMAPVIZ.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for transactionMapViz ───────────────────────────────────────
