...
export const nlAnalyticsQueryRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[nlAnalyticsQuery] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`nlAnalyticsQuery record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[nlAnalyticsQuery] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
