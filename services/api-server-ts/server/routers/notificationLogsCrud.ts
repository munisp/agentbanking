...
function validateNotificationlogscrudInput(
...
const INTEGRITY_RULES_NOTIFICATIONLOGSCRUD = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_NOTIFICATIONLOGSCRUD.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_NOTIFICATIONLOGSCRUD.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _notificationLogsCrud_db = {
