...
export default function RegulatorySandboxTester() {
  ...
  const summary = trpc.regulatorySandboxTester.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.regulatorySandboxTester.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Controlled testing environment for new financial products and regulatory compliance</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("New Test initiated")}>New Test</Button>
        <Button variant="outline" onClick={() => toast.success("Submit for Review initiated")}>Submit for Review</Button>
          </div>
        </div>
        ...
        <Card key="1">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Products Tested</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalProducts ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.approvedProducts ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Test ID</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Product</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Regulation</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.regulation ?? '—')}</td>
              <td className="p-3">{String(row.testPhase ?? '—')}</td>
              <td className="p-3 text-sm text-muted-foreground">{row.startDate ? new Date(String(row.startDate)).toLocaleDateString() : '—'}</td>
