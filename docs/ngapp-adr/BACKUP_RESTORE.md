# 54agent Backup & Restore Procedures

## Overview

This document covers backup and restore procedures for all 54agent data stores.
...
```bash
# /etc/cron.d/54agent-db-backup
0 2 * * * postgres pg_dump -Fc -Z6 \
  --dbname=$DATABASE_URL \
  --file=/backups/postgres/54agent-$(date +\%Y\%m\%d-\%H\%M).dump \
...
wal_level = replica
archive_mode = on
archive_command = 'aws s3 cp %p s3://54agent-backups/wal/%f --sse AES256'
```
...
pg_dump -Fc -Z6 \
  ...
  --file=/backups/54agent-$(date +%Y%m%d-%H%M).dump

# Schema only (for migration reference)
pg_dump --schema-only \
  --dbname="postgresql://postgres:${DB_PASSWORD}@localhost:5432/ngapp" \
  > /backups/54agent-schema-$(date +%Y%m%d).sql

# Verify backup integrity
pg_restore --list /backups/54agent-$(date +%Y%m%d-%H%M).dump | head
```
...
pg_restore -d ngapp -U postgres --clean --if-exists \
  /backups/54agent-YYYYMMDD-HHMM.dump
...
aws s3 cp s3://54agent-backups/base/latest.tar.gz /tmp/
tar xzf /tmp/latest.tar.gz -C /var/lib/postgresql/data/

# 4. Create recovery.conf
cat > /var/lib/postgresql/data/recovery.conf << EOF
restore_command = 'aws s3 cp s3://54agent-backups/wal/%f %p'
...
tar czf /backups/config/54agent-config-$(date +%Y%m%d).tar.gz \
  .env \
  docker-compose.production.yml \
  helm/54agent/values.yaml \
  monitoring/ \
  terraform/

# Upload to S3
aws s3 sync /backups/ s3://54agent-backups/ --sse AES256
```
