// 54agent Agency Banking Platform — ESC/POS Thermal Printer Service
// Language: Rust
...
impl ReceiptEngine {
    ...
    fn render_header(&self, builder: &mut EscPosBuilder, data: &ReceiptData, config: &TerminalPrinterConfig) {
        ...
        builder.line(&data.merchant_name.clone().unwrap_or("54agent AGENCY".to_string()));
        builder.font_size(false, false);
        builder.bold(false);
        if let Some(addr) = &data.merchant_address {
            builder.line(addr);
        }
        builder.line("54agent Agency Banking Platform");
    ...
    fn render_footer(&self, builder: &mut EscPosBuilder, data: &ReceiptData, config: &TerminalPrinterConfig, include_qr: bool) {
        ...
        builder.line(&data.footer_message.clone().unwrap_or("Thank you for using 54agent".to_string()));
...
async fn test_print_handler(
    ...
    let req = PrintReceiptRequest {
        ...
        data: ReceiptData {
            merchant_name: Some("54agent Test".to_string()),
