...
export const offlineQueueRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(dlqMessages)
          .orderBy(desc((dlqMessages as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[offlineQueue] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(dlqMessages)
        .where(eq((dlqMessages as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`offlineQueue record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[offlineQueue] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(dlqMessages);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(dlqMessages)
        .where(gte((dlqMessages as any).createdAt, since))
        .orderBy(desc((dlqMessages as any).id))
        .limit(input.limit);
