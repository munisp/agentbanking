...
export const bulkTransactionProcessingRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(transactions)
          .orderBy(desc((transactions as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[bulkTransactionProcessing] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(transactions)
        .where(eq((transactions as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`bulkTransactionProcessing record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[bulkTransactionProcessing] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(transactions)
        .where(gte((transactions as any).createdAt, since))
        .orderBy(desc((transactions as any).id))
        .limit(input.limit);
