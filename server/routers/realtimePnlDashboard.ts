...
const INTEGRITY_RULES_REALTIMEPNLDASHBOARD = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REALTIMEPNLDASHBOARD.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REALTIMEPNLDASHBOARD.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _realtimePnlDashboard_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "realtimePnlDashboard",
        "mutation",
        "Executed realtimePnlDashboard mutation"
      );
