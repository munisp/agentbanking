...
export const automatedTestingFrameworkRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(loadTestRuns)
          .orderBy(desc((loadTestRuns as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[automatedTestingFramework] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(loadTestRuns)
        .where(eq((loadTestRuns as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`automatedTestingFramework record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[automatedTestingFramework] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(loadTestRuns);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(loadTestRuns)
        .where(gte((loadTestRuns as any).createdAt, since))
        .orderBy(desc((loadTestRuns as any).id))
        .limit(input.limit);
