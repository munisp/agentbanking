/**
 * Analytics Query Router — 54agent POS Shell (Sprint 89)
 *
...
const INTEGRITY_RULES_ANALYTICSQUERY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ANALYTICSQUERY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ANALYTICSQUERY.validateRange(data.amount, 0, 100_000_000)
    )
