...
export const digitalTwinSimulatorRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[digitalTwinSimulator] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`digitalTwinSimulator record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[digitalTwinSimulator] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
