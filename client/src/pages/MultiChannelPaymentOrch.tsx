...
const statusColors: Record<string, string> = {
  ...
  'routed': 'bg-blue-500/20 text-blue-400',
...
export default function MultiChannelPaymentOrch() {
  ...
  const summary = trpc.multiChannelPaymentOrch.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.multiChannelPaymentOrch.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Multi-Channel Payment Orchestration</h1>
            <p className="text-muted-foreground">Route payments across channels (USSD, mobile, web, POS) with failover</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Configure Routes initiated")}>Configure Routes</Button>
        <Button variant="outline" onClick={() => toast.success("Test Failover initiated")}>Test Failover</Button>
          </div>
        </div>
        ...
        <Card key="1">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Channels Active</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.channelSplit ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Failover Events</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.failoverEvents ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
