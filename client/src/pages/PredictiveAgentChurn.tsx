...
export default function PredictiveAgentChurn() {
  ...
  const summary = trpc.predictiveAgentChurn.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.predictiveAgentChurn.list.useQuery({ limit: 20 });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">ML-based churn prediction, at-risk agent identification, and retention actions</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Send Intervention initiated")}>Send Intervention</Button>
        <Button variant="outline" onClick={() => toast.success("Schedule Call initiated")}>Schedule Call</Button>
          </div>
        </div>
        ...
        <Card key="0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">At-Risk Agents</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.atRiskAgents ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
        ...
        <Card key="2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.retentionRate ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.interventionsSent ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Tx Decline</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3 text-sm text-muted-foreground">{row.lastActive ? new Date(String(row.lastActive)).toLocaleDateString() : '—'}</td>
