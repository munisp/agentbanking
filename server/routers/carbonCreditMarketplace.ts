...
function validateCarboncreditmarketplaceInput(
...
const INTEGRITY_RULES_CARBONCREDITMARKETPLACE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CARBONCREDITMARKETPLACE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CARBONCREDITMARKETPLACE.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _carbonCreditMarketplace_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "carbonCreditMarketplace",
        "mutation",
        "Executed carbonCreditMarketplace mutation"
      );
