...
export default function DistributedTracingDash() {
  ...
  const summary = trpc.distributedTracingDash.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.distributedTracingDash.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              End-to-end request tracing, latency analysis, and service
              dependency mapping
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Search Traces initiated")}>
              Search Traces
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Analyze Bottleneck initiated")}
            >
              Analyze Bottleneck
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalTraces ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
          ...
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.p99Latency ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.errorTraces ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">
                            {Number(row.spans ?? 0).toLocaleString()}
                          </td>
                          <td className="p-3">
                            <span
