...
class _SuperAppScreenState extends ConsumerState<SuperAppScreen> {
  ...
  Widget _buildAppRating(Map<String, dynamic> item) {
    final rating = double.tryParse('${item[\'rating\'] ?? 0}') ?? 0.0;
    return Row(children: List.generate(5, (i) => Icon(i < rating.round() ? Icons.star : Icons.star_border, size: 14, color: i < rating.round() ? Colors.amber : Colors.grey)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.apps, size: 24), const SizedBox(width: 8), const Text('Super App Framework')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Super App Framework', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('Mini-app ecosystem — payments, transport, utilities', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Mini Apps', '${_stats?['totalApps'] ?? '\u2014'}', Icons.widgets, Colors.blue),
                              _buildStatCard('Active Users', '${_stats?['activeUsers'] ?? '\u2014'}', Icons.people, Colors.green),
                              _buildStatCard('Daily Launches', '${_stats?['dailyLaunches'] ?? '\u2014'}', Icons.rocket_launch, Colors.orange),
                              _buildStatCard('Revenue', '₦${_stats?['totalRevenue'] ?? '\u2014'}', Icons.attach_money, Colors.purple),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['name'] ?? item['category'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildAppRating(item),
                    ]),
