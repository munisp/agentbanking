"""
Router for middleware-integration service
...
The real implementation lives in the standalone service (services/middleware-integration/).
...
router = APIRouter(prefix="/middleware-integration", tags=["middleware-integration"])

_UNAVAILABLE = (
    "middleware-integration endpoints are not served by this gateway router. "
    "Use the standalone middleware-integration service."
)
...
async def health_check():
    return JSONResponse(
        status_code=503,
        content={"status": "unavailable", "service": "middleware-integration", "detail": _UNAVAILABLE},
    )
