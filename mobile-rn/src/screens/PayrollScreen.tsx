...
const DisbursePct = ({ item }: { item: RecordItem }) => {
    const d = Number(item.disbursed || 0); const t = Number(item.employeeCount || 1);
    const pct = t > 0 ? Math.round(d / t * 100) : 0;
    return (<Text style={{ fontSize: 12, color: pct >= 100 ? '#22c55e' : '#f59e0b' }}>{pct}% disbursed</Text>);
  };
...
export default function PayrollScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/payroll.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/payroll.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Payroll Disbursement...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Payroll Disbursement</Text>
            <Text style={styles.subtitle}>SME payroll through agent network</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>₦{stats?.monthlyDisbursed ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>⏳</Text>
            <Text style={styles.statLabel}>Pending</Text>
            <Text style={styles.statValue}>{stats?.pendingCashOut ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.employerName || `Record #${item.id}`}</Text>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(item.status) + '20' }]}>
              <Text style={[styles.statusText, { color: getStatusColor(item.status) }]}>{(item.status || '—').toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.cardBody}>
            <DisbursePct item={item} />
          </View>
        </View>
      )}
