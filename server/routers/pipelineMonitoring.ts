...
export const pipelineMonitoringRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[pipelineMonitoring] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`pipelineMonitoring record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[pipelineMonitoring] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
