...
import { validateRequest, TerminateContractSchema } from "../../validations";
...
export const terminateContract = asyncHandler(async (req, res) => {
  ...
  const input = validateRequest(TerminateContractSchema, req.body);
  ...
  billing.contract_end_date = new Date(input.effective_date);
  await AppDataSource.manager.save(billing);

  await billingAuditRepository.record({
    tenant_id,
    user_id,
    action: BillingAuditAction.CONTRACT_TERMINATED,
    resource_type: "tenant_billing",
    resource_id: String(billing.id),
    after_state: { status: BillingStatus.SUSPENDED, effective_date: input.effective_date, reason: input.reason },
  });
