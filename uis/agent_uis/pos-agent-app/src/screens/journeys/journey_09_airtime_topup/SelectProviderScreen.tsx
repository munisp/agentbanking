...
export const SelectProviderScreen: React.FC<SelectProviderScreenProps> = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      ...
      try {
        const response = await apiClient.get('/network-operations/providers');
        const data = response?.data || response;
        if (Array.isArray(data) && data.length > 0) {
          setProviders(data.map((p: any) => ({
            id: p.id || p.provider_code,
            name: p.name || p.provider_name,
            color: p.color || p.brand_color || '#666666',
            prefixes: p.prefixes || p.number_prefixes || '',
          })));
        }
      } catch {
...
const makeStyles = (colors: any) => StyleSheet.create({
  ...
  primaryButton: { backgroundColor: colors.primary, padding: 16, borderRadius: 14, alignItems: 'center', marginTop: 8 },
