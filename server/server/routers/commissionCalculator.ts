...
const INTEGRITY_RULES_COMMISSIONCALCULATOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_COMMISSIONCALCULATOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_COMMISSIONCALCULATOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _commissionCalculator_db = {
