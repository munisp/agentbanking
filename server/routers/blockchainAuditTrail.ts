...
export const blockchainAuditTrailRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[blockchainAuditTrail] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`blockchainAuditTrail record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[blockchainAuditTrail] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
