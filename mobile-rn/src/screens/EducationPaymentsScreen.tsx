  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/education.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/education.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Education Payments...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>School fees and exam registration</Text>
      </View>
