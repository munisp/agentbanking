...
export const agentOnboardingWorkflowRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((agentOnboardingProgress as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentOnboardingWorkflow] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(agentOnboardingProgress)
        .where(eq((agentOnboardingProgress as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentOnboardingWorkflow record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentOnboardingWorkflow] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(agentOnboardingProgress);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(agentOnboardingProgress)
        .where(gte((agentOnboardingProgress as any).createdAt, since))
        .orderBy(desc((agentOnboardingProgress as any).id))
        .limit(input.limit);
