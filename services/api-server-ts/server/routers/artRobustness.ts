...
function validateArtrobustnessInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_ARTROBUSTNESS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ARTROBUSTNESS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ARTROBUSTNESS.validateRange(data.amount, 0, 100_000_000)
    )
  ...
  attack: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "artRobustness",
        "mutation",
        "Executed artRobustness mutation"
      );
