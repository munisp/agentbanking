...
@app.get("/api/v1/float/balances")
async def list_all_float_balances(limit: int = 100, offset: int = 0, token: str = Depends(verify_token)):
    """Return float balances for all agents — used by admin float management dashboard."""
    ...
    async with pool.acquire() as conn:
        try:
            rows = await conn.fetch(
                """
                SELECT id, account_id, currency, balance, min_balance, max_balance,
                       status, last_topup_at, created_at, updated_at
                FROM float_accounts
            ...
            total = await conn.fetchval("SELECT COUNT(*) FROM float_accounts")
            return {
