...
export default function AgentDeviceFingerprint() {
  ...
  const summary = trpc.agentDeviceFingerprint.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.agentDeviceFingerprint.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Device registration, verification, and fraud detection via fingerprinting</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Verify Device initiated")}>Verify Device</Button>
        <Button variant="outline" onClick={() => toast.success("Block Device initiated")}>Block Device</Button>
          </div>
        </div>
        ...
        <Card key="0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Registered Devices</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.registeredDevices ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.verifiedDevices ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.suspiciousDevices ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.pendingVerification ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Fingerprint</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Last Seen</th>
              ...
              <td className="p-3">{String(row.fingerprint ?? '—')}</td>
              ...
              <td className="p-3 text-sm text-muted-foreground">{row.lastSeen ? new Date(String(row.lastSeen)).toLocaleDateString() : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
