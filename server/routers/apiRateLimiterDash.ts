...
export const apiRateLimiterDashRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(rateLimitRules)
          .orderBy(desc((rateLimitRules as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[apiRateLimiterDash] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(rateLimitRules)
        .where(eq((rateLimitRules as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`apiRateLimiterDash record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[apiRateLimiterDash] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(rateLimitRules);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(rateLimitRules)
        .where(gte((rateLimitRules as any).createdAt, since))
        .orderBy(desc((rateLimitRules as any).id))
        .limit(input.limit);
