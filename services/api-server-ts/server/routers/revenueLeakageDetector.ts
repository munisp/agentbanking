...
const runScan = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed revenueLeakageDetector mutation"
    );
...
function validateRevenueleakagedetectorInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "revenueLeakageDetector",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_REVENUELEAKAGEDETECTOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REVENUELEAKAGEDETECTOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REVENUELEAKAGEDETECTOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
