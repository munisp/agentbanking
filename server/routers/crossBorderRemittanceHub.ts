...
export const crossBorderRemittanceHubRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[crossBorderRemittanceHub] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`crossBorderRemittanceHub record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[crossBorderRemittanceHub] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
