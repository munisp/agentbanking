...
async function publishussdLocalizationMiddleware(
  action: string,
  ref: string,
...
export const ussdLocalizationRouter = router({
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await publishussdLocalizationMiddleware("update", `${Date.now()}`, {
        action: "update",
      }).catch(() => {});
  ...
  import: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Middleware fan-out (fail-open)
      await publishussdLocalizationMiddleware("import", `${Date.now()}`, {
        action: "import",
      }).catch(() => {});
