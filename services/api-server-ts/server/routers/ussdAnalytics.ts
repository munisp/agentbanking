...
function validateUssdanalyticsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_USSDANALYTICS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_USSDANALYTICS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_USSDANALYTICS.validateRange(data.amount, 0, 100_000_000)
    )
...
// ── Transaction Handling for ussdAnalytics ───────────────────────────────────────
