...
export const realtimeWebSocketFeedsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[realtimeWebSocketFeeds] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`realtimeWebSocketFeeds record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[realtimeWebSocketFeeds] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
