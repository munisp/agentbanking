...
- Automated sync with 54link transactions
"""
...
class ERPNextClient:
    """Full ERPNext REST API client for 54link integration."""
    ...
    def __init__(self):
        ...
        self.company = os.environ.get("ERPNEXT_COMPANY", "54link Agency Banking Ltd")
        self.timeout = 30.0
    ...
    def create_sales_invoice(self, agent_id: str, customer_name: str, items: List[Dict],
        ...
        """Create a sales invoice in ERPNext from a 54link transaction."""
        invoice_items = []
        for item in items:
...
class ERPSyncLog(Base):
    ...
    source_id = Column(String(100), nullable=False)  # 54link transaction/order ID
...
class ERPNextIntegrationService:
    ...
    def sync_transaction(self, req: SyncTransactionRequest) -> ERPSyncLog:
        """
        Sync a 54link transaction to ERPNext:
