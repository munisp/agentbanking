...
const assignTerritory = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed agentHierarchyTerritory mutation"
    );
...
function validateAgenthierarchyterritoryInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "agentHierarchyTerritory",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_AGENTHIERARCHYTERRITORY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTHIERARCHYTERRITORY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTHIERARCHYTERRITORY.validateRange(
        data.amount,
        0,
        100_000_000
      )
