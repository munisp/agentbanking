...
async def list_notifications_by_user(user_id: str, unread_only: bool = False,
    ...
    async with pool.acquire() as conn:
        ...
        unread = await conn.fetchval("SELECT COUNT(*) FROM notifications WHERE user_id=$1 AND read_at IS NULL", user_id)
        return {"notifications": [dict(r) for r in rows], "unread_count": unread}
