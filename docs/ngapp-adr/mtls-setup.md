...
```bash
# 1. Create a local CA
mkdir -p /etc/54agent/certs && cd /etc/54agent/certs

openssl genrsa -out ca.key 4096
openssl req -new -x509 -days 3650 -key ca.key -out ca.crt \
  -subj "/CN=54agent Dev CA/O=54agent/C=NG"

# 2. Generate POS Shell certificate
openssl genrsa -out tls.key 2048
openssl req -new -key tls.key -out tls.csr \
  -subj "/CN=pos-shell.svc.54agent.internal/O=54agent/C=NG"
openssl x509 -req -days 30 -in tls.csr -CA ca.crt -CAkey ca.key \
  -CAcreateserial -out tls.crt

# 3. Set environment variable
export MTLS_CERT_DIR=/etc/54agent/certs
```
...
| --------------- | ------------------- | --------------------------------------------- |
| `MTLS_CERT_DIR` | `/etc/54agent/certs` | Directory with `tls.crt`, `tls.key`, `ca.crt` |
