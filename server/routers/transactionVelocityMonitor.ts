...
export const transactionVelocityMonitorRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(velocityLimits)
          .orderBy(desc((velocityLimits as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[transactionVelocityMonitor] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(velocityLimits)
        .where(eq((velocityLimits as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`transactionVelocityMonitor record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionVelocityMonitor] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(velocityLimits);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(velocityLimits)
        .where(gte((velocityLimits as any).createdAt, since))
        .orderBy(desc((velocityLimits as any).id))
        .limit(input.limit);
