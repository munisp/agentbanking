...
const INTEGRITY_RULES_BILLINGINVOICE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_BILLINGINVOICE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_BILLINGINVOICE.validateRange(data.amount, 0, 100_000_000)
    )
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const invoice = await getStripe().invoices.create({
          ...
          description: `54agent billing for period ${input.periodStart} to ${input.periodEnd}`,
        });
          ...
          line_items: [
            {
              price_data: {
                currency: input.currency,
                product_data: {
                  name: `54agent Invoice: ${input.invoiceId}`,
