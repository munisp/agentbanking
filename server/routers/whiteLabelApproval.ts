...
function validateWhitelabelapprovalInput(
...
const INTEGRITY_RULES_WHITELABELAPPROVAL = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_WHITELABELAPPROVAL.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_WHITELABELAPPROVAL.validateRange(
        data.amount,
        0,
        100_000_000
      )
