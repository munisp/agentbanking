...
import { ArrowUp, ArrowDown, Minus, GitCompare, BarChart3 } from "lucide-react";
...
export default function ReportComparison() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    const params = new URLSearchParams({ reportAId, reportBId });
    fetch(`${CORE_BANKING_URL}/api/v1/report-comparison/compare?${params}`, { headers: authHeaders() })
      .then((r) => r.ok ? r.json() : null)
      .then((json) => setComparisonData(json))
      .catch(() => setComparisonData(null))
      .finally(() => setIsLoading(false));
  }, [reportAId, reportBId]);

  const comparison = { data: comparisonData, isLoading };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Report A (Baseline)</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Report B (Current)</CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Card className="border-blue-500/30">
                <CardContent className="pt-6 text-center">
                  <p className="text-sm text-muted-foreground">Report A Score</p>
                  <p className="text-4xl font-bold text-blue-400">{comparison.data.reportA.score}</p>
                  <p className="text-xs text-muted-foreground mt-1">{comparison.data.reportA.period.start} — {comparison.data.reportA.period.end}</p>
                </CardContent>
              </Card>
              <Card className="border-purple-500/30">
                <CardContent className="pt-6 text-center">
                  ...
                  <p className="text-2xl font-bold">
                    {comparison.data.reportB.score - comparison.data.reportA.score > 0 ? "+" : ""}
                    {comparison.data.reportB.score - comparison.data.reportA.score} pts
                  </p>
                </CardContent>
              </Card>
              <Card className="border-green-500/30">
                <CardContent className="pt-6 text-center">
                  <p className="text-sm text-muted-foreground">Report B Score</p>
                  <p className="text-4xl font-bold text-green-400">{comparison.data.reportB.score}</p>
                  <p className="text-xs text-muted-foreground mt-1">{comparison.data.reportB.period.start} — {comparison.data.reportB.period.end}</p>
                </CardContent>
              </Card>
            </div>
