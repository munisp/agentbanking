...
const createKey = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "apiKeyManagement",
      "mutation",
      "Executed apiKeyManagement mutation"
    );
...
function validateApikeymanagementInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_APIKEYMANAGEMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_APIKEYMANAGEMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_APIKEYMANAGEMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
