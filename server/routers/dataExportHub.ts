...
async function publishdataExportHubMiddleware(
  action: string,
  ref: string,
  ...
  cancelExport: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        await publishdataExportHubMiddleware("createExport", `${Date.now()}`, {
          action: "createExport",
        }).catch(() => {});

        // Middleware fan-out (fail-open)

        await publishdataExportHubMiddleware("cancelExport", `${Date.now()}`, {
          action: "cancelExport",
        }).catch(() => {});
