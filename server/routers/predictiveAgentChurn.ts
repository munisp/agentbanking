...
function validatePredictiveagentchurnInput(
...
const INTEGRITY_RULES_PREDICTIVEAGENTCHURN = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PREDICTIVEAGENTCHURN.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PREDICTIVEAGENTCHURN.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _predictiveAgentChurn_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "predictiveAgentChurn",
        "mutation",
        "Executed predictiveAgentChurn mutation"
      );
