"""
Router for qr-code-service service
...
The real implementation lives in the standalone service (services/qr-code-service/).
...
router = APIRouter(prefix="/qr-code-service", tags=["qr-code-service"])

_UNAVAILABLE = (
    "qr-code-service endpoints are not served by this gateway router. "
    "Use the standalone qr-code-service service."
)
...
async def health_check():
    return JSONResponse(
        status_code=503,
        content={"status": "unavailable", "service": "qr-code-service", "detail": _UNAVAILABLE},
    )
