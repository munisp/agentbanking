...
export const transactionGraphAnalyzerRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[transactionGraphAnalyzer] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`transactionGraphAnalyzer record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionGraphAnalyzer] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
