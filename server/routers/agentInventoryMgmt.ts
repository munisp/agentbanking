...
export const agentInventoryMgmtRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(inventoryItems)
          .orderBy(desc((inventoryItems as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[agentInventoryMgmt] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(inventoryItems)
        .where(eq((inventoryItems as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`agentInventoryMgmt record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentInventoryMgmt] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(inventoryItems);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(inventoryItems)
        .where(gte((inventoryItems as any).createdAt, since))
        .orderBy(desc((inventoryItems as any).id))
        .limit(input.limit);
