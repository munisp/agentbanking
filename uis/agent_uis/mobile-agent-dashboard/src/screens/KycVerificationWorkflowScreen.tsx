...
const KycVerificationWorkflowScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const { data } = await apiClient.get('/kyc/list?page=1&limit=50');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Kyc Verification Workflow</Text>
...
export default KycVerificationWorkflowScreen;
