...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "health-insurance-micro"),
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/health_insurance_micro")
...
pg_client = PostgresClient(DATABASE_URL, "health_analytics")
...
async def health_check():
    return {
        "status": "healthy",
        "service": "health-insurance-micro-analytics",
        "port": PORT,
...
async def analytics_summary():
    ...
    # Cache and publish
    cache.set("health-insurance-micro:summary", summary)
    await dapr.publish("health-insurance-micro.analytics.updated", summary)
    await fluvio.produce("health-insurance-micro-analytics", summary)
    await lakehouse.ingest("health-insurance-micro_analytics", summary)
...
async def forecast(request: ForecastRequest):
    ...
    await dapr.publish("health-insurance-micro.forecast.generated", result)
    return result
...
async def search_analytics(q: str = QueryParam(..., min_length=1)):
    ...
    results = await opensearch.search("health_policies", q)
    if results:
...
async def register_apisix():
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            route = {
                "uri": "/api/v1/health/insurance/micro/analytics/*",
                "upstream": {
                    "type": "roundrobin",
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"54Link Health Insurance Micro-Products Analytics starting on port {PORT}")
