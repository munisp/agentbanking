...
export const nlFinancialQueryRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[nlFinancialQuery] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`nlFinancialQuery record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[nlFinancialQuery] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
