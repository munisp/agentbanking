...
router.get("/weekly-reports", async (req, res) => {
  try {
    const db = await getDb();
    if (!db) return ok(res, { data: [], total: 0, limit: 20, offset: 0 });
    const limit = parseInt(String(req.query.limit ?? 20));
    const offset = parseInt(String(req.query.offset ?? 0));
    const results = await db.select().from(transactions).orderBy(desc(transactions.id)).limit(limit).offset(offset);
    const [{ total }] = await db.select({ total: count() }).from(transactions);
    ok(res, { data: results, total, limit, offset });
  } catch (e) { err(res, e); }
});

router.get("/weekly-reports/latest", async (_req, res) => {
  try {
    const db = await getDb();
    if (!db) return ok(res, { data: [], total: 0 });
    const [record] = await db.select().from(transactions).orderBy(desc(transactions.id)).limit(1);
    ok(res, record ?? null);
  } catch (e) { err(res, e); }
});

router.get("/weekly-reports/schedule", (_req, res) => ok(res, { data: [], total: 0 }));
router.get("/weekly-reports/email-config", (_req, res) => ok(res, { data: [], total: 0 }));
router.get("/weekly-reports/recipients", (_req, res) => res.json({ recipients: [] }));

router.get("/weekly-reports/:id/pdf-html", (_req, res) => ok(res, { data: [], total: 0 }));

router.get("/weekly-reports/:id", async (req, res) => {
  try {
    const db = await getDb();
    if (!db) return ok(res, null);
    const [record] = await db.select().from(transactions).where(eq(transactions.id, parseInt(req.params.id))).limit(1);
    if (!record) return res.status(404).json({ error: "Not found" });
    ok(res, record);
  } catch (e) { err(res, e); }
});

router.post("/weekly-reports/generate", (_req, res) => ok(res, { success: true }));
router.post("/weekly-reports/send-email", (_req, res) => ok(res, { success: true }));
router.post("/weekly-reports/schedule", (_req, res) => ok(res, { success: true }));
router.post("/weekly-reports/email-config", (_req, res) => ok(res, { success: true }));
router.post("/weekly-reports/recipients", (_req, res) => ok(res, { success: true }));
router.delete("/weekly-reports/recipients/:email", (_req, res) => ok(res, { success: true }));
