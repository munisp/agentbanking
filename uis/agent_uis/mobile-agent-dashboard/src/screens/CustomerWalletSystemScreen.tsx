...
const CustomerWalletSystemScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const { data } = await apiClient.get('/wallet/list?page=1&limit=50');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Customer Wallet System</Text>
...
export default CustomerWalletSystemScreen;
