...
from .service import (
    StoreMapService, StoreLocationCreate, NearbySearchRequest,
    StoreReviewCreate, StoreStatus, StoreType, ServiceType
)
...
@router.post("/stores")
def register_store(payload: StoreLocationCreate, svc: StoreMapService = Depends(get_svc)):
...
def find_nearby_stores(payload: NearbySearchRequest, svc: StoreMapService = Depends(get_svc)):
...
def add_review(store_id: str, payload: StoreReviewCreate, svc: StoreMapService = Depends(get_svc)):
