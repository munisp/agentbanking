...
class _MultiCurrencyScreenState extends State<MultiCurrencyScreen> {
  ...
  Future<void> _fetchRates() async {
    ...
    try {
      final data = await _api.getFxRates(baseCurrency: 'NGN');
      final ratesMap = <String, double>{};
      if (data['rates'] is Map) {
        (data['rates'] as Map).forEach((k, v) {
          ratesMap[k.toString()] = (v is num) ? v.toDouble() : double.tryParse(v.toString()) ?? 0;
        });
      }
      if (ratesMap.isEmpty) {
        ratesMap.addAll({
      ...
      setState(() { _rates = ratesMap; _loading = false; });
    } catch (e) {
      setState(() {
        _rates = {
          'NGN/USD': 0.000625, 'NGN/GBP': 0.000500, 'NGN/EUR': 0.000580,
  ...
  double _getRate(String from, String to) {
    ...
    return _rates['$from/$to'] ?? (1.0 / (_rates['$to/$from'] ?? 1.0));
  }

  List<MapEntry<String, double>> get _filteredRates => _rates.entries
      .where((e) => e.key.toLowerCase().contains(_search.toLowerCase()))
      .toList();

  Future<void> _lockRate() async {
    setState(() => _locking = true);
    try {
      final data = await _api.lockFxRate(
        fromCurrency: _from,
        toCurrency: _to,
        amount: double.tryParse(_amount) ?? 0,
      );
      setState(() {
        _lockId = data['lockId']?.toString();
        _locking = false;
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Rate locked: ${_lockId ?? "success"}')),
        );
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Lock failed: $e'), backgroundColor: Colors.red),
        );
      }
    }
  }
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _locking ? null : _lockRate,
                          child: Text(_locking ? 'Locking...' : 'Lock Rate for Transfer'),
                        ),
                      ),
                      if (_lockId != null)
                        Padding(
                          padding: const EdgeInsets.only(top: 8),
                          child: Text('Lock ID: $_lockId', style: TextStyle(color: Colors.green.shade700, fontSize: 12)),
                        ),
                    ]),
                  ),
                ),
