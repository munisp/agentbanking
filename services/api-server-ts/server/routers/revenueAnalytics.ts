...
function validateRevenueanalyticsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_REVENUEANALYTICS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REVENUEANALYTICS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REVENUEANALYTICS.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for revenueAnalytics ───────────────────────────────────────
