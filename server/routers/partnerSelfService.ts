...
function validatePartnerselfserviceInput(
...
const INTEGRITY_RULES_PARTNERSELFSERVICE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PARTNERSELFSERVICE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PARTNERSELFSERVICE.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  createApiKey: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "partnerSelfService",
        "mutation",
        "Executed partnerSelfService mutation"
      );
