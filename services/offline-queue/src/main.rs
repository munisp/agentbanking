/*!
 * offline-queue — 54agent Nigeria Offline Transaction Queue & USSD Encoder
 *
 * HTTP API (port 8032):
...
fn encode_ussd(req: &UssdEncodeRequest) -> UssdResponse {
    ...
    match req.tx_type.as_str() {
        ...
        "Bill Payment" => {
            let ussd = format!("*322*{}*54agent#", amount_str);
            UssdResponse {
                ussd_string: ussd.clone(),
...
async fn main() {
    ...
    let db_path = env::var("OFFLINE_QUEUE_DB").unwrap_or_else(|_| "/tmp/54agent-offline-queue.sqlite".to_string());
