...
function validateDataexportrouterInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_DATAEXPORTROUTER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DATAEXPORTROUTER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DATAEXPORTROUTER.validateRange(
        data.amount,
        0,
        100_000_000
      )
