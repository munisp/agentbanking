...
class OpenBankingScreen extends ConsumerStatefulWidget {
  const OpenBankingScreen({super.key});

  @override
  ConsumerState<OpenBankingScreen> createState() => _OpenBankingScreenState();
}

class _OpenBankingScreenState extends ConsumerState<OpenBankingScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/open_banking.getStats');
      final listResp = await api.get('/api/trpc/open_banking.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildApiKeyStatusIndicator(Map<String, dynamic> item) {
    final status = item['status'] ?? 'unknown';
    final colors = {'active': Colors.green, 'suspended': Colors.red, 'pending': Colors.orange, 'revoked': Colors.grey};
    return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: (colors[status] ?? Colors.grey).withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.circle, size: 8, color: colors[status] ?? Colors.grey), const SizedBox(width: 4), Text(status.toUpperCase(), style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: colors[status] ?? Colors.grey))]));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.api, size: 24), const SizedBox(width: 8), const Text('Open Banking API')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Open Banking API', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('Manage API partners, keys, and usage analytics', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('API Partners', '${_stats?['totalPartners'] ?? '\u2014'}', Icons.people, Colors.blue),
                              _buildStatCard('Active Keys', '${_stats?['activeKeys'] ?? '\u2014'}', Icons.vpn_key, Colors.green),
                              _buildStatCard('Today\'s Requests', '${_stats?['requestsToday'] ?? '\u2014'}', Icons.trending_up, Colors.orange),
                              _buildStatCard('Monthly Revenue', '₦${_stats?['revenueThisMonth'] ?? '\u2014'}', Icons.attach_money, Colors.purple),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['partnerName'] ?? item['callbackUrl'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildApiKeyStatusIndicator(item),
                    ]),
