...
export const merchantSettlementDashboardRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((merchantSettlements as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[merchantSettlementDashboard] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(merchantSettlements)
        .where(eq((merchantSettlements as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`merchantSettlementDashboard record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[merchantSettlementDashboard] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(merchantSettlements);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(merchantSettlements)
        .where(gte((merchantSettlements as any).createdAt, since))
        .orderBy(desc((merchantSettlements as any).id))
        .limit(input.limit);
