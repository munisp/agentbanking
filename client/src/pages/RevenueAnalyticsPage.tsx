...
export default function RevenueAnalyticsPage() {
  ...
  const listData = (d?.revenueBreakdown ?? d?.recent ?? []) as Record<string, unknown>[];
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Revenue tracking, trend analysis, and financial performance dashboards</p>
          </div>
          <div className="flex gap-2 flex-wrap">
          <Button onClick={() => toast.success("Generate Report initiated")}>Generate Report</Button>
          <Button variant="outline" onClick={() => toast.success("Export CSV initiated")}>Export CSV</Button>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.totalRevenue ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.monthlyGrowth ?? 0) + "%"}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Avg Per Agent</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.avgRevenuePerAgent ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.topRevenueSource ?? 0)}</div>
            </CardContent>
          </Card>
            </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <td className="p-3 font-mono">{new Intl.NumberFormat('en-NG', { style: 'currency', currency: 'NGN', maximumFractionDigits: 0 }).format(Number(r.amount ?? 0))}</td>
