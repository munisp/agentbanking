...
export default function KycWorkflow() {
  ...
  const applications = {data: null, isLoading: false, refetch: () => {}};
  // Note: approve/reject are handled by verifyDocument + status update
  const verifyDoc = {
    mutate: () => { toast.success("Document verified"); applications.refetch(); },
    mutateAsync: async () => { toast.success("Document verified"); applications.refetch(); },
