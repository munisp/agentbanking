...
const submitSurvey = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "customerSurveys",
      "mutation",
      "Executed customerSurveys mutation"
    );
...
function validateCustomersurveysInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_CUSTOMERSURVEYS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CUSTOMERSURVEYS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CUSTOMERSURVEYS.validateRange(
        data.amount,
        0,
        100_000_000
      )
