...
function validateOpenbankingapiInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_OPENBANKINGAPI = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_OPENBANKINGAPI.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_OPENBANKINGAPI.validateRange(data.amount, 0, 100_000_000)
    )
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "openBankingApi",
        "mutation",
        "Executed openBankingApi mutation"
      );
