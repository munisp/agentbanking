...
export const taxCollectionRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(vatRecords)
          .orderBy(desc((vatRecords as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[taxCollection] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(vatRecords)
        .where(eq((vatRecords as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`taxCollection record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[taxCollection] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(vatRecords);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(vatRecords)
        .where(gte((vatRecords as any).createdAt, since))
        .orderBy(desc((vatRecords as any).id))
        .limit(input.limit);
