...
export default function LiveChatSupport({ onBack }: { onBack?: () => void }) {
  ...
  const [queuePos] = useState(crypto.getRandomValues(new Uint32Array(1))[0] % 3 + 1);
  ...
  const simulateResponse = useCallback((category: SupportCategory) => {
    setIsTyping(true);
    const delay = 1500 + crypto.getRandomValues(new Uint32Array(1))[0] / 4294967295 * 2000;
    setTimeout(() => {
      setIsTyping(false);
      const pool = BOT_RESPONSES[category] || BOT_RESPONSES.default;
      const text = pool[Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / 4294967295 * pool.length)];
      const msg: Message = {
