...
function validateReferralprogramInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_REFERRALPROGRAM = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REFERRALPROGRAM.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REFERRALPROGRAM.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Audit Metadata ─────────────────────────────────────────────────────────
const _referralProgramAuditMeta = {
  createdAt: () => new Date().toISOString(),
  updatedAt: () => new Date().toISOString(),
  auditTimestamp: () => Date.now(),
  auditSource: "referralProgram",
};
