...
export const emailNotificationsRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(emailQueue)
          .orderBy(desc((emailQueue as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[emailNotifications] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(emailQueue)
        .where(eq((emailQueue as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`emailNotifications record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[emailNotifications] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(emailQueue);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(emailQueue)
        .where(gte((emailQueue as any).createdAt, since))
        .orderBy(desc((emailQueue as any).id))
        .limit(input.limit);
