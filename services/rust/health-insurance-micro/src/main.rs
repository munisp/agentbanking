//! 54Link Health Insurance Micro-Products — Rust Microservice
//!
//! Premium calculation, risk pooling, claims adjudication engine
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/health/premium/calculate — Calculate premium based on risk factors
//!   POST /api/v1/health/claims/adjudicate — Auto-adjudicate claim
//!   POST /api/v1/health/risk/pool — Calculate risk pool metrics
//!   GET  /api/v1/health/risk/exposure — Current risk exposure
//!
//! Port: 8255
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8255),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "health-insurance-micro"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "health-insurance-micro".into(),
...
async fn create_record(
    ...
    dapr.publish("health.policy.created", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("health-insurance-micro-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("health_policies", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("health-insurance-micro:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("insurance_claims", &payload).await;

    // Persist to PostgreSQL
    let _ = state.postgres.persist("health_insurance_micro", &serde_json::json!({
        "service": "health-insurance-micro",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("health-insurance-micro:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("health_policies", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Health Insurance Micro-Products (Rust) starting on port {}", port);
