...
class _SlaMonitoringScreenState extends State<SlaMonitoringScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('SLA Monitoring'),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              title: Text(item['service']?.toString() ?? 'N/A'),
              subtitle: Text(item['target']?.toString() ?? ''),
              trailing: Text(item['actual']?.toString() ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
            ),
          );
        },
      ),
