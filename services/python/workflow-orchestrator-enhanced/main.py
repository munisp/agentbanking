import httpx
"""
Enhanced Workflow Orchestrator - FastAPI microservice
Advanced workflow engine with conditional branching, parallel execution, and SLA monitoring
"""
import os
import logging
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "workflow-orchestrator-enhanced"),
...
app = FastAPI(title="Enhanced Workflow Orchestrator", description="Advanced workflow engine with conditional branching, parallel execution, and SLA monitoring", version="1.0.0")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/workflow_orchestrator_enhanced")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "workflow-orchestrator-enhanced", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
...
async def startup_event():
    ...
    await publish_kafka("workflow.orchestrator.enhanced.started", {
        "service": "workflow-orchestrator-enhanced",
