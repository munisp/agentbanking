...
// ── Constants ─────────────────────────────────────────────────────────────────
const AUTH_TOKEN_KEY = '@54agent:authToken';
const REFRESH_TOKEN_KEY = '@54agent:refreshToken';
const USER_ID_KEY = '@54agent:userId';
const apiClient = new APIClient();

// ── Real API helpers ──────────────────────────────────────────────────────────

/**
 * Verify biometric signature against the 54agent backend.
...
/**
 * Register biometric public key with the 54agent backend.
...
const BiometricAuthScreen: React.FC<BiometricAuthScreenProps> = () => {
  ...
  const handleBiometricAuth = useCallback(async () => {
    ...
    try {
      ...
      const payload = `${epochSeconds}:${userId}:54agent-biometric`;
