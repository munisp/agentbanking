...
export const executiveCommandCenterRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[executiveCommandCenter] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`executiveCommandCenter record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[executiveCommandCenter] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
