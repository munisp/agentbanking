...
const setAlertThreshold = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed gatewayHealthMonitor mutation"
    );
...
function validateGatewayhealthmonitorInput(
...
const INTEGRITY_RULES_GATEWAYHEALTHMONITOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_GATEWAYHEALTHMONITOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_GATEWAYHEALTHMONITOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
