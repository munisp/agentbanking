...
function validateTransactioncsvexportInput(
...
const INTEGRITY_RULES_TRANSACTIONCSVEXPORT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_TRANSACTIONCSVEXPORT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_TRANSACTIONCSVEXPORT.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _transactionCsvExport_db = {
...
// ── Transaction Handling for transactionCsvExport ───────────────────────────────────────
