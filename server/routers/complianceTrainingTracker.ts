...
export const complianceTrainingTrackerRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((trainingEnrollments as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[complianceTrainingTracker] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(trainingEnrollments)
        .where(eq((trainingEnrollments as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`complianceTrainingTracker record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[complianceTrainingTracker] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(trainingEnrollments);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(trainingEnrollments)
        .where(gte((trainingEnrollments as any).createdAt, since))
        .orderBy(desc((trainingEnrollments as any).id))
        .limit(input.limit);
