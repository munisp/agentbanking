...
export const aiCashFlowPredictorRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[aiCashFlowPredictor] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`aiCashFlowPredictor record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[aiCashFlowPredictor] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
