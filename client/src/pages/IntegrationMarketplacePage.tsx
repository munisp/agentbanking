...
export default function IntegrationMarketplacePage() {
  ...
  const { data, isLoading } = trpc.integrationMarketplace.dashboard.useQuery(undefined as any);
  const d = data as Record<string, unknown> | undefined;
  const listData = (d?.integrations ?? d?.recent ?? []) as Record<string, unknown>[];
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Third-party integration catalog, API connectors, and plugin management</p>
          </div>
          <div className="flex gap-2 flex-wrap">
          <Button onClick={() => toast.success("Browse Marketplace initiated")}>Browse Marketplace</Button>
          <Button variant="outline" onClick={() => toast.success("Add Custom initiated")}>Add Custom</Button>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.totalIntegrations ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Active Connectors</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.activeConnectors ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.pendingSetup ?? 0)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{String(d?.apiCallsToday ?? 0)}</div>
            </CardContent>
          </Card>
            </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <th className="text-left p-3 text-sm font-medium text-muted-foreground">Integration</th>
                ...
                <td className="p-3 text-sm text-muted-foreground">{r.lastSync ? new Date(String(r.lastSync)).toLocaleDateString() : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
