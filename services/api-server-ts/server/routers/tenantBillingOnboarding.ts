...
async function getTemporalClient(): Promise<Client | null> {
  ...
  try {
    ...
    temporalClient = new Client({
      connection,
      namespace: process.env.TEMPORAL_NAMESPACE || "54agent",
    });
...
export const BILLING_TEMPLATES = {
  revenue_share: {
    name: "Revenue Share",
    description:
      "54agent takes a percentage of each transaction. Best for high-volume partners.",
    billingModel: "revenue_share" as const,
    revenueShareConfig: {
      startSplitPct: 70, // Client gets 70%, 54agent gets 30%
  ...
  hybrid: {
    ...
    subscriptionConfig: null,
    hybridConfig: {
      reducedSharePct: 15, // 54agent takes only 15% of tx revenue
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "tenantBillingOnboarding",
    action,
    ...details,
  };
  auditFinancialAction(
