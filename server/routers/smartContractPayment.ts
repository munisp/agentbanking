...
function validateSmartcontractpaymentInput(
...
const INTEGRITY_RULES_SMARTCONTRACTPAYMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SMARTCONTRACTPAYMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SMARTCONTRACTPAYMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _smartContractPayment_db = {
