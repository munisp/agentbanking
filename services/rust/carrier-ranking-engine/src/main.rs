...
// Persistence: audit log + state store for carrier-ranking-engine
...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "carrier-ranking-engine".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("carrier-ranking-engine").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "carrier-ranking-engine").await {
            eprintln!("[carrier-ranking-engine] Loaded persisted state from PostgreSQL");
