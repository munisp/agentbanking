...
export const fraudCaseManagementRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(fraudAlerts)
          .orderBy(desc((fraudAlerts as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[fraudCaseManagement] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(fraudAlerts)
        .where(eq((fraudAlerts as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`fraudCaseManagement record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[fraudCaseManagement] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(fraudAlerts)
        .where(gte((fraudAlerts as any).createdAt, since))
        .orderBy(desc((fraudAlerts as any).id))
        .limit(input.limit);
