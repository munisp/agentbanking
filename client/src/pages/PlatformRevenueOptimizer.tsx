...
export default function PlatformRevenueOptimizer() {
  ...
  const summary = trpc.platformRevenueOptimizer.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.platformRevenueOptimizer.list.useQuery({ limit: 20 });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Revenue optimization strategies, pricing experiments, and yield management</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("New Strategy initiated")}>New Strategy</Button>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(summary?.currentRevenue)}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Optimized Revenue</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(summary?.optimizedRevenue)}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.uplift ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.activeStrategies ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Strategy ID</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Strategy</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3 font-mono">{formatCurrency(row.projectedImpact)}</td>
