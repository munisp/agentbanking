...
function validateResiliencehardeningInput(
...
const INTEGRITY_RULES_RESILIENCEHARDENING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_RESILIENCEHARDENING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_RESILIENCEHARDENING.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for resilienceHardening ───────────────────────────────────────
