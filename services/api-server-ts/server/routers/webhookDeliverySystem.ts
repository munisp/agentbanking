...
const createEndpoint = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed webhookDeliverySystem mutation"
    );
...
function validateWebhookdeliverysystemInput(
...
const INTEGRITY_RULES_WEBHOOKDELIVERYSYSTEM = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_WEBHOOKDELIVERYSYSTEM.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_WEBHOOKDELIVERYSYSTEM.validateRange(
        data.amount,
        0,
        100_000_000
      )
