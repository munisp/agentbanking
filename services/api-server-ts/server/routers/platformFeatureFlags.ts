...
function validatePlatformfeatureflagsInput(
...
const INTEGRITY_RULES_PLATFORMFEATUREFLAGS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PLATFORMFEATUREFLAGS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PLATFORMFEATUREFLAGS.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _platformFeatureFlags_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "platformFeatureFlags",
        "mutation",
        "Executed platformFeatureFlags mutation"
      );
