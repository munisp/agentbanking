...
const ApiKeyBadge = ({ item }: { item: RecordItem }) => {
    const colors: Record<string, string> = { active: '#22c55e', suspended: '#ef4444', pending: '#f59e0b', revoked: '#6b7280' };
    return (<View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: (colors[item.status || ''] || '#6b7280') + '20', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 12 }}>
      <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: colors[item.status || ''] || '#6b7280', marginRight: 4 }} />
      <Text style={{ fontSize: 10, fontWeight: '700', color: colors[item.status || ''] || '#6b7280' }}>{(item.status || 'unknown').toUpperCase()}</Text>
    </View>);
  };
...
export default function OpenBankingScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/open_banking.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/open_banking.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Open Banking API...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Open Banking API</Text>
            <Text style={styles.subtitle}>API partners, keys & usage analytics</Text>
          </View>
          <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🤝</Text>
            <Text style={styles.statLabel}>API Partners</Text>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{stats?.requestsToday ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>💰</Text>
            <Text style={styles.statLabel}>Monthly Revenue</Text>
            <Text style={styles.statValue}>₦{stats?.revenueThisMonth ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.partnerName || `Record #${item.id}`}</Text>
            <View style={[styles.statusBadge, { backgroundColor: getStatusColor(item.status) + '20' }]}>
              <Text style={[styles.statusText, { color: getStatusColor(item.status) }]}>{(item.status || '—').toUpperCase()}</Text>
            </View>
          </View>
          <View style={styles.cardBody}>
            <ApiKeyBadge item={item} />
          </View>
        </View>
      )}
