...
const TierBadge = ({ item }: { item: RecordItem }) => {
    const points = Number(item.points_balance || 0);
    let tier = 'BRONZE'; let color = '#92400e';
    if (points >= 10000) { tier = 'PLATINUM'; color = '#607D8B'; }
    else if (points >= 5000) { tier = 'GOLD'; color = '#f59e0b'; }
    else if (points >= 1000) { tier = 'SILVER'; color = '#9ca3af'; }
    return (<View style={{ backgroundColor: color + '20', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 }}>
      <Text style={{ fontSize: 10, fontWeight: '700', color }}>{tier}</Text></View>);
  };
...
export default function LoyaltyProgramScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/loyalty_program.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/loyalty_program.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Coalition Loyalty...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Coalition Loyalty</Text>
            <Text style={styles.subtitle}>{tenantConfig?.name ?? '54agent'} Points — earn & redeem</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{stats?.pointsCirculating ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🎁</Text>
            <Text style={styles.statLabel}>Redemption Rate</Text>
            <Text style={styles.statValue}>{stats?.redemptionRate ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🤝</Text>
            <Text style={styles.statLabel}>Partners</Text>
            <Text style={styles.statValue}>{stats?.coalitionPartners ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.customerName || `Record #${item.id}`}</Text>
