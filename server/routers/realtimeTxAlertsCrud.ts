...
function validateRealtimetxalertscrudInput(
...
const INTEGRITY_RULES_REALTIMETXALERTSCRUD = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REALTIMETXALERTSCRUD.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REALTIMETXALERTSCRUD.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _realtimeTxAlertsCrud_db = {
  ...
  evaluateTransaction: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "realtimeTxAlertsCrud",
        "mutation",
        "Executed realtimeTxAlertsCrud mutation"
      );
