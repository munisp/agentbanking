...
const INTEGRITY_RULES_DIGITALIDENTITYLAYER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DIGITALIDENTITYLAYER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DIGITALIDENTITYLAYER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _digitalIdentityLayer_db = {
