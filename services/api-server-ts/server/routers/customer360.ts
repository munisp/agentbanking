...
function validateCustomer360Input(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_CUSTOMER360 = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CUSTOMER360.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (!INTEGRITY_RULES_CUSTOMER360.validateRange(data.amount, 0, 100_000_000))
...
// ── Transaction Handling for customer360 ───────────────────────────────────────
