...
export const apiVersioningRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(apiKeys)
          .orderBy(desc((apiKeys as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[apiVersioning] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(apiKeys)
        .where(eq((apiKeys as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`apiVersioning record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[apiVersioning] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(apiKeys);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(apiKeys)
        .where(gte((apiKeys as any).createdAt, since))
        .orderBy(desc((apiKeys as any).id))
        .limit(input.limit);
