...
export const dbSchemaMigrationManagerRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[dbSchemaMigrationManager] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`dbSchemaMigrationManager record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[dbSchemaMigrationManager] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
