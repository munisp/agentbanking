...
export default function RevenueForecastingEngine() {
  ...
  const summary = trpc.revenueForecastingEngine.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.revenueForecastingEngine.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Revenue projections, scenario modeling, and budget vs actual tracking</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Run Forecast initiated")}>Run Forecast</Button>
        <Button variant="outline" onClick={() => toast.success("Create Scenario initiated")}>Create Scenario</Button>
          </div>
        </div>
        ...
        <Card key="1">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(summary?.actualRevenue)}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
        ...
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Growth Projection</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.growthProjection ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Projected</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Actual</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Confidence</th>
              ...
              <td className="p-3">{String(row.month ?? '—')}</td>
              <td className="p-3 font-mono">{formatCurrency(row.projected)}</td>
              <td className="p-3 font-mono">{formatCurrency(row.actual)}</td>
