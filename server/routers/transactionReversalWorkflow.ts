...
export const transactionReversalWorkflowRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((reversalRequests as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[transactionReversalWorkflow] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(reversalRequests)
        .where(eq((reversalRequests as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`transactionReversalWorkflow record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionReversalWorkflow] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(reversalRequests);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(reversalRequests)
        .where(gte((reversalRequests as any).createdAt, since))
        .orderBy(desc((reversalRequests as any).id))
        .limit(input.limit);
