...
function validateFloatreconciliationInput(
...
const INTEGRITY_RULES_FLOATRECONCILIATION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_FLOATRECONCILIATION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_FLOATRECONCILIATION.validateRange(
        data.amount,
        0,
        100_000_000
      )
