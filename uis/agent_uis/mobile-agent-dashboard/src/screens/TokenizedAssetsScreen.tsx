...
const TokenBar = ({ item }: { item: RecordItem }) => {
    const sold = Number(item.tokensSold || 0); const total = Number(item.totalTokens || 100);
    const pct = total > 0 ? (sold / total * 100) : 0;
    return (<View><Text style={{ fontSize: 10, color: '#9ca3af' }}>{pct.toFixed(0)}% sold ({sold}/{total})</Text>
      <View style={{ height: 4, backgroundColor: '#e5e7eb', borderRadius: 2, marginTop: 2 }}>
        <View style={{ height: 4, width: Math.min(100, pct) + '%', backgroundColor: pct >= 100 ? '#22c55e' : '#3b82f6', borderRadius: 2 }} /></View></View>);
  };
...
export default function TokenizedAssetsScreen() {
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Tokenized Assets...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Fractional ownership marketplace</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>{stats?.totalHolders ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📈</Text>
            <Text style={styles.statLabel}>Market Cap</Text>
            <Text style={styles.statValue}>₦{stats?.marketCap ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>💰</Text>
            <Text style={styles.statLabel}>Dividends</Text>
            <Text style={styles.statValue}>₦{stats?.dividendsPaid ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.assetName || `Record #${item.id}`}</Text>
