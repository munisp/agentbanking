...
const INTEGRITY_RULES_REPORTTEMPLATEDESIGNER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_REPORTTEMPLATEDESIGNER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_REPORTTEMPLATEDESIGNER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _reportTemplateDesigner_db = {
  ...
  createTemplate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "reportTemplateDesigner",
        "mutation",
        "Executed reportTemplateDesigner mutation"
      );
