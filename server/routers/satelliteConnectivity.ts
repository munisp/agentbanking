...
const INTEGRITY_RULES_SATELLITECONNECTIVITY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SATELLITECONNECTIVITY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SATELLITECONNECTIVITY.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _satelliteConnectivity_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "satelliteConnectivity",
        "mutation",
        "Executed satelliteConnectivity mutation"
      );
