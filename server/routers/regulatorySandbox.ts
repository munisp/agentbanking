...
async function publishregulatorySandboxMiddleware(
  action: string,
  ref: string,
...
export const regulatorySandboxRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await publishregulatorySandboxMiddleware("create", `${Date.now()}`, {
        action: "create",
      }).catch(() => {});
  ...
  execute: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Middleware fan-out (fail-open)
      await publishregulatorySandboxMiddleware("execute", `${Date.now()}`, {
        action: "execute",
      }).catch(() => {});
