...
export default function CustomerWalletSystem() {
  ...
  const summary = trpc.customerWalletSystem.getStats.useQuery()?.data as Record<string, unknown> | undefined;
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Customer Wallet System</h1>
            <p className="text-muted-foreground">Digital wallet management, balance tracking, and wallet-to-wallet transfers</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Create Wallet initiated")}>Create Wallet</Button>
        <Button variant="outline" onClick={() => toast.success("Top Up initiated")}>Top Up</Button>
        <Button variant="outline" onClick={() => toast.success("Freeze initiated")}>Freeze</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalWallets ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
        ...
        <Card key="2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.activeWallets ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.transfersToday ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Wallet ID</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3 font-mono">{formatCurrency(row.balance)}</td>
              <td className="p-3">{String(row.tier ?? '—')}</td>
              <td className="p-3 text-sm text-muted-foreground">{row.lastActivity ? new Date(String(row.lastActivity)).toLocaleDateString() : '—'}</td>
