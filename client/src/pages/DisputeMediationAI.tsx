...
export default function DisputeMediationAI() {
  ...
  const summary = trpc.disputeMediationAI.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.disputeMediationAI.listMediations.useQuery({ limit: 20, offset: page * 20 });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">AI-powered dispute resolution, automated mediation, and fairness scoring</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Submit Case initiated")}>Submit Case</Button>
        <Button variant="outline" onClick={() => toast.success("Override AI initiated")}>Override AI</Button>
          </div>
        </div>
        ...
        <Card key="0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">AI Cases</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalCases ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.autoResolved ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.avgResolutionTime ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.fairnessScore ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Dispute Type</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Amount</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">AI Recommendation</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.aiRecommendation ?? '—')}</td>
