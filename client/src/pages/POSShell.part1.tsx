...
import { AmountDisplay } from "./POSShell.part11";
import { SuccessScreen } from "./POSShell.part5";
import { ReceiptModal, fmt } from "./POSShell.part6";
import { BORDER, CARD, DISP, GOLD, GREEN, MONO, QR_TTL_MS, RED } from "./POSShell.shared";

export function QRPaymentScreen({ onBack }: { onBack: () => void }) {
                  ...
                  <button
                    onClick={() => {
                      ...
                      const rows = activeQRs
                        ...
                        .join("");
                      const _agent = usePosStore.getState().agent;
                      const _agentName = _agent?.name ?? "—";
                      const _agentCode = _agent?.agentCode ?? "—";
                      const _serialNo = _agent?.terminalSerial ?? "—";
