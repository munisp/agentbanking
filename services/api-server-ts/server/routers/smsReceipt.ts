...
function buildReceiptSMS(data: {
  ...
  lines.push(`Powered by 54agent Agency Banking`);
  return lines.join("\n");
}
...
const INTEGRITY_RULES_SMSRECEIPT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SMSRECEIPT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (!INTEGRITY_RULES_SMSRECEIPT.validateRange(data.amount, 0, 100_000_000))
  ...
  sendUssd: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const lines = [`54agent USSD Receipt`, `Dial: ${input.ussdCode}`];
