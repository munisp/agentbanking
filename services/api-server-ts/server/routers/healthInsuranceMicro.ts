...
function validateHealthinsurancemicroInput(
...
const INTEGRITY_RULES_HEALTHINSURANCEMICRO = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_HEALTHINSURANCEMICRO.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_HEALTHINSURANCEMICRO.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _healthInsuranceMicro_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "healthInsuranceMicro",
        "mutation",
        "Executed healthInsuranceMicro mutation"
      );
