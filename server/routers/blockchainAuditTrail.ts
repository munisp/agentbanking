...
function validateBlockchainaudittrailInput(
...
const INTEGRITY_RULES_BLOCKCHAINAUDITTRAIL = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_BLOCKCHAINAUDITTRAIL.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_BLOCKCHAINAUDITTRAIL.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _blockchainAuditTrail_db = {
...
// ── Transaction Handling for blockchainAuditTrail ───────────────────────────────────────
