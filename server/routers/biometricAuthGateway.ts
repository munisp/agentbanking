...
import { auditLog, faceEnrollments } from "../../drizzle/schema";
...
function validateBiometricauthgatewayInput(
...
const INTEGRITY_RULES_BIOMETRICAUTHGATEWAY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_BIOMETRICAUTHGATEWAY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_BIOMETRICAUTHGATEWAY.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _biometricAuthGateway_db = {
...
// ── Transaction Handling for biometricAuthGateway ───────────────────────────────────────
