...
class _PayrollScreenState extends ConsumerState<PayrollScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/payroll.getStats');
      final listResp = await api.get('/api/trpc/payroll.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildDisbursementProgress(Map<String, dynamic> item) {
    final disbursed = int.tryParse('${item[\'disbursed\'] ?? 0}') ?? 0;
    final total = int.tryParse('${item[\'employeeCount\'] ?? 1}') ?? 1;
    final pct = total > 0 ? (disbursed / total * 100).toStringAsFixed(0) : '0';
    return Text('$pct% disbursed', style: TextStyle(fontSize: 12, color: disbursed >= total ? Colors.green : Colors.orange));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.account_balance, size: 24), const SizedBox(width: 8), const Text('Payroll Disbursement')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Payroll Disbursement', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('SME payroll through agent network', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Employers', '${_stats?['totalEmployers'] ?? '\u2014'}', Icons.business, Colors.blue),
                              _buildStatCard('Employees', '${_stats?['totalEmployees'] ?? '\u2014'}', Icons.people, Colors.green),
                              _buildStatCard('Monthly Disbursed', '₦${_stats?['monthlyDisbursed'] ?? '\u2014'}', Icons.payments, Colors.orange),
                              _buildStatCard('Pending Cash-Out', '${_stats?['pendingCashOut'] ?? '\u2014'}', Icons.pending_actions, Colors.red),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['employerName'] ?? item['employeeCount'] ?? item['totalAmount'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildDisbursementProgress(item),
                    ]),
