...
function validateAgentloanoriginationInput(
...
const INTEGRITY_RULES_AGENTLOANORIGINATION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTLOANORIGINATION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTLOANORIGINATION.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _agentLoanOrigination_db = {
...
// ── Transaction Handling for agentLoanOrigination ───────────────────────────────────────
