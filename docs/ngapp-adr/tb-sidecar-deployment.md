...
The **54agent TB Sidecar** is a Go process that runs on every POS terminal alongside the Node.js server. It provides:
...
1. Creates the `54agent` system user and `/var/lib/54agent/tb-data`, `/var/log/54agent`, `/etc/54agent` directories
2. Downloads and installs TigerBeetle v0.16.78 to `/usr/local/bin/tigerbeetle`
3. Installs the sidecar binary to `/usr/local/bin/54agent-tb-sidecar`
4. Creates the environment file at `/etc/54agent/sidecar.env`
5. Registers and starts the `54agent-tb-sidecar` systemd service

---

## Configuration

Edit `/etc/54agent/sidecar.env` after installation:

```bash
# PostgreSQL connection string for metadata sync
POSTGRES_URL=postgresql://posadmin:pos54agent2026@db.54agent.internal:5432/pos54agent
...
systemctl status 54agent-tb-sidecar

# View live logs
journalctl -u 54agent-tb-sidecar -f

# Restart after config change
sudo systemctl restart 54agent-tb-sidecar

# Stop the service
sudo systemctl stop 54agent-tb-sidecar
```
...
2. 54agent-tb-sidecar  (systemd: After=network.target)
3. 54agent-pos-shell   (Node.js server, reads TB_SIDECAR_URL)
```
...
| Symptom                                        | Cause                                    | Fix                                                                                    |
| ---------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------------------- |
| `tbSidecar: "offline"` in `/api/health`        | Sidecar not running or wrong port        | `systemctl status 54agent-tb-sidecar`; check `SIDECAR_PORT`                             |
| `sync/status` shows `postgres: "disconnected"` | PG unreachable from terminal             | Check `POSTGRES_URL` in `/etc/54agent/sidecar.env`                                      |
| `sync/status` shows `failed > 0`               | Transfers failed to sync after 3 retries | `journalctl -u 54agent-tb-sidecar -n 100` for error details                             |
| TigerBeetle binary not found                   | Installer skipped step 2                 | `sudo bash tb-sidecar/scripts/install-sidecar.sh` again                                |
| Port 7070 already in use                       | Another process on the port              | Change `SIDECAR_PORT` in `/etc/54agent/sidecar.env` and `TB_SIDECAR_URL` in Node.js env |
