...
// ─── SSL config (SEC-19) ──────────────────────────────────────────────────────
// POSTGRES_SSL="require" means ENCRYPT *AND VERIFY* (rejectUnauthorized: true).
// TLS certificate verification may only be disabled via the explicit opt-in
// POSTGRES_SSL_INSECURE=true (e.g. a local dev database with a self-signed
// certificate). Combining it with production is a startup-fatal
// misconfiguration — previously "require" silently disabled verification.
function getSslConfig(): boolean | { rejectUnauthorized: boolean } {
  const insecureSsl = process.env.POSTGRES_SSL_INSECURE === "true";
  if (
    insecureSsl &&
    (process.env.NODE_ENV === "production" || process.env.ENVIRONMENT === "production")
  ) {
    throw new Error(
      "FATAL: POSTGRES_SSL_INSECURE=true is not allowed in production — database TLS certificate verification must stay enabled."
    );
  }
