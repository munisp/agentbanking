...
export const intelligentRoutingEngineRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[intelligentRoutingEngine] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`intelligentRoutingEngine record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[intelligentRoutingEngine] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
