...
class PosSettlementScreen extends StatefulWidget {
  const PosSettlementScreen({super.key});
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              title: Text(item['batchRef']?.toString() ?? 'N/A'),
              subtitle: Text(item['totalAmount']?.toString() ?? ''),
              trailing: Text(item['netAmount']?.toString() ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
            ),
          );
        },
      ),
