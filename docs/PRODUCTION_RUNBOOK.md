...
|---------|----------|------|-----------------|
| 54Link API | TypeScript | 3001 | `GET /api/health` |
...
```bash
# Recent error logs
docker logs --tail 500 54link-api 2>&1 | grep -i error
...
curl -s http://localhost:8080/auth/realms/54link/.well-known/openid-configuration | jq .issuer
...
kafka-consumer-groups.sh --bootstrap-server localhost:9092 \
  --describe --group 54link-kyc-consumer

# Reset consumer offset (CAUTION: may cause reprocessing)
kafka-consumer-groups.sh --bootstrap-server localhost:9092 \
  --group 54link-kyc-consumer \
  --topic kyc-events \
...
```bash
# 1. Build and tag
docker build -t 54link/api:$(git rev-parse --short HEAD) .
docker push 54link/api:$(git rev-parse --short HEAD)

# 2. Deploy with Helm (blue-green)
helm upgrade 54link ./helm/54link \
  --set image.tag=$(git rev-parse --short HEAD) \
  --set deployment.strategy=blue-green \
  --namespace production

# 3. Verify health
kubectl rollout status deployment/54link-api -n production
...
helm rollback 54link 0 --namespace production

# Or revert to specific revision
helm rollback 54link <REVISION> --namespace production
```
...
### Dashboards
- **Platform Overview**: Grafana → 54Link Platform Overview
- **Transaction Metrics**: Grafana → 54Link Transactions
- **Service Health**: Grafana → 54Link Service Health
