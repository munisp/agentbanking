...
async function publishsupplyChainMiddleware(
  action: string,
  ref: string,
  payload: Record<string, unknown>,
) {
  const topic = `supply-chain.${action}` as any;
  ...
  ingestToLakehouse("supply-chain", { ref, action, ...payload, timestamp: ts }).catch(() => {});
}
