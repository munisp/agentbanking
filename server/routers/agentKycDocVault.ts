...
const INTEGRITY_RULES_AGENTKYCDOCVAULT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTKYCDOCVAULT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTKYCDOCVAULT.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  uploadDocument: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "agentKycDocVault",
        "mutation",
        "Executed agentKycDocVault mutation"
      );
