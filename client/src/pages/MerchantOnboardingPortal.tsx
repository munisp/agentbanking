...
export default function MerchantOnboardingPortal() {
  ...
  const summary = trpc.merchantOnboardingPortal.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.merchantOnboardingPortal.listApplications.useQuery({ limit: 20 });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Merchant registration, KYB verification, and activation workflow</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("New Application initiated")}>New Application</Button>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalApplications ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.pendingReview ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.approved ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.rejected ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">App ID</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.businessName ?? '—')}</td>
              <td className="p-3">{String(row.category ?? '—')}</td>
              <td className="p-3">{String(row.state ?? '—')}</td>
              <td className="p-3 text-sm text-muted-foreground">{row.submittedAt ? new Date(String(row.submittedAt)).toLocaleDateString() : '—'}</td>
