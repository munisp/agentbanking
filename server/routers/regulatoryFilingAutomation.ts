...
export const regulatoryFilingAutomationRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((complianceFilings as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[regulatoryFilingAutomation] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(complianceFilings)
        .where(eq((complianceFilings as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`regulatoryFilingAutomation record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[regulatoryFilingAutomation] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(complianceFilings);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(complianceFilings)
        .where(gte((complianceFilings as any).createdAt, since))
        .orderBy(desc((complianceFilings as any).id))
        .limit(input.limit);
