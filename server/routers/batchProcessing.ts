...
function validateBatchprocessingInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_BATCHPROCESSING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_BATCHPROCESSING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_BATCHPROCESSING.validateRange(
        data.amount,
        0,
        100_000_000
      )
