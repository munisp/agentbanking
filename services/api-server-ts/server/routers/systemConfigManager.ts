...
const setConfig = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "systemConfigManager",
      "mutation",
      "Executed systemConfigManager mutation"
    );
...
function validateSystemconfigmanagerInput(
...
const INTEGRITY_RULES_SYSTEMCONFIGMANAGER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SYSTEMCONFIGMANAGER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SYSTEMCONFIGMANAGER.validateRange(
        data.amount,
        0,
        100_000_000
      )
