...
function validateAgritechpaymentsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_AGRITECHPAYMENTS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGRITECHPAYMENTS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGRITECHPAYMENTS.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
export const agritechPaymentsRouter = router({
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      const result = await db.execute(
        sql`SELECT COUNT(*) as cnt FROM "agri_farms"`
      );
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        const result = await db.execute(
          sql`SELECT id, data, status, created_at, agent_id FROM "agri_farms" ORDER BY created_at DESC LIMIT ${lim} OFFSET ${off}`
        );
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "agritechPayments",
        "mutation",
        "Executed agritechPayments mutation"
      );
      ...
      const result = await db.execute(
        sql`INSERT INTO "agri_farms" (data, status, tenant_id) VALUES (${jsonStr}::jsonb, 'active', 'default') RETURNING id`
      );
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const result = await db.execute(
        sql`SELECT id, data, status, created_at, agent_id, metadata FROM "agri_farms" WHERE id = ${recordId}`
      );
  ...
  analytics: protectedProcedure.query(async () => {
    ...
    try {
      const result = await db.execute(
        sql`SELECT status, COUNT(*) as cnt FROM "agri_farms" GROUP BY status`
      );
  ...
  serviceHealth: protectedProcedure.query(async () => {
    const services = [
      { name: "AgriTech Payments (Go)", url: "http://localhost:8242/health" },
      { name: "AgriTech Payments (Rust)", url: "http://localhost:8243/health" },
      {
        name: "AgriTech Payments (Python)",
        url: "http://localhost:8244/health",
      },
    ];
