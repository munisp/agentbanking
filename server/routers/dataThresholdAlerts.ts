...
export const dataThresholdAlertsRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((observabilityAlerts as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[dataThresholdAlerts] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(observabilityAlerts)
        .where(eq((observabilityAlerts as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`dataThresholdAlerts record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[dataThresholdAlerts] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(observabilityAlerts);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(observabilityAlerts)
        .where(gte((observabilityAlerts as any).createdAt, since))
        .orderBy(desc((observabilityAlerts as any).id))
        .limit(input.limit);
