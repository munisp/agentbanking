...
class SettlementEngine:
    ...
    async def approve_settlement_batch(self, batch_id: str, approval: SettlementApprovalRequest) -> bool:
        """Approve or reject a settlement batch — row-locked to prevent concurrent double-approval."""
        async with self.db.transaction():
            # FOR UPDATE ensures only one concurrent approval wins; the second sees the updated status
            batch = await self.db.fetchrow(
                "SELECT * FROM settlement_batches WHERE id = $1 FOR UPDATE", batch_id
            )
    ...
    async def process_settlement_batch(self, batch_id: str, notify_agents: bool = True) -> Dict[str, Any]:
        ...
        Items are claimed atomically with FOR UPDATE SKIP LOCKED (marked PROCESSING inside a
        short transaction) before any external HTTP calls are made.  This prevents two concurrent
        invocations from processing the same item twice.
        """
        # Get batch — plain read, no lock needed here (status guard below is the safety net)
        batch = await self.db.fetchrow(
        ...
        await self.db.execute("""
            UPDATE settlement_batches SET status = $1, updated_at = $2 WHERE id = $3
        """, SettlementStatus.PROCESSING, datetime.utcnow(), batch_id)

        # Atomically claim items as PROCESSING so concurrent invocations skip them
        async with self.db.transaction():
            items = await self.db.fetch("""
                SELECT * FROM settlement_items
                WHERE batch_id = $1 AND status = ANY($2::text[])
                FOR UPDATE SKIP LOCKED
            """, batch_id, [SettlementItemStatus.PENDING.value, SettlementItemStatus.FAILED.value])

            if items:
                item_ids = [item['id'] for item in items]
                await self.db.execute("""
                    UPDATE settlement_items SET status = $1 WHERE id = ANY($2::uuid[])
                """, SettlementItemStatus.PROCESSING.value, item_ids)
        # Transaction released — items are exclusively claimed
        ...
        for item in items:
            try:
                ...
                await self.db.execute("""
                    ...
                    WHERE id = $4
                """, SettlementItemStatus.COMPLETED.value, transfer_id, datetime.utcnow(), item['id'])
                ...
                await self.db.execute("""
                    ...
                    WHERE id = $3
                """, SettlementItemStatus.FAILED.value, str(e), item['id'])
    ...
    async def retry_failed_settlements(self, batch_id: str, max_retries: int = 3) -> Dict[str, Any]:
        """Retry failed settlement items. Promotes batch to COMPLETED once all items succeed."""
        # Atomically claim retryable items to prevent concurrent retry runs
        async with self.db.transaction():
            items = await self.db.fetch("""
                SELECT * FROM settlement_items
                WHERE batch_id = $1 AND status = $2 AND retry_count < $3
                FOR UPDATE SKIP LOCKED
            """, batch_id, SettlementItemStatus.FAILED.value, max_retries)

            if items:
                item_ids = [item['id'] for item in items]
                await self.db.execute("""
                    UPDATE settlement_items SET status = $1 WHERE id = ANY($2::uuid[])
                """, SettlementItemStatus.RETRYING.value, item_ids)
        ...
        # Promote batch to COMPLETED if no non-completed items remain
        remaining = await self.db.fetchval("""
            SELECT COUNT(*) FROM settlement_items
            WHERE batch_id = $1 AND status NOT IN ($2, $3)
        """, batch_id, SettlementItemStatus.COMPLETED.value, SettlementItemStatus.RETRYING.value)

        if remaining == 0:
            await self.db.execute("""
                UPDATE settlement_batches
                SET status = $1, processed_at = COALESCE(processed_at, $2), updated_at = $3
                WHERE id = $4 AND status != $1
            """, SettlementStatus.COMPLETED, datetime.utcnow(), datetime.utcnow(), batch_id)
            logger.info(f"All items settled — batch {batch_id} promoted to COMPLETED")
...
async def _run_process_batch(batch_id: str, notify_agents: bool) -> None:
    """Standalone coroutine for background batch processing — owns its own DB connection."""
    conn = await get_db_connection()
    try:
        redis_conn = await get_redis_connection()
        http = await get_http_client()
        engine = SettlementEngine(conn, redis_conn, http)
        await engine.process_settlement_batch(batch_id, notify_agents)
    except Exception as exc:
        logger.error(f"Background batch processing failed for {batch_id}: {exc}")
    finally:
        await release_db_connection(conn)
...
async def process_settlement_batch(
    ...
    """Process an approved settlement batch (runs in background with its own DB connection)."""
    background_tasks.add_task(_run_process_batch, batch_id, process_request.notify_agents)
    return {
        'batch_id': batch_id,
