...
class AnaasScreen extends ConsumerStatefulWidget {
  const AnaasScreen({super.key});

  @override
  ConsumerState<AnaasScreen> createState() => _AnaasScreenState();
}

class _AnaasScreenState extends ConsumerState<AnaasScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/anaas.getStats');
      final listResp = await api.get('/api/trpc/anaas.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildSlaGauge(Map<String, dynamic> item) {
    final sla = double.tryParse('${item[\'sla_score\'] ?? 0}') ?? 0.0;
    final color = sla >= 99 ? Colors.green : sla >= 95 ? Colors.orange : Colors.red;
    return Text('${sla.toStringAsFixed(1)}% SLA', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: color));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.business, size: 24), const SizedBox(width: 8), const Text('ANaaS / Embedded Finance')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('ANaaS / Embedded Finance', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('Agent Network as a Service — white-label banking', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Tenants', '${_stats?['totalTenants'] ?? '\u2014'}', Icons.domain, Colors.blue),
                              _buildStatCard('Shared Agents', '${_stats?['sharedAgents'] ?? '\u2014'}', Icons.people, Colors.green),
                              _buildStatCard('Monthly Revenue', '₦${_stats?['monthlyRevenue'] ?? '\u2014'}', Icons.attach_money, Colors.orange),
                              _buildStatCard('Avg SLA', '${_stats?['avgSlaScore'] ?? '\u2014'}', Icons.speed, Colors.purple),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['tenantName'] ?? item['type'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
