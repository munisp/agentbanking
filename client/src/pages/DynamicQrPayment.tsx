...
export default function DynamicQrPayment() {
  ...
  const summary = trpc.dynamicQrPayment.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.dynamicQrPayment.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">QR code generation, payment collection, and merchant QR management</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Generate QR initiated")}>Generate QR</Button>
        <Button variant="outline" onClick={() => toast.success("Bulk Generate initiated")}>Bulk Generate</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalQrCodes ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalPayments ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Scans</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.merchantId ?? '—')}</td>
              <td className="p-3 font-mono">{formatCurrency(row.amount)}</td>
              <td className="p-3">{String(row.type ?? '—')}</td>
              <td className="p-3">{Number(row.scans ?? 0).toLocaleString()}</td>
