...
const CqrsEventStoreScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const { data } = await apiClient.get('/qr/list?page=1&limit=50');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Cqrs Event Store</Text>
...
export default CqrsEventStoreScreen;
