...
export const platformMetricsExporterRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[platformMetricsExporter] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`platformMetricsExporter record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[platformMetricsExporter] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
