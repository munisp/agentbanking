// 54agent Agent Store Service — Go Microservice
// Port: 8220
...
func (mc *MiddlewareClients) validateJWT(token string) (map[string]interface{}, error) {
	url := fmt.Sprintf("%s/realms/54agent/protocol/openid-connect/userinfo", mc.config.KeycloakURL)
...
func (mc *MiddlewareClients) handleUpdateStore(w http.ResponseWriter, r *http.Request) {
	...
	if v, ok := updates["storeName"].(string); ok {
		store.StoreName = v
	}
	if v, ok := updates["description"].(string); ok {
		store.Description = v
	}
	if v, ok := updates["logoUrl"].(string); ok {
		store.LogoURL = v
	}
	if v, ok := updates["bannerUrl"].(string); ok {
		store.BannerURL = v
	}
	if v, ok := updates["themeColor"].(string); ok {
		store.ThemeColor = v
	}
	if v, ok := updates["aboutHtml"].(string); ok {
		store.AboutHTML = v
	}
	...
	if v, ok := updates["lga"].(string); ok {
		store.LGA = v
	}
	if v, ok := updates["deliveryEnabled"].(bool); ok {
		store.DeliveryEnabled = v
	}
	if v, ok := updates["pickupEnabled"].(bool); ok {
		store.PickupEnabled = v
	}
...
func (mc *MiddlewareClients) handleNearbyStores(w http.ResponseWriter, r *http.Request) {
	...
	for _, s := range reg.stores {
		if s.Status != "active" || s.Latitude == 0 || s.Longitude == 0 {
			continue
		}
...
func (mc *MiddlewareClients) handleUpdateFulfillmentStatus(w http.ResponseWriter, r *http.Request) {
	...
	if req.Status == "delivered" {
		tracking.ActualDelivery = time.Now()
	}
	if req.Notes != "" {
		tracking.DeliveryNotes = req.Notes
	}
	reg.mu.Unlock()
