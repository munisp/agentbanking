...
function validateCustomerjourneymapperInput(
...
function logOperation(action: string, details: Record<string, unknown>) {
  const auditEntry = {
    ...
    createdAt: Date.now(),
    updatedAt: Date.now(),
    resource: "customerJourneyMapper",
    action,
    ...details,
  };
  auditFinancialAction(
...
const INTEGRITY_RULES_CUSTOMERJOURNEYMAPPER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CUSTOMERJOURNEYMAPPER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CUSTOMERJOURNEYMAPPER.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for customerJourneyMapper ───────────────────────────────────────
