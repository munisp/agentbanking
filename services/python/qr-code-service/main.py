...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "qr-code-service"),
...
app = FastAPI(title="QR Code Service", description="QR Code Service for Remittance Platform", version="1.0.0")
...
async def health_check():
    try:
        ...
        return {"status": "healthy", "service": "qr-code-service", "database": "connected"}
    except Exception as e:
...
async def create_item(item: ItemCreate, token: str = Depends(verify_token)):
    ...
    async with pool.acquire() as conn:
        ...
        query = f"INSERT INTO qr_codes ({', '.join(cols)}) VALUES ({ph}) RETURNING *"
...
async def list_items(skip: int = 0, limit: int = 50, token: str = Depends(verify_token)):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM qr_codes ORDER BY created_at DESC LIMIT $1 OFFSET $2",
            limit, skip
        )
...
async def get_item(item_id: str, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM qr_codes WHERE id=$1", uuid.UUID(item_id))
        if not row:
...
async def update_item(item_id: str, item: ItemUpdate, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        existing = await conn.fetchrow("SELECT * FROM qr_codes WHERE id=$1", uuid.UUID(item_id))
        if not existing:
            raise HTTPException(status_code=404, detail="Item not found")
        ...
        for k, v in updates.items():
            ...
            idx += 1
        query = f"UPDATE qr_codes SET {', '.join(set_parts)}, updated_at=NOW() WHERE id=$1 RETURNING *"
...
async def delete_item(item_id: str, token: str = Depends(verify_token)):
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        result = await conn.execute("DELETE FROM qr_codes WHERE id=$1", uuid.UUID(item_id))
        if result == "DELETE 0":
            raise HTTPException(status_code=404, detail="Item not found")
        return {"deleted": True}
