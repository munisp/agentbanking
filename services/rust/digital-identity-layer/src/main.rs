//! 54Link Digital Identity Layer — Rust Microservice
//!
//! DID document management, verifiable credential engine, zero-knowledge proofs
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/identity/did/create — Create DID document
//!   POST /api/v1/identity/did/resolve — Resolve DID to document
//!   POST /api/v1/identity/vc/sign — Sign verifiable credential
//!   POST /api/v1/identity/vc/verify — Verify credential proof
//!   POST /api/v1/identity/zkp/generate — Generate zero-knowledge proof
//!
//! Port: 8276
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8276),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "digital-identity-layer"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "digital-identity-layer".into(),
...
async fn create_record(
    ...
    dapr.publish("identity.verified", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("digital-identity-layer-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("did_identities", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("digital-identity-layer:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("identity_verifications", &payload).await;

    // Persist to PostgreSQL
    let _ = state.postgres.persist("digital_identity_layer", &serde_json::json!({
        "service": "digital-identity-layer",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("digital-identity-layer:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("did_identities", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Digital Identity Layer (Rust) starting on port {}", port);
