...
class FloatReconciliationScreen extends StatefulWidget {
  const FloatReconciliationScreen({super.key});
  @override
  State<FloatReconciliationScreen> createState() => _FloatReconciliationScreenState();
}

class _FloatReconciliationScreenState extends State<FloatReconciliationScreen> {
  ...
  Future<void> _load() async {
    try {
      final result = await ApiService.instance.get('/float/list', queryParams: {'page': '1', 'limit': '50'});
      setState(() {
        _data = result ?? {};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Float Reconciliation'),
        actions: [
