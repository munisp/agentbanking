...
export default function PublishReadinessChecker() {
  ...
  const summary = trpc.publishReadinessChecker.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.publishReadinessChecker.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Publish Readiness Checker</h1>
            <p className="text-muted-foreground">Pre-deployment checks, readiness gates, and go/no-go decisions</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Run All Checks initiated")}>Run All Checks</Button>
        <Button variant="outline" onClick={() => toast.success("Deploy initiated")}>Deploy</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalChecks ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
        ...
        <Card key="2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.blockers ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Last Deploy</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.lastDeploy ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Check ID</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Check Name</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Run At</th>
                      </tr>
                    </thead>
                    <tbody>
              ...
              <td className="p-3 text-sm text-muted-foreground">{row.runAt ? new Date(String(row.runAt)).toLocaleDateString() : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
