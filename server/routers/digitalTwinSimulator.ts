...
function validateDigitaltwinsimulatorInput(
...
const INTEGRITY_RULES_DIGITALTWINSIMULATOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_DIGITALTWINSIMULATOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_DIGITALTWINSIMULATOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _digitalTwinSimulator_db = {
...
// ── Transaction Handling for digitalTwinSimulator ───────────────────────────────────────
