//! 54Link NFC Tap-to-Pay — Rust Microservice
//!
//! EMV kernel, cryptogram verification, secure element communication
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/nfc/emv/parse — Parse EMV TLV data
//!   POST /api/v1/nfc/emv/verify-cryptogram — Verify ARQC/TC cryptogram
//!   POST /api/v1/nfc/emv/generate-arpc — Generate ARPC response
//!   POST /api/v1/nfc/secure/derive-key — Derive session key from master
//!
//! Port: 8237
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8237),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "nfc-tap-to-pay"});
...
async fn create_record(
    ...
    dapr.publish("nfc.tap.initiated", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("nfc-tap-to-pay-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("nfc_terminals", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("nfc-tap-to-pay:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("nfc_transactions", &payload).await;
        // Persist to PostgreSQL
        match state.postgres.insert("nfc_transactions", &payload).await {
            Ok(row) => info!("[Postgres] Inserted into nfc_transactions: {:?}", row.get("id")),
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("nfc-tap-to-pay:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("nfc_terminals", &query).await;
    if !results.is_empty() {
