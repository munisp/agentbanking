...
function validateVaultsecretsInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_VAULTSECRETS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_VAULTSECRETS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_VAULTSECRETS.validateRange(data.amount, 0, 100_000_000)
    )
