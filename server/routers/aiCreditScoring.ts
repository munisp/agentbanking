...
async function publishaiCreditScoringMiddleware(
  action: string,
  ref: string,
...
export const aiCreditScoringRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await publishaiCreditScoringMiddleware("create", `${Date.now()}`, {
        action: "create",
      }).catch(() => {});
  ...
  updateStatus: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      // Middleware fan-out (fail-open)
      await publishaiCreditScoringMiddleware("updateStatus", `${Date.now()}`, {
        action: "updateStatus",
      }).catch(() => {});
