...
const VoiceCommandPosScreen: React.FC = () => {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const { data } = await apiClient.get('/pos/list?page=1&limit=50');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Voice Command Pos</Text>
...
export default VoiceCommandPosScreen;
