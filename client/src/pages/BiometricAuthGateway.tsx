...
const statusColors: Record<string, string> = {
  ...
  'spoof': 'bg-red-500/20 text-red-400'
};
...
export default function BiometricAuthGateway() {
  ...
  const summary = trpc.biometricAuthGateway.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.biometricAuthGateway.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Biometric Auth Gateway</h1>
            <p className="text-muted-foreground">Fingerprint, face, and voice authentication management</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Enroll User initiated")}>Enroll User</Button>
        <Button variant="outline" onClick={() => toast.success("Test Auth initiated")}>Test Auth</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalEnrollments ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Auth Success</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card key="2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.failedAttempts ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Spoof Detected</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.spoofDetected ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Result</th>
              ...
              <td className="p-3">{String(row.userId ?? '—')}</td>
              <td className="p-3">{String(row.method ?? '—')}</td>
