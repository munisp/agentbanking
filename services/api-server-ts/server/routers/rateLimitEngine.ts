...
function validateRatelimitengineInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_RATELIMITENGINE = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_RATELIMITENGINE.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_RATELIMITENGINE.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  createRule: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "rateLimitEngine",
        "mutation",
        "Executed rateLimitEngine mutation"
      );
