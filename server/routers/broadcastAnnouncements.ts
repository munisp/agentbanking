...
export const broadcastAnnouncementsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[broadcastAnnouncements] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`broadcastAnnouncements record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[broadcastAnnouncements] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
