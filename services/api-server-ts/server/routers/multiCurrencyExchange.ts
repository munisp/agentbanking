...
// Mockware remediation: the previous scaffold served agentPushSubscriptions
// rows from getRates/convert/getCorridors and hardcoded stats. Corridor
// rates now come from the live FX rate feed (lib/fxRateFeed.ts) with a
// DB-backed cache + staleness guard; convert performs a real FX exchange
// (float debit + GL entry + TigerBeetle posting with corrected accounts);
// setSpread persists to system_config.
import { z } from "zod";
...
import { calculateFee } from "../lib/domainCalculations";
import {
  getFreshRates,
  getLiveRate,
  getSpreads,
...
const getRates = protectedProcedure
  ...
  .query(async ({ input }) => {
    try {
      ...
      return {
        items: all.slice(offset, offset + lim),
        total: all.length,
        page: input.page ?? 1,
        limit: lim,
      };
...
const convert = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    // Conversion Payable 3002 / Credit Agent Float 2001). Fail-open is not
    ...
    auditFinancialAction(
      ...
      "convert",
      `FX exchange ${ref}: ${input.amount} ${input.fromCurrency} → ${convertedAmount} ${input.toCurrency} @ ${rate} (${ledgerStatus})`
    );
...
const setSpread = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    try {
      ...
      // without persisting (the previous version mutated an unrelated
      // agentPushSubscriptions row).
      const persistedSpreads = await saveSpread(pair, spread);

      await writeAuditLog({
        agentId: 0,
        agentCode: "system",
        action: "FX_SPREAD_UPDATED",
        resource: "multiCurrencyExchange",
        resourceId: pair,
        status: "success",
        metadata: { pair, spread, persisted: true },
      });
