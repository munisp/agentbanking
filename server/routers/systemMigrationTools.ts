...
function validateSystemmigrationtoolsInput(
...
const INTEGRITY_RULES_SYSTEMMIGRATIONTOOLS = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_SYSTEMMIGRATIONTOOLS.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_SYSTEMMIGRATIONTOOLS.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _systemMigrationTools_db = {
...
// ── Transaction Handling for systemMigrationTools ───────────────────────────────────────
