...
export default function PlatformRecommendations() {
  ...
  const summary = trpc.platformRecommendations.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.platformRecommendations.list.useQuery({ limit: 20 });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">AI-powered platform optimization suggestions and action items</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Generate initiated")}>Generate</Button>
        <Button variant="outline" onClick={() => toast.success("Implement initiated")}>Implement</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalRecommendations ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.implemented ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(summary?.potentialSavings)}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Risk Reductions</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.riskReductions ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Recommendation</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Effort</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.impact ?? '—')}</td>
              <td className="p-3">{String(row.effort ?? '—')}</td>
