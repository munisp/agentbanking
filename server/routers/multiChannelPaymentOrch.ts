...
export const multiChannelPaymentOrchRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[multiChannelPaymentOrch] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`multiChannelPaymentOrch record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[multiChannelPaymentOrch] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
