...
function validateAgentinventorymgmtInput(
...
const INTEGRITY_RULES_AGENTINVENTORYMGMT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTINVENTORYMGMT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTINVENTORYMGMT.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for agentInventoryMgmt ───────────────────────────────────────
