...
const INTEGRITY_RULES_CARRIERLIVEPRICING = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_CARRIERLIVEPRICING.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_CARRIERLIVEPRICING.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  updateRate: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "carrierLivePricing",
        "mutation",
        "Executed carrierLivePricing mutation"
      );
