...
class VATReturn(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    __table_args__ = (Index("ix_vat_return_entity_period", "entity_id", "period"),)


class VATAutomationConfig(Base):
    """Per-agent VAT automation preferences."""
    __tablename__ = "vat_automation_configs"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid4()))
    entity_id = Column(String(100), nullable=False, unique=True, index=True)
    # Auto-record VAT transaction whenever a payment event arrives
    auto_record_vat = Column(Boolean, default=True)
    # Auto-generate monthly VAT return on 1st of following month
    auto_generate_return = Column(Boolean, default=True)
    # Auto-file the generated return with FIRS (requires auto_generate_return)
    auto_file_firs = Column(Boolean, default=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class VATReturnRequest(BaseModel):
    entity_id: str
    # entity_name is optional — looked up from VATRegistration if omitted
    entity_name: Optional[str] = None
    # Accept either period string "YYYY-MM" or explicit year/month
...
class AutomationConfigSchema(BaseModel):
    auto_record_vat: bool = True
    auto_generate_return: bool = True
    auto_file_firs: bool = False
...
class NigeriaVATService:
    ...
    def calculate_vat(
        self, taxable_amount: Decimal, category: VATCategory
    ...
    def check_registration_required(self, entity_id: str) -> Dict:
        ...
        """
        reg = (
            self.db.query(VATRegistration)
            .filter(VATRegistration.entity_id == entity_id)
            .first()
        )
        if not reg:
            return {
    ...
    def register_for_vat(self, req: VATRegistrationRequest) -> VATRegistration:
        ...
        existing = (
            self.db.query(VATRegistration)
            .filter(VATRegistration.entity_id == req.entity_id)
            .first()
        )
        if existing:
    ...
    def record_vat_transaction(self, req: VATTransactionCreate) -> VATTransaction:
        ...
        txn = VATTransaction(
            ...
            vat_rate=(
                VAT_STANDARD_RATE
                if req.vat_category not in (VATCategory.EXEMPT, VATCategory.ZERO_RATED)
                else Decimal("0")
            ),
    ...
    def generate_monthly_return(self, req: VATReturnRequest) -> VATReturn:
        ...
        """
        # Resolve year/month from either period string or explicit fields
        if req.period:
            year, month = int(req.period[:4]), int(req.period[5:7])
        else:
            year, month = req.year, req.month

        # Look up entity_name from registration if not provided
        entity_name = req.entity_name
        if not entity_name:
            reg = (
                self.db.query(VATRegistration)
            ...
            entity_name = reg.entity_name if reg else req.entity_id

        period = f"{year}-{month:02d}"
        period_start = date(year, month, 1)
        last_day = (period_start.replace(day=28) + timedelta(days=4)).replace(
            day=1
        ) - timedelta(days=1)
        # Filing deadline: 21st of following month
        if month == 12:
            filing_deadline = date(year + 1, 1, 21)
        else:
            filing_deadline = date(year, month + 1, 21)
        ...
        total_exempt_sales = sum(
            t.taxable_amount
            for t in output_txns
            if t.vat_category == VATCategory.EXEMPT
        )
        total_zero_rated_sales = sum(
            t.taxable_amount
            for t in output_txns
            if t.vat_category == VATCategory.ZERO_RATED
        )
        ...
        if today > filing_deadline:
            months_late = (
                (today.year - filing_deadline.year) * 12
                + today.month
                - filing_deadline.month
            )
            late_penalty = self._round_vat(
                net_vat_payable * PENALTY_RATE_PER_MONTH * months_late
            )
        ...
        logger.info(
            f"Generated VAT return for {req.entity_id} period {period}: net payable NGN {net_vat_payable}"
        )
        return vat_return
    ...
    def record_payment(
        self, return_id: str, amount_paid: Decimal, payment_date: date
    ) -> VATReturn:
    ...
    def get_vat_summary(self, entity_id: str, period: str) -> VATSummaryResponse:
        ...
        txns = (
            self.db.query(VATTransaction)
            .filter(
                VATTransaction.entity_id == entity_id, VATTransaction.period == period
            )
            .all()
        )
        ...
        effective_rate = (
            (output_vat / total_sales * 100) if total_sales > 0 else Decimal("0")
        )
    ...
    def list_transactions(self, limit: int = 100, offset: int = 0) -> list[Dict]:
        ...
        txns = (
            self.db.query(VATTransaction)
            .order_by(VATTransaction.transaction_date.desc(), VATTransaction.id.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
        return [
            {
                ...
                "vat_category": t.vat_category.value,
                ...
                "is_input_vat": t.is_input_vat,
                "is_output_vat": t.is_output_vat,
                "is_withholding_vat": t.is_withholding_vat,
                "customer_tin": t.customer_tin,
                "supplier_tin": t.supplier_tin,
                "invoice_number": t.invoice_number,
            }
            for t in txns
        ]

    def list_businesses_with_transactions(self) -> list[Dict]:
        """Return businesses that have VAT transactions, aggregated across all periods."""
        txns = self.db.query(VATTransaction).all()
        ...
        for txn in txns:
            ...
            bucket["periods"].add(txn.period)
            bucket["transaction_count"] += 1
            if txn.is_output_vat:
                bucket["output_vat"] += Decimal(str(txn.vat_amount))
                bucket["taxable_sales"] += Decimal(str(txn.taxable_amount))
            if txn.is_input_vat:
                bucket["input_vat"] += Decimal(str(txn.vat_amount))
            current_ts = txn.transaction_date.isoformat()
            if (
                bucket["latest_transaction_at"] is None
                or current_ts > bucket["latest_transaction_at"]
            ):
                bucket["latest_transaction_at"] = current_ts

        results = []
        for bucket in grouped.values():
            net_vat = bucket["output_vat"] - bucket["input_vat"]
            results.append(
                {
                    ...
                    "taxable_sales": str(bucket["taxable_sales"]),
                    "output_vat": str(bucket["output_vat"]),
                    "input_vat": str(bucket["input_vat"]),
                    "net_vat": str(max(net_vat, Decimal("0"))),
                    "latest_transaction_at": bucket["latest_transaction_at"],
                }
            )

        results.sort(key=lambda row: row["latest_transaction_at"] or "", reverse=True)
        return results
        ...
        for month in range(1, 13):
            ...
            if vat_return:
                month_data = {
                    ...
                    "filed_at": (
                        str(vat_return.filed_at) if vat_return.filed_at else None
                    ),
                }
                annual_data["months"].append(month_data)
                annual_data["annual_totals"][
                    "total_taxable_sales"
                ] += vat_return.total_taxable_sales
                annual_data["annual_totals"][
                    "total_output_vat"
                ] += vat_return.output_vat
                annual_data["annual_totals"][
                    "total_input_vat"
                ] += vat_return.input_vat_claimable
                annual_data["annual_totals"][
                    "total_net_vat_paid"
                ] += vat_return.amount_paid
                annual_data["annual_totals"][
                    "total_penalties"
                ] += vat_return.late_filing_penalty
    ...
    def get_automation_config(self, entity_id: str) -> VATAutomationConfig:
        config = (
            self.db.query(VATAutomationConfig)
            .filter(VATAutomationConfig.entity_id == entity_id)
            .first()
        )
        if not config:
            config = VATAutomationConfig(entity_id=entity_id)
            self.db.add(config)
            self.db.commit()
    ...
    def update_automation_config(
        self, entity_id: str, data: "AutomationConfigSchema"
    ) -> VATAutomationConfig:
        config = self.get_automation_config(entity_id)
        config.auto_record_vat = data.auto_record_vat
        config.auto_generate_return = data.auto_generate_return
        config.auto_file_firs = data.auto_file_firs
        self.db.commit()
        self.db.refresh(config)
        return config

    # ─────────────────────────────────────────────
    # AUTO-RECORD from payment event
    # ─────────────────────────────────────────────

    def record_vat_from_payment_event(
        self,
        entity_id: str,
        transaction_ref: str,
        amount: Decimal,
        note: str,
        transaction_date: datetime,
    ) -> Optional["VATTransaction"]:
        """
        Called by the Dapr event consumer when a TRANSACTION_INITIATED event arrives.
        Agency banking fees are VAT-exempt per FIRS schedule. We still record the
        transaction so agents have a full audit trail and monthly summary.
        Idempotent — skips if transaction_ref already exists.
        """
        cfg = self.get_automation_config(entity_id)
        if not cfg.auto_record_vat:
            return None

        # Idempotency check
        existing = (
            self.db.query(VATTransaction)
            .filter(VATTransaction.transaction_ref == transaction_ref)
            .first()
        )
        if existing:
            return existing

        # Agency banking fees / money transfers are VAT-exempt (FIRS Finance Act 2020)
        ...
        txn = VATTransaction(
            ...
            description=note or "Agent banking transaction",
    ...
    # ─────────────────────────────────────────────
    # AUTO-GENERATE returns (called by scheduler)
    # ─────────────────────────────────────────────

    def auto_generate_for_period(self, year: int, month: int) -> list[dict]:
        """
        Generate VAT returns for all entities that have auto_generate_return=True
        and have transactions in the given period. Called on the 1st of each month
        for the prior month. Returns a list of result dicts.
        """
        period = f"{year}-{month:02d}"
        results = []

        # Find all entities with transactions in this period
        entity_ids = (
            self.db.query(VATTransaction.entity_id)
            .filter(VATTransaction.period == period)
            .distinct()
            .all()
        )
        ...
        for (entity_id,) in entity_ids:
            cfg = self.get_automation_config(entity_id)
            if not cfg.auto_generate_return:
                continue

            # Skip if a return already exists
            existing = (
                self.db.query(VATReturn)
                .filter(
                    VATReturn.entity_id == entity_id,
                    VATReturn.period == period,
                )
                .first()
            )
            if existing:
                results.append({"entity_id": entity_id, "period": period, "status": "already_exists", "return_id": existing.id})
                continue

            # Look up entity name from registration (fall back to entity_id)
            reg = (
                self.db.query(VATRegistration)
                .filter(VATRegistration.entity_id == entity_id)
                .first()
            )
            entity_name = reg.entity_name if reg else entity_id
            tin = reg.tin if reg else None
            vat_reg_number = reg.vat_registration_number if reg else None

            req = VATReturnRequest(
                entity_id=entity_id,
                entity_name=entity_name,
                year=year,
                month=month,
                tin=tin,
                vat_registration_number=vat_reg_number,
            )
            try:
                vat_return = self.generate_monthly_return(req)
                result = {"entity_id": entity_id, "period": period, "status": "generated", "return_id": vat_return.id}

                # Auto-file if configured
                if cfg.auto_file_firs:
                    filed = self.auto_file_return(vat_return.id)
                    result["filed"] = filed
            except Exception as exc:
                result = {"entity_id": entity_id, "period": period, "status": "error", "error": str(exc)}
                logger.error(f"Auto-generate VAT return failed for {entity_id}/{period}: {exc}")

            results.append(result)

        logger.info(f"Auto-generated VAT returns for {period}: {len(results)} entities processed")
        return results

    def auto_file_return(self, return_id: str) -> bool:
        """
        Auto-file a VAT return. Generates a system receipt reference.
        In production this would call the FIRS eTax portal API.
        """
        vat_return = self.db.query(VATReturn).filter(VATReturn.id == return_id).first()
        if not vat_return or vat_return.status not in (ReturnStatus.DRAFT, ReturnStatus.OVERDUE):
            return False
        # Generate system receipt — replace with real FIRS API call in production
        receipt = f"FIRS-AUTO-{vat_return.period}-{str(uuid4())[:8].upper()}"
        vat_return.status = ReturnStatus.FILED
        vat_return.firs_receipt_number = receipt
        vat_return.filed_at = datetime.utcnow()
        self.db.commit()
        logger.info(f"Auto-filed VAT return {return_id} for {vat_return.entity_id}: receipt {receipt}")
        return True
