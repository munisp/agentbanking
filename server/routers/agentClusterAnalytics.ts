...
export const agentClusterAnalyticsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[agentClusterAnalytics] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`agentClusterAnalytics record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[agentClusterAnalytics] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
