...
export const cbdcIntegrationGatewayRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[cbdcIntegrationGateway] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`cbdcIntegrationGateway record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[cbdcIntegrationGateway] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
