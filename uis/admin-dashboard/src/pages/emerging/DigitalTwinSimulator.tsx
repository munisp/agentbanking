...
export default function DigitalTwinSimulator() {
  ...
  const summary = trpc.digitalTwinSimulator.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.digitalTwinSimulator.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              System simulation, capacity modeling, and what-if scenario
              analysis
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("New Simulation initiated")}>
              New Simulation
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Clone Scenario initiated")}
            >
              Clone Scenario
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.activeSimulations ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalScenarios ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="2">
          ...
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.computeTime ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
