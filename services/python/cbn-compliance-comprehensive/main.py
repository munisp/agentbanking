import httpx
"""
CBN Compliance Engine - FastAPI microservice
Central Bank of Nigeria regulatory compliance with automated reporting, threshold monitoring, and filing
"""
import os
import logging
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "cbn-compliance-comprehensive"),
...
app = FastAPI(title="CBN Compliance Engine", description="Central Bank of Nigeria regulatory compliance with automated reporting, threshold monitoring, and filing", version="1.0.0")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/cbn_compliance_comprehensive")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "cbn-compliance-comprehensive", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
...
async def startup_event():
    ...
    await publish_kafka("cbn.compliance.comprehensive.started", {
        "service": "cbn-compliance-comprehensive",
