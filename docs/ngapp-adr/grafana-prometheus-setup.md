...
The 54agent POS Shell exposes a Prometheus-compatible metrics endpoint at `GET /api/metrics`. This guide walks through connecting it to a Prometheus scraper and importing the pre-built Grafana dashboard.
...
scrape_configs:
  - job_name: "pos-shell"
    ...
    static_configs:
      - targets:
          - "pos-shell.54agent.internal:443"
        labels:
...
k6 run --out cloud tests/load/transaction-throughput.js \
  -e BASE_URL=https://pos-shell.54agent.internal \
  -e AGENT_CODE=AGT001 \
  -e AGENT_PIN=1234
```
