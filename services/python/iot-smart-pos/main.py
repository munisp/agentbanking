...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "iot-smart-pos"),
...
pg_client = PostgresClient(DATABASE_URL, "iot_analytics")
...
async def health_check():
    return {
        "status": "healthy",
        "service": "iot-smart-pos-analytics",
        "port": PORT,
...
async def analytics_summary():
    ...
    # Cache and publish
    cache.set("iot-smart-pos:summary", summary)
    await dapr.publish("iot-smart-pos.analytics.updated", summary)
    await fluvio.produce("iot-smart-pos-analytics", summary)
    await lakehouse.ingest("iot-smart-pos_analytics", summary)
...
async def forecast(request: ForecastRequest):
    ...
    await dapr.publish("iot-smart-pos.forecast.generated", result)
    return result
...
async def register_apisix():
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            route = {
                "uri": "/api/v1/iot/smart/pos/analytics/*",
                "upstream": {
                    "type": "roundrobin",
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"54Link IoT Smart POS Analytics starting on port {PORT}")
