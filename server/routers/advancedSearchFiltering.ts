...
export const advancedSearchFilteringRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[advancedSearchFiltering] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`advancedSearchFiltering record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[advancedSearchFiltering] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
