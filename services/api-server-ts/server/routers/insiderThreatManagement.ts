...
// See note in ../middleware/insiderThreatPrevention.ts — "insider.*" topics are
// cast to the declared KafkaTopic type until ../kafkaClient (owned by another
// wave) adds them to its union.
...
export const insiderThreatManagementRouter = router({
  ...
  requestStepUp: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      publishEvent("insider.auth.step-up" as KafkaTopicName, `SU-${agent.id}-${Date.now()}`, {
        agentCode: agent.agentCode,
        type: "step_up_issued",
      }).catch(() => {});
