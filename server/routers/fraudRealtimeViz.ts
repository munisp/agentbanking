...
export const fraudRealtimeVizRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[fraudRealtimeViz] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`fraudRealtimeViz record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[fraudRealtimeViz] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
