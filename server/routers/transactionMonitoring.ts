...
export const transactionMonitoringRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          ...
          .orderBy(desc((txMonitoringAlerts as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[transactionMonitoring] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(txMonitoringAlerts)
        .where(eq((txMonitoringAlerts as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`transactionMonitoring record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[transactionMonitoring] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(txMonitoringAlerts);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(txMonitoringAlerts)
        .where(gte((txMonitoringAlerts as any).createdAt, since))
        .orderBy(desc((txMonitoringAlerts as any).id))
        .limit(input.limit);
