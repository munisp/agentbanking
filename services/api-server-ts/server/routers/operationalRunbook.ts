...
function validateOperationalrunbookInput(
...
const INTEGRITY_RULES_OPERATIONALRUNBOOK = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_OPERATIONALRUNBOOK.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_OPERATIONALRUNBOOK.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for operationalRunbook ───────────────────────────────────────
