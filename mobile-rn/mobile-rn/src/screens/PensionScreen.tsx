...
const PensionBar = ({ item }: { item: RecordItem }) => {
    const c = Number(item.total_contributed || 0); const t = Number(item.target || 1000000);
    const pct = t > 0 ? Math.min(100, c / t * 100) : 0;
    return (<View><Text style={{ fontSize: 10, color: '#9ca3af' }}>₦{(c/1000).toFixed(0)}K of ₦{(t/1000).toFixed(0)}K</Text>
      <View style={{ height: 4, backgroundColor: '#e5e7eb', borderRadius: 2, marginTop: 2 }}>
        <View style={{ height: 4, width: pct + '%', backgroundColor: '#22c55e', borderRadius: 2 }} /></View></View>);
  };
...
export default function PensionScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/pension.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/pension.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Micro-Pension...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Micro-Pension</Text>
            <Text style={styles.subtitle}>Informal sector pension savings</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>₦{stats?.totalContributions ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📈</Text>
            <Text style={styles.statLabel}>Avg Monthly</Text>
            <Text style={styles.statValue}>₦{stats?.avgMonthlyContrib ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>💸</Text>
            <Text style={styles.statLabel}>Withdrawals</Text>
            <Text style={styles.statValue}>{stats?.withdrawalRequests ?? '—'}</Text>
          </View>
          </View>
