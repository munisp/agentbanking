...
export default function CustomerSegmentationEngine() {
  ...
  const summary = trpc.customerSegmentationEngine.getSummary.useQuery()
    ?.data as Record<string, unknown> | undefined;
  const listQ = trpc.customerSegmentationEngine.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              RFM analysis, behavioral segments, and targeted marketing groups
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Create Segment initiated")}>
              Create Segment
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Run Analysis initiated")}
            >
              Run Analysis
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.totalSegments ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
          ...
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.highValueCustomers ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.churnRisk ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">
                            {Number(row.customerCount ?? 0).toLocaleString()}
                          </td>
