...
async function publishcrossBorderRemittanceHubMiddleware(
  event: string,
  key: string,
  payload: Record<string, unknown>
) {
  publishEvent("transfers.initiated", key, {
    event,
    ...payload,
    timestamp: Date.now(),
  }).catch(() => {});
  ...
  publishTxToFluvio({
    ...
    agentCode: String(payload.agentId ?? "system"),
    amount: Number(payload.amount ?? 0),
    type: `transfers.initiated.${event}`,
    timestamp: Date.now(),
  }).catch(() => {});
  dapr
    .publishEvent("pubsub", `transfers.initiated.${event}`, { key, ...payload })
    .catch(() => {});
  ingestToLakehouse("crossBorderRemittanceHub", {
    event,
    key,
    ...payload,
    timestamp: new Date().toISOString(),
...
export const crossBorderRemittanceHubRouter = router({
  ...
  initiateTransfer: protectedProcedure
    ...
    .mutation(async () => {
      try {
        await publishcrossBorderRemittanceHubMiddleware(
          "initiateTransfer",
          `${Date.now()}`,
