...
async function publishmqttBridgeMiddleware(
  action: string,
  ref: string,
  ...
  testMqttBridge: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        await publishmqttBridgeMiddleware("testMqttBridge", `${Date.now()}`, {
          action: "testMqttBridge",
        }).catch(() => {});
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        try {
          ...
          const latencyMs = Date.now() - start;
          // Middleware fan-out (fail-open)
          await publishmqttBridgeMiddleware("publishTest", `${Date.now()}`, {
            action: "publishTest",
          }).catch(() => {});
