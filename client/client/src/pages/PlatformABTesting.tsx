...
export default function PlatformABTesting() {
  ...
  const summary = trpc.platformABTesting.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.platformABTesting.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Experiment management, variant allocation, and statistical
              analysis
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button
              onClick={() => toast.success("Create Experiment initiated")}
            >
              Create Experiment
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Conclude initiated")}
            >
              Conclude
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.activeExperiments ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
          ...
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.significantResults ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">
                            {Number(row.variants ?? 0).toLocaleString()}
                          </td>
                          <td className="p-3">
