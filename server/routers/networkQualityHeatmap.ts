...
export const networkQualityHeatmapRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[networkQualityHeatmap] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`networkQualityHeatmap record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[networkQualityHeatmap] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
