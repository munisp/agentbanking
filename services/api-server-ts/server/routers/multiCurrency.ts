...
function validateMulticurrencyInput(data: Record<string, unknown>): boolean {
...
const INTEGRITY_RULES_MULTICURRENCY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_MULTICURRENCY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_MULTICURRENCY.validateRange(data.amount, 0, 100_000_000)
    )
  ...
  listBalances: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "multiCurrency",
        "mutation",
        "Executed multiCurrency mutation"
      );
