...
export default function OfflinePosMode() {
  ...
  const summary = trpc.offlinePosMode.getStats.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = { data: { items: [], data: [], total: 0 }, isLoading: false } as any;
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Offline POS Mode</h1>
            <p className="text-muted-foreground">Offline transaction queuing, sync management, and conflict resolution</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Force Sync initiated")}>Force Sync</Button>
        <Button variant="outline" onClick={() => toast.success("Resolve Conflicts initiated")}>Resolve Conflicts</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.offlineDevices ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.pendingSync ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Queued Transactions</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalQueued ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Sync Success</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.syncSuccessRate ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Queued Tx</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3 font-mono">{formatCurrency(row.totalAmount)}</td>
              <td className="p-3 text-sm text-muted-foreground">{row.lastSync ? new Date(String(row.lastSync)).toLocaleDateString() : '—'}</td>
