...
const INTEGRITY_RULES_AGENTTRAININGACADEMY = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AGENTTRAININGACADEMY.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AGENTTRAININGACADEMY.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _agentTrainingAcademy_db = {
  ...
  enrollAgent: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "agentTrainingAcademy",
        "mutation",
        "Executed agentTrainingAcademy mutation"
      );
