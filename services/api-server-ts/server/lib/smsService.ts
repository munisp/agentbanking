 ...
 *   AT_SENDER_ID           — Africa's Talking sender ID (e.g. "54agent")
 *   TERMII_API_KEY         — Termii API key (existing)
 *   TERMII_SENDER_ID       — Termii sender ID (default: "54agent")
 */
...
async function sendViaAfricasTalking(
  ...
  const senderId = process.env.AT_SENDER_ID ?? "54agent";
...
async function sendViaTermii(msg: SmsMessage): Promise<{ messageId: string }> {
  ...
  const senderId = process.env.TERMII_SENDER_ID ?? "54agent";
...
export function buildRateAlertSms(opts: {
  ...
  return {
    to: "",
    body: `54agent Alert: ${opts.baseCurrency}/${opts.targetCurrency} has ${verb} your target of ${opts.targetRate}. Current rate: ${opts.currentRate}. Log in to manage your alerts.`,
  };
}
...
export function buildFraudAlertSms(opts: {
  ...
  return {
    to: "",
    body: `54agent FRAUD ALERT [${opts.severity.toUpperCase()}]: ${opts.type} detected. Amount: ${opts.currency} ${opts.amount.toLocaleString()}. Agent: ${opts.agentName}. Take immediate action.`,
  };
}
...
export function buildTransactionConfirmSms(opts: {
  ...
  return {
    to: "",
    body: `54agent: ${opts.type} of ${opts.currency} ${opts.amount.toLocaleString()} confirmed. Ref: ${opts.ref}${opts.customerName ? `. Customer: ${opts.customerName}` : ""}. Thank you.`,
  };
}
...
export function buildOtpSms(opts: {
  ...
  return {
    to: "",
    body: `Your 54agent verification code is: ${opts.otp}. Valid for ${opts.expiresInMinutes} minutes. Do not share this code.`,
  };
}
...
export function buildSettlementSms(opts: {
  ...
  return {
    to: "",
    body: `54agent Daily Settlement: ${opts.agentName}, ${opts.txCount} transactions processed. Volume: ${opts.currency} ${opts.totalVolume.toLocaleString()}. Commission: ${opts.currency} ${opts.commission.toLocaleString()}.`,
  };
}
