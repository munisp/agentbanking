"""
Shareable Links - FastAPI microservice
Dynamic link generation for payment requests, invoices, and agent referrals with tracking
"""
import os
import logging
...
app = FastAPI(title="Shareable Links", description="Dynamic link generation for payment requests, invoices, and agent referrals with tracking", version="1.0.0")

import psycopg2
import psycopg2.extras

DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/shareable_links")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "shareable-links", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
