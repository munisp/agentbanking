...
const StarRating = ({ item }: { item: RecordItem }) => {
    const rating = Math.round(Number(item.rating || 0));
    return (<View style={{ flexDirection: 'row' }}>{[1,2,3,4,5].map(i => (<Text key={i} style={{ fontSize: 12, color: i <= rating ? '#f59e0b' : '#d1d5db' }}>★</Text>))}</View>);
  };
...
export default function SuperAppScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/super_app.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/super_app.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Super App Framework...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Super App Framework</Text>
            <Text style={styles.subtitle}>Mini-app ecosystem</Text>
          </View>
          ...
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🚀</Text>
            <Text style={styles.statLabel}>Daily Launches</Text>
            <Text style={styles.statValue}>{stats?.dailyLaunches ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>💰</Text>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.name || `Record #${item.id}`}</Text>
