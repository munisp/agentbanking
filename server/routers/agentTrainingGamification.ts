...
async function publishagentTrainingGamificationMiddleware(
  action: string,
  ref: string,
  ...
  updateProgress: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await publishAgentTrainingGamificationMiddleware("updateProgress", `${Date.now()}`, { action: "updateProgress" }).catch(() => {});
