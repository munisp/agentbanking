...
export default function IntelligentRoutingEngine() {
  ...
  const summary = trpc.intelligentRoutingEngine.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.intelligentRoutingEngine.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Intelligent Routing Engine</h1>
            <p className="text-muted-foreground">Smart transaction routing, provider selection, and failover management</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Configure Routes initiated")}>Configure Routes</Button>
        <Button variant="outline" onClick={() => toast.success("Test Provider initiated")}>Test Provider</Button>
          </div>
        </div>
        ...
        <Card key="0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Total Routed</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalRouted ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
        ...
        <Card key="3">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(summary?.costSavings)}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Route ID</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Success</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Avg Cost</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{(row.successRate ?? 0) + '%'}</td>
              <td className="p-3 font-mono">{formatCurrency(row.avgCost)}</td>
