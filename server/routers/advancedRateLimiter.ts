...
function validateAdvancedratelimiterInput(
...
const INTEGRITY_RULES_ADVANCEDRATELIMITER = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_ADVANCEDRATELIMITER.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_ADVANCEDRATELIMITER.validateRange(
        data.amount,
        0,
        100_000_000
      )
  ...
  createRule: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "advancedRateLimiter",
        "mutation",
        "Executed advancedRateLimiter mutation"
      );
