  ...
  createSchedule: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        // GL double-entry journal: Automated settlement batch
        try {
          const db = (await getDb())!;
          await db.insert(gl_journal_entries).values({
            entryNumber: `JE-${Date.now()}-${crypto.randomInt(9999).toString().padStart(4, "0")}`,
            description: "Automated settlement batch",
            debitAccountId: 2001,
        ...
        publishEvent("pos.settlement.scheduled" as KafkaTopic, "system", {
          action: "automated_settlement_batch",
          timestamp: new Date().toISOString(),
        });
