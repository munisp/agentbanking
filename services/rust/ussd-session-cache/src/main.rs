...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "ussd-session-cache".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("ussd-session-cache").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "ussd-session-cache").await {
            eprintln!("[ussd-session-cache] Loaded persisted state from PostgreSQL");
