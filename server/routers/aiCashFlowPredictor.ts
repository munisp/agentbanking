...
function validateAicashflowpredictorInput(
...
const INTEGRITY_RULES_AICASHFLOWPREDICTOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_AICASHFLOWPREDICTOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_AICASHFLOWPREDICTOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
// ── Transaction Handling for aiCashFlowPredictor ───────────────────────────────────────
