...
const setThreshold = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "txVelocityMonitor",
      "mutation",
      "Executed txVelocityMonitor mutation"
    );
...
function validateTxvelocitymonitorInput(
...
const INTEGRITY_RULES_TXVELOCITYMONITOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_TXVELOCITYMONITOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_TXVELOCITYMONITOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
export const txVelocityMonitorRouter = router({
  getCurrentTps,
  getVelocityHistory,
  list: getVelocityHistory, // alias of getVelocityHistory (broken-call fix)
