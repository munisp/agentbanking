...
export default function RegulatoryFilingAutomation() {
  ...
  const summary = trpc.regulatoryFilingAutomation.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.regulatoryFilingAutomation.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold">Regulatory Filing Automation</h1>
            <p className="text-muted-foreground">Automated regulatory report filing, compliance deadlines, and submission tracking</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Generate Report initiated")}>Generate Report</Button>
        <Button variant="outline" onClick={() => toast.success("Submit Filing initiated")}>Submit Filing</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalFilings ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.onTime ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.overdue ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.nextDeadline ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Filing ID</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Report Type</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Regulator</th>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{String(row.regulator ?? '—')}</td>
              <td className="p-3">{String(row.period ?? '—')}</td>
              <td className="p-3 text-sm text-muted-foreground">{row.deadline ? new Date(String(row.deadline)).toLocaleDateString() : '—'}</td>
