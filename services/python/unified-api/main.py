...
async def lifespan(app: FastAPI):
    ...
    try:
        _db_pool = await asyncpg.create_pool(
            ...
            command_timeout=30, server_settings={"application_name": "54link-api"}
        )
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "unified-api"),
...
app = FastAPI(
    ...
    description="Production API for 54Link - Agency Banking, Remittance, POS, KYC, Compliance",
    version="14.0.0",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def generate_qr_code(data: QRGenerateRequest, db=Depends(get_db)):
    ...
    qr_data = f"54LINK:{data.agent_id}:{data.transaction_type}:{qr_id}"
...
async def root():
    return {
        "service": "54Link Agency Banking Platform API",
        "version": "14.0.0",
        "status": "operational",
        "docs": "/docs",
