//! 54Link BNPL Engine — Rust Microservice
//!
//! Credit scoring engine, risk assessment, repayment probability calculation
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/bnpl/score — Calculate credit score for BNPL
//!   POST /api/v1/bnpl/risk-assess — Full risk assessment
//!   GET  /api/v1/bnpl/risk/portfolio — Portfolio risk metrics
//!   POST /api/v1/bnpl/repayment-probability — Predict repayment probability
//!
//! Port: 8234
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8234),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "bnpl-engine"});
...
async fn create_record(
    ...
    dapr.publish("bnpl.application.created", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("bnpl-engine-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("bnpl_applications", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("bnpl-engine:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("bnpl_transactions", &payload).await;

    // Persist to PostgreSQL
    let _ = state.postgres.persist("bnpl_engine", &serde_json::json!({
        "service": "bnpl-engine",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("bnpl-engine:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("bnpl_applications", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link BNPL Engine (Rust) starting on port {}", port);
