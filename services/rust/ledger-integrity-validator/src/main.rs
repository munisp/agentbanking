...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "ledger-integrity-validator".to_string();
...
.bind(format!("audit_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()))
                .bind(&val)
                .bind("ledger-integrity-validator")
                .execute(pool).await;
        }
        log.push(AuditEntry {
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("ledger-integrity-validator").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "ledger-integrity-validator").await {
            eprintln!("[ledger-integrity-validator] Loaded persisted state from PostgreSQL");
