...
async function publishplatformHealthScorecardMiddleware(
  action: string,
  ref: string,
  payload: Record<string, unknown>,
) {
  const topic = `scoring.${action}` as any;
  ...
  ingestToLakehouse("scoring", { ref, action, ...payload, timestamp: ts }).catch(() => {});
}
