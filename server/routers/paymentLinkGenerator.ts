...
function validatePaymentlinkgeneratorInput(
...
const INTEGRITY_RULES_PAYMENTLINKGENERATOR = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PAYMENTLINKGENERATOR.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PAYMENTLINKGENERATOR.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _paymentLinkGenerator_db = {
...
// ── Transaction Handling for paymentLinkGenerator ───────────────────────────────────────
