...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "nfc-tap-to-pay"),
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/nfc_tap_to_pay")
...
async def health_check():
    return {
        "status": "healthy",
        "service": "nfc-tap-to-pay-analytics",
        "port": PORT,
...
async def analytics_summary():
    ...
    # Cache and publish
    cache.set("nfc-tap-to-pay:summary", summary)
    await dapr.publish("nfc-tap-to-pay.analytics.updated", summary)
    await fluvio.produce("nfc-tap-to-pay-analytics", summary)
    await lakehouse.ingest("nfc-tap-to-pay_analytics", summary)
...
async def forecast(request: ForecastRequest):
    ...
    await dapr.publish("nfc-tap-to-pay.forecast.generated", result)
    return result
...
async def search_analytics(q: str = QueryParam(..., min_length=1)):
    ...
    results = await opensearch.search("nfc_terminals", q)
    if results:
...
async def register_apisix():
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            route = {
                "uri": "/api/v1/nfc/tap/to/pay/analytics/*",
                "upstream": {
                    "type": "roundrobin",
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"54Link NFC Tap-to-Pay Analytics starting on port {PORT}")
