...
TEMPLATES = {
    "transaction_receipt": (
        "54agent: {tx_type} of NGN {amount} successful.\n"
        "Ref: {ref}\n"
        "Balance: NGN {balance}\n"
        "Date: {date}"
    ),
    "otp_verification": (
        "54agent: Your OTP is {otp}. "
        "Valid for {expiry_minutes} minutes. "
        "Do not share this code."
    ),
    "settlement_summary": (
        "54agent Daily Settlement:\n"
    ...
    "fraud_alert": (
        "54agent ALERT: Suspicious activity detected.\n"
        "Type: {alert_type}\n"
        "Amount: NGN {amount}\n"
        "Action required. Contact support."
    ),
    "float_topup": (
        "54agent: Float top-up of NGN {amount} {status}.\n"
    ...
    "pin_reset": (
        "54agent: Your PIN reset code is {otp}.\n"
