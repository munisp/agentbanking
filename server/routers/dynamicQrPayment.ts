...
export const dynamicQrPaymentRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        const results = await database
          .select()
          .from(qrCodes)
          .orderBy(desc((qrCodes as any).id))
          .limit(input.limit)
          .offset(input.offset);
        ...
        console.error("[dynamicQrPayment] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [record] = await database
        .select()
        .from(qrCodes)
        .where(eq((qrCodes as any).id, input.id))
        .limit(1);

      if (!record) {
        throw new Error(`dynamicQrPayment record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[dynamicQrPayment] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
  ...
  getSummary: protectedProcedure.query(async () => {
    ...
    const [totalRow] = await database.select({ total: count() }).from(qrCodes);
    return {
  ...
  getRecent: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await database
        .select()
        .from(qrCodes)
        .where(gte((qrCodes as any).createdAt, since))
        .orderBy(desc((qrCodes as any).id))
        .limit(input.limit);
