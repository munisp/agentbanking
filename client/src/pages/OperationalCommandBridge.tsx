...
export default function OperationalCommandBridge() {
  ...
  const summary = trpc.operationalCommandBridge.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.operationalCommandBridge.list.useQuery({
    limit: 20,
    offset: page * 20,
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Centralized operations monitoring, real-time metrics, and system
              health
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Run Diagnostics initiated")}>
              Run Diagnostics
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Alert Setup initiated")}
            >
              Alert Setup
            </Button>
          </div>
          ...
          <Card key="0">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.systemsOnline ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="1">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.activeAlerts ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="2">
                    ...
                    <tbody>
                      {items.map((row: any, idx: number) => (
                        ...
                        >
                          ...
                          <td className="p-3">
                            {Number(row.latency ?? 0).toLocaleString()}
                          </td>
