...
export default function RegulatorySandboxTester() {
  ...
  const summary = trpc.regulatorySandboxTester.getSummary.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.regulatorySandboxTester.list.useQuery({
    limit: 20,
    offset: page * 20,
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Controlled testing environment for new financial products and
              regulatory compliance
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("New Test initiated")}>
              New Test
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Submit for Review initiated")}
            >
              Submit for Review
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.approvedProducts ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
