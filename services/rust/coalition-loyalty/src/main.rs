//! 54Link Coalition Loyalty Program — Rust Microservice
//!
//! Real-time points ledger, atomic earn/burn, balance computation
//!
//! ## Integrations:
...
//!
//! ## Endpoints:
//!   POST /api/v1/loyalty/ledger/credit — Credit points to ledger
//!   POST /api/v1/loyalty/ledger/debit — Debit points from ledger
//!   GET  /api/v1/loyalty/ledger/{memberId} — Points ledger history
//!   POST /api/v1/loyalty/ledger/expire — Expire stale points
//!
//! Port: 8288
...
impl Config {
    fn from_env() -> Self {
        Self {
            port: std::env::var("PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8288),
...
impl LakehouseClient {
    async fn ingest(&self, table: &str, data: &serde_json::Value) {
        let payload = serde_json::json!({"table": table, "data": data, "source": "coalition-loyalty"});
...
async fn health(state: axum::extract::State<Arc<AppState>>) -> impl IntoResponse {
    Json(HealthResponse {
        status: "healthy".into(),
        service: "coalition-loyalty".into(),
...
async fn create_record(
    ...
    dapr.publish("loyalty.points.earned", &event).await;

    // Record in TigerBeetle
    state.tigerbeetle.create_transfer(0, 1, 0, 1, 1).await;

    // Stream to Fluvio
    state.fluvio.produce("coalition-loyalty-events", &event).await;

    // Index in OpenSearch
    state.opensearch.index("loyalty_members", &id, &payload).await;

    // Cache result
    state.cache.set(&format!("coalition-loyalty:{}", id), &payload.to_string(), 3600);

    // Ingest to Lakehouse for analytics
    state.lakehouse.ingest("loyalty_events", &payload).await;

    // Persist to PostgreSQL
    let _ = state.postgres.persist("coalition_loyalty", &serde_json::json!({
        "service": "coalition-loyalty",
...
async fn get_record(
    ...
    // Check cache first
    if let Some(cached) = state.cache.get(&format!("coalition-loyalty:{}", id)) {
...
async fn search_records(
    ...
    let results = state.opensearch.search("loyalty_members", &query).await;
    if !results.is_empty() {
...
async fn main() {
    ...
    info!("54Link Coalition Loyalty Program (Rust) starting on port {}", port);
