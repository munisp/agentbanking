import httpx
"""
Store Map Service - FastAPI microservice
Agent and store location mapping with proximity search, clustering, and coverage analysis
"""
import os
import logging
...
if _otel_endpoint:
    try:
        ...
        _resource = Resource.create({
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "store-map-service"),
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/store_map_service")
...
@app.get("/health")
async def health():
    return {"status": "healthy", "service": "store-map-service", "version": "1.0.0", "timestamp": datetime.utcnow().isoformat()}
...
async def startup_event():
    ...
    await publish_kafka("store.map.service.started", {
        "service": "store-map-service",
