...
function validateTransactionreconciliationInput(
...
const INTEGRITY_RULES_TRANSACTIONRECONCILIATION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_TRANSACTIONRECONCILIATION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_TRANSACTIONRECONCILIATION.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _transactionReconciliation_db = {
