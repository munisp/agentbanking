...
function validateLakehouseaiintegrationInput(
...
const INTEGRITY_RULES_LAKEHOUSEAIINTEGRATION = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_LAKEHOUSEAIINTEGRATION.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_LAKEHOUSEAIINTEGRATION.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _lakehouseAiIntegration_db = {
  ...
  train: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "lakehouseAiIntegration",
        "mutation",
        "Executed lakehouseAiIntegration mutation"
      );
