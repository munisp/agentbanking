...
setup_logging("instagram-service")
...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/instagram_service")
...
# Configuration
class Config:
    API_KEY = os.getenv("INSTAGRAM_API_KEY", "demo_key")
    API_SECRET = os.getenv("INSTAGRAM_API_SECRET", "demo_secret")
    WEBHOOK_SECRET = os.getenv("INSTAGRAM_WEBHOOK_SECRET", "webhook_secret")
    API_BASE_URL = os.getenv("INSTAGRAM_API_URL", "https://api.instagram.com")
...
messages_cache = []  # PG-backed via pg_get_list("instagram-service", "messages")
orders_cache = []  # PG-backed via pg_get_list("instagram-service", "orders")
...
async def create_order(order: OrderMessage):
    """Create an order from Instagram message"""
    global order_count
...
async def webhook_handler(request: Request):
    ...
    try:
        ...
        signature = request.headers.get("X-Instagram-Signature", "")
