...
const TermChip = ({ item }: { item: RecordItem }) => {
    const term = item.term || 'First';
    return (<View style={{ backgroundColor: '#ede9fe', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 }}><Text style={{ fontSize: 10, color: '#7c3aed' }}>{term} Term</Text></View>);
  };
...
export default function EducationPaymentsScreen() {
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Education Payments...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Education Payments</Text>
            <Text style={styles.subtitle}>School fees & exam registrations</Text>
          </View>
          <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>🏫</Text>
            <Text style={styles.statLabel}>Schools</Text>
            <Text style={styles.statValue}>{stats?.registeredSchools ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>👨‍🎓</Text>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>₦{stats?.feesCollected ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📝</Text>
            <Text style={styles.statLabel}>Exam Regs</Text>
            <Text style={styles.statValue}>{stats?.examRegistrations ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.schoolName || `Record #${item.id}`}</Text>
