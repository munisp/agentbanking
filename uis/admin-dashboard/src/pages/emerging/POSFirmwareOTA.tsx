...
export default function POSFirmwareOTA() {
  ...
  const summary = trpc.posFirmwareOTA.getStats.useQuery()?.data as
    | Record<string, unknown>
    | undefined;
  const listQ = trpc.posFirmwareOTA.list.useQuery({
    limit: 20,
    offset: page * 20,
  });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Over-the-air POS firmware updates, rollout management, and version
              tracking
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Button onClick={() => toast.success("Push Update initiated")}>
              Push Update
            </Button>
            <Button
              variant="outline"
              onClick={() => toast.success("Schedule Rollout initiated")}
            >
              Schedule Rollout
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card key="1">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.upToDate ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.pendingUpdate ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
          <Card key="3">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(summary?.currentVersion ?? 0).toLocaleString()}
              </div>
            </CardContent>
          </Card>
        </div>
