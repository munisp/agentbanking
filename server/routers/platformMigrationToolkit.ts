...
function validatePlatformmigrationtoolkitInput(
...
const INTEGRITY_RULES_PLATFORMMIGRATIONTOOLKIT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_PLATFORMMIGRATIONTOOLKIT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_PLATFORMMIGRATIONTOOLKIT.validateRange(
        data.amount,
        0,
        100_000_000
      )
...
const _platformMigrationToolkit_db = {
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      auditFinancialAction(
        ...
        "platformMigrationToolkit",
        "mutation",
        "Executed platformMigrationToolkit mutation"
      );
