...
DATABASE_URL = os.environ.get("DATABASE_URL", "postgres://postgres:postgres@localhost:5432/tiktok_service")
...
def log_audit(action: str, entity_id: str, data: str = ""):
    ...
    description="TikTok Shop integration",
    version="1.0.0"
)
...
class Config:
    ...
    API_SECRET = os.getenv("TIKTOK_API_SECRET", "demo_secret")
    WEBHOOK_SECRET = os.getenv("TIKTOK_WEBHOOK_SECRET", "webhook_secret")
    API_BASE_URL = os.getenv("TIKTOK_API_URL", "https://api.tiktok.com")
...
messages_cache = []  # PG-backed via pg_get_list("tiktok-service", "messages")
orders_cache = []  # PG-backed via pg_get_list("tiktok-service", "orders")
...
async def create_order(order: OrderMessage):
    """Create an order from Tiktok message"""
    global order_count
