...
export default function PlatformABTesting() {
  ...
  const summary = trpc.platformABTesting.getSummary.useQuery()?.data as Record<string, unknown> | undefined;
  const listQ = trpc.platformABTesting.list.useQuery({ limit: 20, offset: page * 20, search: search || undefined });
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Experiment management, variant allocation, and statistical analysis</p>
          </div>
          <div className="flex gap-2 flex-wrap">
        <Button onClick={() => toast.success("Create Experiment initiated")}>Create Experiment</Button>
        <Button variant="outline" onClick={() => toast.success("Conclude initiated")}>Conclude</Button>
          </div>
        </div>
        ...
        <Card key="0">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.activeExperiments ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="1">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.totalParticipants ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="2">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Significant Results</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.significantResults ?? 0).toLocaleString()}</div>
          </CardContent>
        </Card>
        <Card key="3">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(summary?.avgLift ?? 0) + "%"}</div>
          </CardContent>
        </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Variants</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Participants</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">P-Value</th>
            <th className="text-left p-3 text-sm font-medium text-muted-foreground">Status</th>
              ...
              <td className="p-3">{Number(row.variants ?? 0).toLocaleString()}</td>
              <td className="p-3">{Number(row.participants ?? 0).toLocaleString()}</td>
              <td className="p-3">{String(row.pValue ?? '—')}</td>
