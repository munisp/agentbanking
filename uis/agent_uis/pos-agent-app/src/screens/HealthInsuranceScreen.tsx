...
const ClaimBadge = ({ item }: { item: RecordItem }) => {
    const st = item.status || 'active';
    const colors: Record<string, string> = { active: '#22c55e', claim_pending: '#f59e0b', claim_paid: '#3b82f6', expired: '#6b7280' };
    const icons: Record<string, string> = { active: '✅', claim_pending: '⏳', claim_paid: '💰', expired: '❌' };
    return (<View style={{ flexDirection: 'row', alignItems: 'center' }}><Text>{icons[st] || '❓'} </Text><Text style={{ fontSize: 11, color: colors[st] || '#6b7280' }}>{st.replace('_', ' ')}</Text></View>);
  };
...
export default function HealthInsuranceScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsRes, listRes] = await Promise.all([
        fetch(`${API_BASE}/health_insurance.getStats`).then(r => r.json()),
        fetch(`${API_BASE}/health_insurance.list?input=${encodeURIComponent(JSON.stringify({ limit: 20, offset: 0 }))}`).then(r => r.json()),
      ]);
  ...
  if (loading) return (<View style={styles.center}><ActivityIndicator size="large" color="#3b82f6" /><Text style={styles.loadingText}>Loading Health Insurance Micro...</Text></View>);
      ...
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <Text style={styles.title}>Health Insurance Micro</Text>
            <Text style={styles.subtitle}>Community health insurance</Text>
          </View>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>Total Premiums</Text>
            <Text style={styles.statValue}>₦{stats?.totalPremiums ?? '—'}</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>⏳</Text>
          ...
          <View style={styles.statCard}>
            <Text style={styles.statIcon}>📊</Text>
            <Text style={styles.statLabel}>Claims Ratio</Text>
            <Text style={styles.statValue}>{stats?.claimRatio ?? '—'}</Text>
          </View>
          </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle} numberOfLines={1}>{item.holderName || `Record #${item.id}`}</Text>
