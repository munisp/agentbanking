...
class _LoyaltyProgramScreenState extends ConsumerState<LoyaltyProgramScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final statsResp = await api.get('/api/trpc/loyalty_program.getStats');
      final listResp = await api.get('/api/trpc/loyalty_program.list?input={"limit":20,"offset":0}');
      setState(() {
  ...
  Widget _buildLoyaltyTier(Map<String, dynamic> item) {
    final points = int.tryParse('${item[\'points_balance\'] ?? 0}') ?? 0;
    String tier; Color color;
    if (points >= 10000) { tier = 'PLATINUM'; color = const Color(0xFF607D8B); } else if (points >= 5000) { tier = 'GOLD'; color = Colors.amber; } else if (points >= 1000) { tier = 'SILVER'; color = Colors.grey; } else { tier = 'BRONZE'; color = Colors.brown; }
    return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: color.withOpacity(0.15), borderRadius: BorderRadius.circular(8)), child: Text(tier, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: color)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(children: [Icon(Icons.card_giftcard, size: 24), const SizedBox(width: 8), const Text('Coalition Loyalty')]),
      ...
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : RefreshIndicator(onRefresh: _loadData, child: CustomScrollView(slivers: [
              SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Coalition Loyalty', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text('54Link Points — earn at any agent, redeem anywhere', style: theme.textTheme.bodyMedium?.copyWith(color: Colors.grey)),
              ]))),
              SliverPadding(padding: const EdgeInsets.symmetric(horizontal: 16),
                sliver: SliverGrid(gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.6),
                  delegate: SliverChildListDelegate([
                              _buildStatCard('Members', '${_stats?['totalMembers'] ?? '\u2014'}', Icons.group, Colors.blue),
                              _buildStatCard('Points Circulating', '${_stats?['pointsCirculating'] ?? '\u2014'}', Icons.stars, Colors.amber),
                              _buildStatCard('Redemption Rate', '${_stats?['redemptionRate'] ?? '\u2014'}', Icons.redeem, Colors.green),
                              _buildStatCard('Partners', '${_stats?['coalitionPartners'] ?? '\u2014'}', Icons.handshake, Colors.purple),
                  ]))),
              ...
              SliverList(delegate: SliverChildBuilderDelegate((context, index) {
                ...
                return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: ListTile(leading: CircleAvatar(backgroundColor: theme.colorScheme.primaryContainer,
                    ...
                    title: Text('${item['customerName'] ?? item['phoneNumber'] ?? \'Record #${item[\'id\']}\'}', overflow: TextOverflow.ellipsis),
                    subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Status: ${item[\'status\'] ?? \'\u2014\'}', style: const TextStyle(fontSize: 12)),
                      const SizedBox(height: 4),
                      _buildLoyaltyTier(item),
                    ]),
