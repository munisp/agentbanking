"""
Router for wechat-service service
...
The real implementation lives in the standalone service (services/wechat-service/).
...
router = APIRouter(prefix="/wechat-service", tags=["wechat-service"])

_UNAVAILABLE = (
    "wechat-service endpoints are not served by this gateway router. "
    "Use the standalone wechat-service service."
)
...
async def health_check():
    return JSONResponse(
        status_code=503,
        content={"status": "unavailable", "service": "wechat-service", "detail": _UNAVAILABLE},
    )
