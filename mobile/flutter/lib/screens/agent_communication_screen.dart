...
class _AgentCommunicationScreenState extends State<AgentCommunicationScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Agent Communication'),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              title: Text(item['messageType']?.toString() ?? 'N/A'),
              subtitle: Text(item['recipient']?.toString() ?? ''),
              trailing: Text(item['subject']?.toString() ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
            ),
          );
        },
      ),
