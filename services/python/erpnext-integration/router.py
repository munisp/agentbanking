...
from .service import (
    ERPNextIntegrationService, SyncTransactionRequest,
    FinancialSummaryRequest, AgentPerformanceReport
)
...
def sync_transaction(payload: SyncTransactionRequest, svc: ERPNextIntegrationService = Depends(get_svc)):
    """Sync a 54link transaction to ERPNext (creates Sales Invoice + Journal Entry)."""
...
def get_sync_status(agent_id: str, limit: int = Query(50, ge=1, le=200), svc: ERPNextIntegrationService = Depends(get_svc)):
...
def retry_failed_syncs(agent_id: str, svc: ERPNextIntegrationService = Depends(get_svc)):
...
def get_financial_summary(payload: FinancialSummaryRequest, svc: ERPNextIntegrationService = Depends(get_svc)):
...
def get_profit_loss(
    ...
    profile = svc.db.query(AgentAccountingProfile).filter(
...
def get_customer_ledger(
    ...
    if not profile or not profile.erp_customer_name:
        raise HTTPException(status_code=404, detail="Agent accounting not set up. Call /erp/agents/{agent_id}/setup first.")
