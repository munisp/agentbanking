...
const approve = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    auditFinancialAction(
      ...
      "mutation",
      "Executed kycDocumentManagement mutation"
    );
...
function validateKycdocumentmanagementInput(
...
const INTEGRITY_RULES_KYCDOCUMENTMANAGEMENT = {
...
function applyIntegrityChecks(data: Record<string, unknown>) {
  ...
  if (typeof data.id === "number") {
    if (!INTEGRITY_RULES_KYCDOCUMENTMANAGEMENT.validateId(data.id))
      errors.push("Invalid id");
  }
  if (typeof data.amount === "number") {
    if (
      !INTEGRITY_RULES_KYCDOCUMENTMANAGEMENT.validateRange(
        data.amount,
        0,
        100_000_000
      )
