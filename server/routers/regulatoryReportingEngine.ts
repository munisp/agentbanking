...
export const regulatoryReportingEngineRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        console.error("[regulatoryReportingEngine] list error:", error);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!record) {
        throw new Error(`regulatoryReportingEngine record #${input.id} not found`);
      }
      return record;
    }),
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      console.error("[regulatoryReportingEngine] getStats error:", error);
      return {
        total: 0,
        active: 0,
        recent: 0,
