...
fn log_audit(action: &str, entity_id: &str) {
    ...
    let flush_svc_name = "adaptive-compression".to_string();
...
fn main() {
    ...
    if let Some(pool) = init_pg_pool("adaptive-compression").await {
        PG_POOL.set(pool).ok();
    }
    // Load persisted state
    if let Some(pool) = get_pg_pool() {
        if let Some(saved) = pg_load_state(pool, "stats", "adaptive-compression").await {
            eprintln!("[adaptive-compression] Loaded persisted state from PostgreSQL");
